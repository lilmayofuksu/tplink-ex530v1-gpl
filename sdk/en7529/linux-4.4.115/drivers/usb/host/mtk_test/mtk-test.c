...
#define RESOURCE_NUM_MAC	0
#define RESOURCE_NUM_IPPC	1
...
int g_xhci_irq;
unsigned long g_xhci_u3h_base;
unsigned long g_xhci_ippc_base;
struct mtk_u3h_hw *u3h_hw = NULL;
resource_size_t g_res_start;
resource_size_t g_res_size;
#endif
...
#ifdef TCSUPPORT_CPU_ARMV8
static int usb_hcd_versatile_base_probe(struct platform_device* pdev){
	...
	if (!pdev->dev.of_node) {
		dev_err(&pdev->dev, "No xhci hcd DT node found (Keng-Chih)\n");
		return -EINVAL;
	}
	u3h_hw = devm_kzalloc(&pdev->dev, sizeof(struct mtk_u3h_hw), GFP_KERNEL);
	if (!u3h_hw){
		printk("\n devm_kzalloc failed (Keng-Chih)\n");
		return -ENOMEM;
	}
	platform_set_drvdata(pdev, u3h_hw);
	/* get usb u3h base address */	  
	res = platform_get_resource(pdev, IORESOURCE_MEM, RESOURCE_NUM_MAC);
	if (!res)		 
		return -ENODEV;
	
	u3h_hw->u3h_virtual_base = devm_ioremap_resource(&pdev->dev, res);
	g_res_start = res->start;
	g_res_size = resource_size(res);
	g_xhci_u3h_base = (unsigned long)u3h_hw->u3h_virtual_base;

	/* get usb ippc base address */ 
	res_ippc = platform_get_resource(pdev, IORESOURCE_MEM, RESOURCE_NUM_IPPC);
	if (!res_ippc)
		return -ENODEV;
	u3h_hw->ippc_virtual_base = devm_ioremap_resource(&pdev->dev, res_ippc);

	if (!u3h_hw->ippc_virtual_base && !u3h_hw->u3h_virtual_base) {		 
		dev_dbg(&pdev->dev, "error mapping memory for u3h & ippc \n");		
		release_mem_region(res_ippc->start, resource_size(res_ippc));				
		return -EFAULT;    
	}
	g_xhci_ippc_base = (unsigned long)u3h_hw->ippc_virtual_base;

	/* get irq num */
	irq = platform_get_irq(pdev, 0);
	if(irq <= 0) {
		printk("\n get xhci irq number failed \n");
		return -ENODEV;
	}
	g_xhci_irq = irq;
	
#if 0
	printk("[xhci u3h] res->name:%s\n", res->name);
	printk("[xhci u3h] res->start:0x%x ===\n", res->start);
	printk("[xhci u3h] res->end:0x%x ===\n", res->end);
	printk("[xhci u3h] u3h_hw->u3h_virtual_base:0x%x\n", (unsigned long)u3h_hw->u3h_virtual_base);
	printk("[xhci ippc] res_ippc->name:%s\n", res_ippc->name);
	printk("[xhci ippc] res_ippc->start:0x%x ===\n", res_ippc->start);
	printk("[xhci ippc] res_ippc->end:0x%x ===\n", res_ippc->end);
	printk("[xhci ippc] u3h_hw->ippc_virtual_base:0x%x\n", (unsigned long)u3h_hw->ippc_virtual_base);
#endif

	return 0;
}

static int usb_hcd_versatile_base_remove(struct platform_device *pdev)
{
    struct resource 		*res;	
    struct resource 		*res_ippc;	
	
    res = platform_get_resource(pdev, IORESOURCE_MEM, RESOURCE_NUM_MAC);
    iounmap(u3h_hw->u3h_virtual_base);
...
const struct of_device_id usb_xhci_arm64_of_match[] = {	
	{ .compatible = "econet,ecnt-xhci" },	
	{ }
};
MODULE_DEVICE_TABLE(of, usb_xhci_arm64_of_match);

static struct platform_driver xhci_versatile_base_driver = {
	.probe =	usb_hcd_versatile_base_probe,
	.remove =	usb_hcd_versatile_base_remove,
	.driver = {
        .name = "ecnt-xhci",
        .of_match_table = usb_xhci_arm64_of_match,		
	}
};
#endif	
...
#ifdef TCSUPPORT_CPU_ARMV8
	retval = platform_driver_register(&xhci_versatile_base_driver);
	if (retval < 0)
	{
