...
#ifdef CONFIG_NEED_SG_DMA_LENGTH
            sg->dma_length = tmp_mtk_sg->sg_len;
#endif		
...
int f_power_config_lpm(u32 slot_id, u32 hirdm, u32 L1_timeout, u32 rwe, u32 besl, u32 besld, u32 hle
	...
	xhci = hcd_to_xhci(my_hcd);
	/*set FPGA Phy clock free run and test pm*/
	if(isFPGA){
		addr = SSUSB_U2_SYS_BASE + 0x4;
        	temp = xhci_readl(xhci, addr);
		temp &= ~(0x1);
		xhci_writel(xhci, temp, addr);
		printk("FPGA set Phy Clock free run addr %x Value %x\n", addr, xhci_readl(xhci, addr));
	}
