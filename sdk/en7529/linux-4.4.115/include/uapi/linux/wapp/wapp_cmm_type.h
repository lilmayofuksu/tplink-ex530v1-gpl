...
#endif
#ifdef CONFIG_TP_IMAGE
#define MAX_STA_PER_WDEV	64
typedef struct GNU_PACKED wdev_sta_mac_s {
	uint8_t addr[MAC_ADDR_LEN];
	uint32_t associated_time;
} wdev_sta_mac_t;

typedef struct GNU_PACKED wdev_sta_list_s {
	unsigned char count;
	wdev_sta_mac_t maclist[MAX_STA_PER_WDEV];
} wdev_sta_list_t;
#endif
...
#ifdef CONFIG_TP_IMAGE
typedef struct GNU_PACKED _map_assoc_ts_cfg
{
	u8 ifname[IFNAMSIZ];
	u16 vid;
} map_assoc_ts_cfg;
