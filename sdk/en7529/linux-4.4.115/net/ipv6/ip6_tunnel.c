			 ...
			 )
{
	...
	if (needFrag) {
		//skb->ignore_df = 1;
		skb->ignore_df = ignore_df;
		skbDstMtu = ip6_skb_dst_mtu(skb);
		/* BUG562096: skb的frag_max_size需要不大于MTU,避免分片失败修改Dslite的MTU,从而导致没有在IPv6层分片 */
		if (IP6CB(skb)->frag_max_size > skbDstMtu)
		{
			IP6CB(skb)->frag_max_size = skbDstMtu;
		}
