...
void addrconf_mod_rs_timer(struct inet6_dev *idev,
				  unsigned long when)
{
...
static void addrconf_rs_timer(unsigned long data)
{
	...
	if ( 0 == ecnt_addrconf_ppp_ondemand_config_inline_hook(idev, idev) )
		goto out;
...
static void addrconf_dad_completed(struct inet6_ifaddr *ifp)
{
	...
	if (send_rs) {
		...
		if ( 0 == ecnt_addrconf_ppp_ondemand_down_inline_hook(dev, ifp) )
		{
...
restart:
		hlist_for_each_entry_rcu_bh(ifp, &inet6_addr_lst[i], addr_lst) {
			...
			ecnt_addrconf_verify_rtnl_inline_hook(ifp, age);
...
/* porting from linux-3.10.14, by hqj 2019.02.12 */
static
int addrconf_sendrs(struct ctl_table *ctl, int write,
			    void __user *buffer, size_t *lenp,
			    loff_t *ppos)
{
	//int *valp = ctl->data;
