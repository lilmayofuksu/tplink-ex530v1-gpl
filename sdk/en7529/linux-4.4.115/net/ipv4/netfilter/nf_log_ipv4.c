...
fallback:
#ifdef CONFIG_TP_FIREWALL_LOG
	if (dev->hard_header_len >= 6 && skb->mac_header != skb->network_header)
	{
		const unsigned char *p = skb_mac_header(skb);
		unsigned int i;

		nf_log_buf_add(m, "MACDST=%02x", *p++);
		for (i = 1; i < 6; i++, p++)
		{
		...
		if (dev->hard_header_len >= 12)
		{
			nf_log_buf_add(m, "MACSRC=%02x", *p++);
			for (i = 1; i < 6; i++, p++)
			{
				nf_log_buf_add(m, ":%02x", *p);
			}
		...
		{
			nf_log_buf_add(m, "MACSRC= ");
		}
	}
	else
	{
		nf_log_buf_add(m, "MACDST= MACSRC= ");
	}
#else /* CONFIG_TP_FIREWALL_LOG */
