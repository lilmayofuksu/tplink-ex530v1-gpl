...
get_skb_tcpdata(struct sk_buff* skb, char** pptcpdata, uint* ptcpdatalen)
{
	...
	*ptcpdatalen = ((char*)skb_transport_header(skb) + skb->len) - *pptcpdata;
	...
	ecnt_get_skb_tcpdata_inline_hook(skb, pptcpdata, ptcpdatalen);
}
			 ...
			 u_int16_t rtp_srcport)
{
	...
	nf_ct_expect_init(rtp_new, NF_CT_EXPECT_CLASS_DEFAULT,
					...
					IPPROTO_UDP, NULL, &be_port);

	pr_debug("init_rtp_expect (%p) rtpip=%pI4, dstport=[%d], srcport=[%d]\n",
		rtp_new, &rtpip, port, rtp_srcport);

	rtp_new->flags = NF_CT_EXPECT_PERMANENT;
	rtp_new->expectfn = nf_nat_rtsp_expected;
	rtp_new->saved_addr = ct->tuplehash[!dir].tuple.src.u3;
	rtp_new->saved_proto.udp.port = be_port;
	rtp_new->dir = dir;
	...
	tuple.src.u.all = htons(rtp_srcport);
	tuple.src.l3num = PF_INET;
	tuple.dst.u3.ip = ct->tuplehash[dir].tuple.dst.u3.ip;
	tuple.dst.u.all = be_port;
			 ...
			 u_int16_t rtp_srcport)
{
	int rc = NF_ACCEPT;
	int uport =0;
	u_int16_t loport = ntohs(ct->loport);
	u_int16_t hiport = ntohs(ct->hiport);
	
	if ( loport < hiport )
	{
		/* init port range*/
		for ( uport = loport;  uport <= hiport; uport ++ )
