...
/*add by liaoyilu 2020-11-30, for hyfi-bridge*/
#ifdef CONFIG_HYFI_BRIDGE 
...
/*add by liaoyilu 2020-11-30, for hyfi-bridge*/
	...
	if (p->flags & BR_LEARNING){
		...
		ecnt_br_fdb_set_vlan_inline_hook(br, eth_hdr(skb)->h_source, vid, skb);
	}
...
/*add by liaoyilu 2020-11-30, for hyfi-bridge*/
		...
		ecnt_br_multicast_recover_vlan_inline_hook(skb);
...
static int br_handle_local_finish(struct net *net, struct sock *sk, struct sk_buff *skb)
{
	...
	if (p->flags & BR_LEARNING && br_should_learn(p, skb, &vid)){
		...
		ecnt_br_fdb_set_vlan_inline_hook(p->br, eth_hdr(skb)->h_source, vid, skb);
	}
