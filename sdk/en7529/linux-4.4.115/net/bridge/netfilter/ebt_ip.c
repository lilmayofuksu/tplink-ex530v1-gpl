...
ebt_ip_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	/*xyzhu_20100413*/
	if ( info->bitmask & EBT_IP_SOURCE &&
		FWINV( (ntohl(ih->saddr & info->smsk[0]) < ntohl(info->saddr[0])) ||
			   (ntohl(ih->saddr & info->smsk[0]) > ntohl(info->saddr[1])), EBT_IP_SOURCE ) ) {
		return false;
	}

	if ( info->bitmask & EBT_IP_DEST &&
		FWINV( (ntohl(ih->daddr & info->dmsk[0]) < ntohl(info->daddr[0])) ||
			   (ntohl(ih->daddr & info->dmsk[0]) > ntohl(info->daddr[1])), EBT_IP_DEST ) ) {
		return false;	   
	}
