...
#endif
{
	...
	#else
	ep = (struct xhci_virt_ep *) from_timer(ep, t, stop_cmd_timer);
	#endif
	xhci = ep->xhci;
...
#else
void xhci_handle_command_timeout(struct timer_list *t)
#endif
{
	struct xhci_hcd *xhci;
	int ret;
	...
	#else
	xhci = (struct xhci_hcd *) from_timer(xhci, t, cmd_timer);
	#endif
