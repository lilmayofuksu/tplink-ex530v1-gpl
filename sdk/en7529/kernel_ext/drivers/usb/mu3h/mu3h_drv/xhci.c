...
#endif
{
	...
	#else
	xhci = (struct xhci_hcd *)from_timer(xhci, t, comp_mode_recovery_timer);
	#endif
...
static void compliance_mode_recovery_timer_init(struct xhci_hcd *xhci)
{
	...
	set_timer_slack(&xhci->comp_mode_recovery_timer,
			msecs_to_jiffies(COMP_MODE_RCVRY_MSECS));
	#else
	timer_setup(&xhci->comp_mode_recovery_timer, compliance_mode_recovery, 0);
	xhci->comp_mode_recovery_timer.expires = jiffies +
			msecs_to_jiffies(COMP_MODE_RCVRY_MSECS);
	#endif
...
#else
	.flags =		HCD_MEMORY | HCD_USB3 | HCD_SHARED | HCD_DMA,
#endif
