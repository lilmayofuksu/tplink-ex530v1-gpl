...
static void setIPPC_SSM(struct device *dev)
{
...
#else
void setIPPC_SSM(struct device *dev) {}
#endif
void enableAllClockPower(struct device *dev){
	...
	if(isEN7523DU || isEN7523GU || isEN7523SU || isEN7523DT || isEN7523DTM) {
		u3_port_num = 0;
	} else {
...
//	u32 temp;
	...
	if(port_rev == 0x3){
		addr = (u32 __iomem *)(u3h_hw->ippc_virtual_base + U3H_SSUSB_U3_CTRL_0P + real_index * 0x08);
		u3h_writelmsk(addr,0x1<<1,SSUSB_U3_PORT_PDN);
	}
	else if(port_rev == 0x2){
		addr = (u32 __iomem *)(u3h_hw->ippc_virtual_base + U3H_SSUSB_U2_CTRL_0P + real_index * 0x08);
