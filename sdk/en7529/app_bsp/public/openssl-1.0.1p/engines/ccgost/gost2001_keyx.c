                           ...
                           const unsigned char *ukm)
{
    ...
    X = BN_CTX_get(ctx);
    Y = BN_CTX_get(ctx);
    res = EC_GROUP_get_order(EC_KEY_get0_group(priv_key), order, ctx);
    res = BN_mod_mul(p, key, UKM, order, ctx);
    res = EC_POINT_mul(EC_KEY_get0_group(priv_key), pnt, NULL, pub_key, p, ctx);
    res = EC_POINT_get_affine_coordinates_GFp(EC_KEY_get0_group(priv_key),
                                        pnt, X, Y, ctx);
    /*
