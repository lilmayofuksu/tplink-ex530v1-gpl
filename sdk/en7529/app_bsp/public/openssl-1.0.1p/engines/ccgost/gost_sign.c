...
DSA_SIG *gost_do_sign(const unsigned char *dgst, int dlen, DSA *dsa)
{
    ...
    do {
        ...
        res = BN_mod_mul(tmp, dsa->priv_key, newsig->r, dsa->q, ctx);
        res = BN_mod_mul(tmp2, k, md, dsa->q, ctx);
        if (!newsig->s) {
            newsig->s = BN_new();
            if(!newsig->s) {
                GOSTerr(GOST_F_GOST_DO_SIGN, ERR_R_MALLOC_FAILURE);
        ...
        res = BN_mod_add(newsig->s, tmp, tmp2, dsa->q, ctx);
    }
                   ...
                   DSA_SIG *sig, DSA *dsa)
{
    ...
    res = BN_mod_mul(z1, sig->s, v, dsa->q, ctx);
    BN_sub(tmp, dsa->q, sig->r);
    res = BN_mod_mul(z2, tmp, v, dsa->p, ctx);
    BN_mod_exp(tmp, dsa->g, z1, dsa->p, ctx);
    BN_mod_exp(tmp2, dsa->pub_key, z2, dsa->p, ctx);
    res = BN_mod_mul(tmp3, tmp, tmp2, dsa->p, ctx);
...
int gost_sign_keygen(DSA *dsa)
{
    ...
    res = BN_rand_range(dsa->priv_key, dsa->q);
