...
static void write_pidfile(char *funcName)
{
	...
	snprintf(pid_file_path, sizeof(pid_file_path), "%s%s_%s.pid", path_name, funcName, MainIfName);
...
int Apd_init_sockets(rtapd *rtapd)
{
    ...
    for(i = 0; i < rtapd->conf->SsidNum; i++)
    {
		...
		if (i == 0)
			strncpy(ifr.ifr_name, rtapd->main_wlan_name,IFNAMSIZ-1);
		else
...
int main(int argc, char *argv[])
{
 ...
 //   if (strcmp(argv[0], "rtinicapd") == 0)
    if (strcmp(argv[0], "rtdot1xd_AC") == 0 || strcmp(argv[0],"/userfs/bin/rtdot1xd_AC") ==0)
		...
		switch (c)
        {
			...
			case 'i':
				...
				if (strlen(infName))
				{
					...
					strncpy(MainIfName, infName, IFNAMSIZ-1);
				}
			...
			case 'p':
				...
				if (strlen(preName))
				{
					...
					strncpy(prefix_name, preName, IFNAMSIZ-1);
				}
	...
	DBGPRINT(RT_DEBUG_TRACE, "Ralink DOT1X daemon, version = '%s' \n", dot1x_version);
