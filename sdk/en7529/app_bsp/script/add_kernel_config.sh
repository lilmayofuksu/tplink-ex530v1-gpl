...
if [ "$TCSUPPORT_CZ_GENERAL" != "" ] || [ "$TCSUPPORT_SAMBA" != "" ] || [ "$TCSUPPORT_SAMBA_IPv6" != "" ] || [ "$TCSUPPORT_SAMBA_4" != "" ] || [ "$TCSUPPORT_DMS" != "" ] || [ "$TCSUPPORT_6RD" != "" ] || [ "$TCSUPPORT_IPV6_PRIVACYADDRS" != "" ] || [ "$TCSUPPORT_VPN" != "" ] || [ "$TCSUPPORT_CPU_EN7512" != "" ] || [ "$TCSUPPORT_CPU_EN7521" != "" ] || [ "$TCSUPPORT_CPU_EN7516" != "" ] || [ "$TCSUPPORT_CPU_EN7527" != "" ] || [ "$TCSUPPORT_CPU_EN7580" != "" ]; then
	...
	if [ "$TCSUPPORT_SAMBA" != "" ] || [ "$TCSUPPORT_SAMBA_IPv6" != "" ] || [ "$TCSUPPORT_SAMBA_4" != "" ];then
		...
		if [ "$TCSUPPORT_KERNEL" != "y" ];then
			SEDCMD="$SEDCMD -e 's/# CONFIG_EFI_PARTITION is not set/CONFIG_EFI_PARTITION=y/'"
		fi
	fi
	...
	if [ "$TCSUPPORT_CPU_EN7512" != "" ] || [ "$TCSUPPORT_CPU_EN7521" != "" ] || [ "$TCSUPPORT_CPU_EN7516" != "" ] || [ "$TCSUPPORT_CPU_EN7527" != "" ] || [ "$TCSUPPORT_CPU_EN7580" != "" ];then

		echo "delete previous kernel config for HIGHMEM"
		SEDCMD="$SEDCMD -e 's/CONFIG_HIGHMEM=y//'"
		SEDCMD="$SEDCMD -e 's/CONFIG_BOUNCE=y//'"
		SEDCMD="$SEDCMD -e 's/# CONFIG_DEBUG_HIGHMEM is not set//'"
		SEDCMD="$SEDCMD -e 's/# CONFIG_HIGHMEM is not set//'"

		if [ "$TCSUPPORT_MIPS_1004K" != "" ];then
			if [ "$TCSUPPORT_HIGHMEM" != "" ];then
				echo "Add kernel config for HIGHMRM"
				SEDCMD="$SEDCMD -e '130a CONFIG_HIGHMEM=y'"
				SEDCMD="$SEDCMD -e '131a CONFIG_BOUNCE=y'"
				SEDCMD="$SEDCMD -e '132a # CONFIG_DEBUG_HIGHMEM is not set'"
			else
				SEDCMD="$SEDCMD -e '130a # CONFIG_HIGHMEM is not set'"
			fi
		else
			if [ "$TCSUPPORT_HIGHMEM" != "" ];then
				echo "Add kernel config for HIGHMRM"
				...
				SEDCMD="$SEDCMD -e '131a CONFIG_BOUNCE=y'"
				SEDCMD="$SEDCMD -e '132a # CONFIG_DEBUG_HIGHMEM is not set'"
				echo "delete previous kernel config for PAGE_SIZE"
				SEDCMD="$SEDCMD -e 's/CONFIG_PAGE_SIZE_4KB=y//'"
				SEDCMD="$SEDCMD -e 's/CONFIG_PAGE_SIZE_16KB=y//'"
				SEDCMD="$SEDCMD -e 's/# CONFIG_PAGE_SIZE_4KB is not set//'"
				SEDCMD="$SEDCMD -e 's/# CONFIG_PAGE_SIZE_16KB is not set//'"
				echo "Add kernel config for PAGE_SIZE_16KB"
				SEDCMD="$SEDCMD -e '108a # CONFIG_PAGE_SIZE_4KB is not set'"
				SEDCMD="$SEDCMD -e '109a CONFIG_PAGE_SIZE_16KB=y'"
			else
				SEDCMD="$SEDCMD -e '130a # CONFIG_HIGHMEM is not set'"
			fi
		fi
