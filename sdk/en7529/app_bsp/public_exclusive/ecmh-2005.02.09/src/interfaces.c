...
***************************************
 $Author: aaron.zhang $
 $Id: //BBN_Linux/Branch/Branch_for_New_ECNT_Kernel4_4_7527BSP_20180411/tclinux_phoenix/apps/public/ecmh-2005.02.09/src/interfaces.c#1 $
 $Date: 2018/04/11 $
**************************************/
...
#endif
{
	...
	if(strstr(intn->name, "eth") != NULL || strstr(intn->name, "ra") != NULL
		|| strstr(intn->name, "oam") != NULL
		|| strstr(intn->name, "omci") != NULL){
	...
	if((strstr(intn->name, "pon0") != NULL) || (strstr(intn->name, "ppp") != NULL) ){
	...
	if (g_conf->promisc)
	{
		...
		memset(&ifr, 0, sizeof(ifr));
		snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s", intn->name );
