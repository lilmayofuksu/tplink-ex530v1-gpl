/*
 * "$Id: //BBN_Linux/DEV/main/tclinux_phoenix/apps/public/mxml-2.4/mxmldoc.c#1 $"
 *
             ...
             mxml_node_t *type)		/* I - Type nodes */
{
  mxml_node_t	*variable,		/* New variable */
		*node,			/* Current node */
		*next;			/* Next node */
  static char		buffer[16384];		/* String buffer */
  ...
  */
  ...
  if (node)
  {
	...
	*bufptr++ = ' ';
      ...
      strncpy(bufptr, node->value.text.string, sizeof(buffer) - strlen(buffer) - 1);
  ...
  */
  ...
  else
  {
    ...
    */
    ...
    strncpy(buffer, type->last_child->value.text.string, sizeof(buffer) - 1);
...
{
  static char		text[10240];		/* Description text */
  char since[255],		/* @since value */
          ...
          mxml_node_t *tree)		/* I - Function tree */
{
  ...
  static char		buffer[65536];		/* String buffer */
  char *bufptr;		/* Pointer into buffer */
...
{
  static char	text[10240];			/* Text for description */
...
/*
 * End of "$Id: //BBN_Linux/DEV/main/tclinux_phoenix/apps/public/mxml-2.4/mxmldoc.c#1 $".
 */
