...
static void command_jump(struct iptables_command_state *cs)
{
	...
	if (cs->target->real_name == NULL) {
		strncpy(cs->target->t->u.user.name, cs->jumpto, sizeof(cs->target->t->u.user.name) - 1);
	} else {
		strncpy(cs->target->t->u.user.name, cs->target->real_name, sizeof(cs->target->t->u.user.name) - 1);
		...
		struct xtc_handle **handle, bool restore)
{
	...
	if (command == CMD_APPEND
		...
		if (cs.jumpto && cs.target && ip6tc_is_chain(cs.jumpto, *handle)) {
			fprintf(stderr,
