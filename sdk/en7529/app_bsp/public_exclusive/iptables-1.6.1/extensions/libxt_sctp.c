...
print_chunk_flags(uint32_t chunknum, uint8_t chunk_flags, uint8_t chunk_flags_mask)
{
	...
	for (j = 0; j < ARRAY_SIZE(sctp_chunk_names); ++j){
		if (sctp_chunk_names[j].chunk_type == chunknum){
			break;
		}
	}
	if(j == ARRAY_SIZE(sctp_chunk_names)){
		printf("chunknum do not exit in sctp_chunk_names!!!");
		return;
	}
	...
	for (i = 7; i >= 0; i--) {
		if (chunk_flags_mask & (1 << i)) {
			if (chunk_flags & (1 << i)) {
				printf("%c", sctp_chunk_names[j].valid_flags[7-i]);
			} else {
				printf("%c", tolower(sctp_chunk_names[j].valid_flags[7-i]));
			}
		}
	}
}
