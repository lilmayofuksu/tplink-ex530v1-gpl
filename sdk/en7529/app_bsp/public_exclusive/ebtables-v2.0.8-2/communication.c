...
static struct ebt_replace *translate_user2kernel(struct ebt_u_replace *u_repl)
{
	...
	char *p, *base;
	int i, j;
	unsigned int entries_size = 0, *chain_offsets = NULL;
	...
	if (!chain_offsets)
		ebt_print_memory();
	memset(chain_offsets, 0, u_repl->num_chains * sizeof(unsigned int));
	/* Determine size */
   ...
   char command)
{
	...
	if(repl->nentries > 
		(((~(unsigned int)0)/2 - sizeof(struct ebt_replace) - repl->entries_size)/sizeof(struct ebt_counter)))
		goto close_file;
	...
	if (fseek(file, sizeof(struct ebt_replace), SEEK_SET) ||
	   ...
	   fseek(file, sizeof(struct ebt_replace) + repl->entries_size,
		 SEEK_SET)
	   || (repl->counters != NULL &&fread((char *)repl->counters, sizeof(char),
