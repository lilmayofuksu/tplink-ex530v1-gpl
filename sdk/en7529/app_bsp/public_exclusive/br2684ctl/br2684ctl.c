...
int create_br(int itfnum, int payload)
{
  ...
  if (lastsock<0) {
    ...
    if( itfnum>=0 && itfnum<1234567890){
      ...
      ni.mtu = 1500;
      sprintf(ni.ifname, "%s%d", intf_name, itfnum);
		...
		int qos_type, int pcr, int scr, int mbs, int bufsize)
{
    ...
    sprintf(be.ifspec.spec.ifname, "%s%d", intf_name, lastitf);
...
void usage(char *s)
{
  printf("usage: %s [-b] [[-c number] [-e 0|1|2|3] [-n intf_name][-a [itf.]vpi.vci]*]*\n", s);
...
int main (int argc, char **argv)
{
  ...
  if (argc>1)
    while ((c = getopt(argc, argv,"a:bc:e:t:p:q:m:s:n:?h")) !=EOF)
      switch (c) {
      case 'a':
		params.astr=optarg;
	    break;
