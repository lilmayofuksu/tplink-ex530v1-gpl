...
extern unsigned int tftp_limit;
#define	EcntTftpOurPort (69);
#endif
#endif
...
#if defined(CONFIG_ECNT)
#if defined(CONFIG_ECNT_TFTP_AUTO_UPGRADE)
		if ((load_addr + newsize) >= tftp_limit)
		{
			printf("Error: TFTP file size is too big.\n");
			printf("Exceed limit 0x%lx",tftp_limit);
			net_set_state(NETLOOP_FAIL);
			return;
		}
#endif
	    ...
	    unsigned len)
{
	...
	case TFTP_WRQ:
		...
		if (i >= MAX_LEN)
			i = (MAX_LEN - 1);
		strncpy(tftp_filename, pkt, i);
		tftp_filename[i] = '\0';
		setenv("filename", tftp_filename);
#endif
#endif
		TftpRemoteIP = sip;
		TftpRemotePort = src;
#if defined(CONFIG_ECNT)
		TftpOurPort = EcntTftpOurPort;
#else
