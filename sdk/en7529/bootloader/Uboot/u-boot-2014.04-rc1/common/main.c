...
#include "../drivers/misc/ecnt/net/ecnt_eth.h"
...
#if defined(INCLUDE_BOOT_AGINETCONFIG)
		int ret = AginetConfigProcess(60000, 3);
		if (ret != 0)
		{
			printf("\nAginetConfigProcess end, ret = %d\n", ret);
			abort  = 1;
			break;
		}
#endif
...
#if defined(TCSUPPORT_MTD_PARTITIONS_CMDLINE)
	if(ecnt_parse_cmdline_partitions() == 0) {
		printf("error ecnt_parse_cmdline_partitions() init fail.\n");
		hang();
	}

#ifndef CONFIG_TP_IMAGE
	if (abort)
		return ecnt_abortboot_keyed(bootdelay);
#endif /* CONFIG_TP_IMAGE */
#endif

#ifdef CONFIG_TP_IMAGE
	tp_boot_set_all_led_off();
	if (abort)
	{
