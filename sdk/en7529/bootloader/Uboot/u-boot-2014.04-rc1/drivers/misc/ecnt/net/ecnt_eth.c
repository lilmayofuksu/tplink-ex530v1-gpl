...
void macResetSwMAC(void)
{
	...
	mdelay(1);
	write_reg_word(GSW_MFC, 0x404040e0);
	write_reg_word(GSW_APC, 0x08160816);
#endif /* CONFIG_TP_IMAGE */

}

#if defined(CONFIG_ECNT_MULTIUPGRADE)
...
#if defined(INCLUDE_BOOT_AGINETCONFIG)
		else if (ntohs(eth_hdr->h_proto) == AGC_ETHER_TYPE)
		{
			cp_recvProcess(dscpInfo.skb->data, dscpInfo.skb->len);
		}
...
static int ecnt_eth_init(struct eth_device* dev, bd_t* bis)
{
	if(ecnt_eth_initd == 0 ){
		...
		uint8 k;
		int err;
#if defined(TC3262) && defined(CONFIG_ECNT_MULTIUPGRADE)
		uint32 reg_val = 0;
#endif
		...
		macGetMacAddr(mac_p, NetOurEther);
#else
