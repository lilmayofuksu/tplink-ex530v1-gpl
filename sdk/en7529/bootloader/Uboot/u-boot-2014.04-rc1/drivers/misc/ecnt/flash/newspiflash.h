...
#if defined(CONFIG_ECNT_UBOOT)
#define SF_CLK_CHANGE				(0x1FA201B8)
#else
...
#define SF_OP_FAST_RD_QUAD_OUT			0x6B		/* Fast Read Data Quad Output */
#define SF_OP_FAST_RD_DUAL_IO			0xBB		/* Fast Read Data Dual I/O */
#define SF_OP_FAST_RD_QUAD_IO			0xEB		/* Fast Read Data Quad I/O */
#define SF_OP_PAGE_PROGRAM				0x02		/* Page Program */
#define SF_OP_QUAD_PAGE_PROGRAM			0x32		/* Quad Page Program */
...
#define SF_OP_4BYTES_FAST_RD_QUAD_OUT	0x6C		/* Fast Read Data Quad Output in 4Bytes Address */
#define SF_OP_4BYTES_FAST_RD_DUAL_IO	0xBC		/* Fast Read Data Dual I/O in 4Bytes Address */
#define SF_OP_4BYTES_FAST_RD_QUAD_IO	0xEC		/* Fast Read Data Quad I/O in 4Bytes Address */
...
#define	FAST_READ_QUADOUT	0x4
#define	FAST_READ_QUADIO	0x5

#define	PAGE_PROGRAM		0x0
#define	QUAD_PAGE_PROGRAM	0x1
...
//		len: 	the value is [1, FlashSize-1]
//		mode:	1 means 4bytes, 0 means 3bytes
#define SEND_FASTREAD_QUADOUT_CMD(dAddr, data, len, mode)			\
		{															\
		  SEND_MANUAL_BEGIN_CMD										\
		  SEND_CSL_CMD												\
		  /* write op_cmd 0x6B */									\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTS, OP_REPEAT_ONE) 			\
		  SEND_DFIFO_WRITE_CMD(SF_OP_FAST_RD_QUAD_OUT)				\
		  /* write address */										\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTS, ((mode)?0x04:0x03))		\
		  if(mode){													\
			 SEND_DFIFO_WRITE_CMD(bHiByte(wHiWord(dAddr))) 			\
		  ...
		  while((len) > 0) {										\
			 if((len) % SF_RD_MAX) {								\
				SEND_OPFIFO_WRITE_CMD(OP_INQ, ((len)%SF_RD_MAX))	\
				do													\
				{													\
					SEND_DFIFO_READ_CMD(*((data)++)) 				\
				} while ((--(len)) % SF_RD_MAX); 					\
			 } else { 												\
				SEND_OPFIFO_WRITE_CMD(OP_INQ, SF_RD_MAX)			\
				do													\
				{													\
					SEND_DFIFO_READ_CMD(*((data)++)) 				\
				} while ((--(len)) % SF_RD_MAX); 					\
...
// Send fast read quad I/O command 
// Parameters:
//		dAddr: 	3bytes or 4bytes length, depend the value of mode
//		data:	type is char*
//		len: 	the value is [1, FlashSize-1]
//		mode:	1 means 4bytes, 0 means 3bytes
#define SEND_FASTREAD_QUADIO_CMD(dAddr, data, len, mode)			\
		{															\
		  SEND_MANUAL_BEGIN_CMD										\
		  SEND_CSL_CMD												\
		  /* write op_cmd 0xEB */									\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTS, OP_REPEAT_ONE) 			\
		  SEND_DFIFO_WRITE_CMD(SF_OP_FAST_RD_QUAD_IO)				\
		  /* write address */										\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTQ, ((mode)?0x04:0x03))		\
		  if(mode){													\
			 SEND_DFIFO_WRITE_CMD(bHiByte(wHiWord(dAddr))) 			\
		  }															\
		  SEND_DFIFO_WRITE_CMD(bLoByte(wHiWord(dAddr))) 			\
		  ...
		  SEND_DFIFO_WRITE_CMD(bLoByte(wLoWord(dAddr))) 			\
		  /* write M7-0 data */										\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTQ, OP_REPEAT_ONE)				\
		  SEND_DFIFO_WRITE_CMD(DUMMY_DATA)							\
		  /* write dummy data */									\
		  SEND_OPFIFO_WRITE_CMD(OP_OQ2IQ, OP_REPEAT_TWO)			\
		  SEND_DFIFO_WRITE_CMD(DUMMY_DATA)							\
		  SEND_DFIFO_WRITE_CMD(DUMMY_DATA)							\
		  /* read data */											\
		  while((len) > 0) {										\
...
// Send quad page program command 
// Parameters:
//		dAddr: 	3bytes or 4bytes length, depend the value of mode
//		data:	type is char*
//		len: 	the value is [1, 256]
//		mode:	1 means 4bytes, 0 means 3bytes
//		program_mode: 0 means page program, 1 means quad page program
#define SEND_PAGE_QUAD_PROGRAM_CMD(dAddr, data, len, mode)			\
		{															\
		  SEND_MANUAL_WREN_BEGIN_CMD								\
		  SEND_CSL_CMD												\
		  /* write op_cmd */										\
		  ...
		  SEND_DFIFO_WRITE_CMD(SF_OP_QUAD_PAGE_PROGRAM) 			\
		  /* write address */										\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTS, ((mode)?0x04:0x03))		\
		  if(mode){													\
			 SEND_DFIFO_WRITE_CMD(bHiByte(wHiWord(dAddr))) 			\
		  ...
		  SEND_DFIFO_WRITE_CMD(bLoByte(wLoWord(dAddr))) 			\
		  /* page program */										\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTQ, (len))						\
		  do														\
		  {															\
			 SEND_DFIFO_WRITE_CMD(*((data)++)) 						\
		  } while ((--(len)) > 0); 									\
...
// Send 4-Bytes fast read quad output command 
// Parameters:
//		dAddr: 	4bytes length
//		data:	type is char*
//		len: 	the value is [1, FlashSize-1]
#define SEND_4BYTES_FASTREAD_QUADOUT_CMD(dAddr, data, len)			\
		{															\
		  SEND_MANUAL_BEGIN_CMD										\
		  SEND_CSL_CMD												\
		  /* write op_cmd 0x6C */									\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTS, OP_REPEAT_ONE) 			\
		  SEND_DFIFO_WRITE_CMD(SF_OP_4BYTES_FAST_RD_QUAD_OUT)		\
		  /* write address */										\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTS, OP_REPEAT_FOUR)			\
		  SEND_DFIFO_WRITE_CMD(bHiByte(wHiWord(dAddr))) 			\
		  SEND_DFIFO_WRITE_CMD(bLoByte(wHiWord(dAddr))) 			\
...
// Send 4-Bytes fast read quad I/O command 
// Parameters:
//		dAddr: 	4bytes length
//		data:	type is char*
//		len: 	the value is [1, FlashSize-1]
#define SEND_4BYTES_FASTREAD_QUADIO_CMD(dAddr, data, len)			\
		{															\
		  SEND_MANUAL_BEGIN_CMD										\
		  SEND_CSL_CMD												\
		  /* write op_cmd 0xEC */									\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTS, OP_REPEAT_ONE) 			\
		  SEND_DFIFO_WRITE_CMD(SF_OP_4BYTES_FAST_RD_QUAD_IO)		\
		  /* write address */										\
		  SEND_OPFIFO_WRITE_CMD(OP_OUTQ, OP_REPEAT_FOUR)			\
		  SEND_DFIFO_WRITE_CMD(bHiByte(wHiWord(dAddr))) 			\
		  SEND_DFIFO_WRITE_CMD(bLoByte(wHiWord(dAddr))) 			\
		  SEND_DFIFO_WRITE_CMD(bHiByte(wLoWord(dAddr))) 			\
		  SEND_DFIFO_WRITE_CMD(bLoByte(wLoWord(dAddr))) 			\
		  /* write M7-0 data */										\
		  SEND_OPFIFO_WRITE_CMD(OP_OD2IQ, OP_REPEAT_ONE)			\
		  SEND_DFIFO_WRITE_CMD(DUMMY_DATA)							\
		  /* write dummy data */									\
		  SEND_OPFIFO_WRITE_CMD(OP_OD2IQ, OP_REPEAT_TWO)			\
		  SEND_DFIFO_WRITE_CMD(DUMMY_DATA)							\
		  SEND_DFIFO_WRITE_CMD(DUMMY_DATA)							\
		  /* read data */											\
		  while((len) > 0) {										\
