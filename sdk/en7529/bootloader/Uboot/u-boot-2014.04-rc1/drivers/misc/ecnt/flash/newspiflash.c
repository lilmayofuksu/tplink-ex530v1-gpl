...
#if defined(SPRAM_IMG)
extern void prom_puts(const char *s);
//#define prom_printf(fmt, args...) prom_puts(fmt)
...
#define SPI_REG_BASE		0x1fbc0000
#define SPI_REG_MASTER		0x1fbc0028
#define SPI_REG_MOREBUF		0x1fbc002c
#define SPI_REG_SPACE_CR	0x1fbc003c
#else
...
static int spiflash_write_internal (struct mtd_info *mtd,unsigned long to,unsigned long len,unsigned long *retlen,const unsigned char *buf, int speed);

/* Mapping of generic opcodes to STM serial flash opcodes */

struct mtd_info mtd;
int program_speed = PAGE_PROGRAM;
...
spiflash_manual_read_internal (struct mtd_info *mtd, unsigned long from,unsigned long len,unsigned long *retlen,unsigned char *buf, unsigned char read_type)
{
	...
	if((Address3B4B_Value == 2) && (mtd->size >= SIZE_32MiB) && (is_MX25L256 == 0) && ((dest_addr>>24) != 0)) {
		...
		case FAST_READ_QUADOUT:
			SEND_4BYTES_FASTREAD_QUADOUT_CMD(from, buf, len);
			break;

		case FAST_READ_QUADIO:
			SEND_4BYTES_FASTREAD_QUADIO_CMD(from, buf, len);
			break;
		...
		case FAST_READ_QUADOUT:
			SEND_FASTREAD_QUADOUT_CMD(from, buf, len, mode);
			break;

		case FAST_READ_QUADIO:
			SEND_FASTREAD_QUADIO_CMD(from, buf, len, mode);
			break;
...
static int
spiflash_manual_fast_read_quadOut(struct mtd_info *mtd, unsigned long from,unsigned long len,unsigned long *retlen,unsigned char *buf)
{
	spiflash_manual_read_internal(mtd, from, len, retlen, buf, FAST_READ_QUADOUT);
   	return (0);
}

static int
spiflash_manual_fast_read_quadIO(struct mtd_info *mtd, unsigned long from,unsigned long len,unsigned long *retlen,unsigned char *buf)
{
	spiflash_manual_read_internal(mtd, from, len, retlen, buf, FAST_READ_QUADIO);
   	return (0);
}
...
static int
spiflash_write_internal (struct mtd_info *mtd,unsigned long to,unsigned long len,unsigned long *retlen,const unsigned char *buf, int program_speed)
{
	u8 cur_mode=0;
	u8 mode=0;
	static int cnt=0;
	...
	while(pp_len > 0) {
		...
		if(program_speed == PAGE_PROGRAM) {
			SEND_PAGE_PROGRAM_CMD(to, buf, pp_cur_len, mode);
		} else {
			SEND_PAGE_QUAD_PROGRAM_CMD(to, buf, pp_cur_len, mode);
		}
...
{
	return spiflash_write_internal(&mtd, to, len, retlen, buf, program_speed);
}
...
unsigned long ReadSPIDWord(unsigned long index)
{
	...
	spiflash_read_internal(&mtd, index, 4, &retlen, &dword);
	
	return dword;
}

/* only use in boot.img */
void Quad_Mode_Enable(int index)
{
#if !defined(BOOTROM_EXT) && !defined(SPRAM_IMG) && !defined(LZMA_IMG)
	u8 val=0;
	u8 check=0;
	...
	if( (flash_tables[index].mfr_id == MANUFACTURER_GIGADEVICE) )
	{
		...
		if( (flash_tables[index].dev_id == GD25Q256C) ) {
			SEND_READ_REGISTER_CMD(SF_OP_RD_STATUS1, val );
			val |= 0x40;
			SEND_WRITE_REGISTER_CMD(SF_OP_WR_STATUS1, val);
...
void spi_nor_set_clock_speed( u32 clk)
{
	...
	if(!isFPGA) {
		...
		printf("Set SPI Clock to %u Mhz\n", (dividend / (clock_factor * 2)));
	} else {
		printf("FPGA can not Set SPI Clock, FPGA SPI Clock is fixed to 25 Mhz\n");
	}
}
#endif
	...
	*retlen = AutoMappingTab[mode];
	...
	switch(AutoMappingTab[mode]) {
		...
		case 3:
			...
			break;
		case 4:
			program_speed = QUAD_PAGE_PROGRAM;
	...
	unsigned long len, unsigned long *retlen, unsigned char *buf, u32 mode)
{
	...
	*retlen = ManualMappingTab[mode];
	...
	case 4:
		program_speed = QUAD_PAGE_PROGRAM;
		return spiflash_manual_fast_read_quadOut(&mtd, from, len, retlen, buf);
	case 5:
		program_speed = QUAD_PAGE_PROGRAM;
		return spiflash_manual_fast_read_quadIO(&mtd, from, len, retlen, buf);
	}
}
