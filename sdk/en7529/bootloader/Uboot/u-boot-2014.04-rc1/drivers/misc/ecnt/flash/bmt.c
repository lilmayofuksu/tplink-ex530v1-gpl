...
{
    unsigned int chip_size = (1 << ra->flash->chip_shift);
    unsigned int block_size = (1 << ra->flash->erase_shift);
    ...
    nand_chip_bmt = ra;
    total_block_count = chip_size / block_size;
...
u32 get_system_block_count(void)
{
	return system_block_count;
}

u32 get_bmt_block_index(void)
{
	return bmt_block_index;
}

u32 get_bbt_block_index(void)
{
	return bbt_block_index;
}
...
int CopyFlashData(void)
{
	...
	ret = nandflash_read(bob_data_read_offset, 1280, &retlen, bob_data, &status);
	if (ret != 0)
		return -1;
	...
	ret = nandflash_read(eeprom_data_read_offset, 1024, &retlen, eeprom_data, &status);
	if (ret != 0)
		return -1;
...
int check_BMT_reservearea_empty_or_not(unsigned long index)
{
	...
	ret = nandflash_read(index, 16 , &retlen, reservearea_data, &status);
