...
int tcPhyInit(macAdapter_t * mac_p){
	...
	switch (tcPhyVer) {
		...
		case tcPhyVer_7522Ge: 		// MT7522 Ge
            mt7512FECfgLoad(0, DO_4_PORT, 8);
...
void SetEphyLED(void)
{
	miiStationWrite(12,0x0d,0x001f);
	miiStationWrite(12,0x0e,0x0021);
	miiStationWrite(12,0x0d,0x401f);
	miiStationWrite(12,0x0e,0x800a);

	miiStationWrite(12,0x0d,0x001f);
	miiStationWrite(12,0x0e,0x0024);
	miiStationWrite(12,0x0d,0x401f);
	miiStationWrite(12,0x0e,0xc007);

	miiStationWrite(12,0x0d,0x001f);
	miiStationWrite(12,0x0e,0x0025);
	miiStationWrite(12,0x0d,0x401f);
	miiStationWrite(12,0x0e,0x003f);

	miiStationWrite(12,0x0d,0x001f);
	miiStationWrite(12,0x0e,0x0026);
	miiStationWrite(12,0x0d,0x401f);
	miiStationWrite(12,0x0e,0x4007);

	miiStationWrite(12,0x0d,0x001f);
	miiStationWrite(12,0x0e,0x0027);
	miiStationWrite(12,0x0d,0x401f);
	miiStationWrite(12,0x0e,0x003f);
}
...
{
/*
	if((((VPint(0xbfb00064)&0xffff0000))==0x00070000) && (PDIDR==3))	// allen_20160831, only for EN751221 E3
	{	
...
void macPhyReset_tc3162l2(void){
...
void macPhyReset_mac(void){
	miiStationWrite(mac_p->enetPhyAddr, PHY_CONTROL_REG, PHY_RESET );
	miiStationWrite(mac_p->enetPhyAddr, PHY_CONTROL_REG, MIIDR_AUTO_NEGOTIATE );
}

/*=====from 7512_eth.c to tcswitch.c*/
...
uint32 tcMiiExtStationRead_CL22(uint32 port_num, uint32 dev_num, uint32 reg_num)
{
    ...
    const uint16 MMD_addr_data_register=0xE;	// 0xe=14	
  	const uint16 page_reg=31;
	uint32 value=0;
	miiStationWrite(port_num, page_reg, 0x00); //switch to main page
	miiStationWrite(port_num, MMD_Control_register, (0<<14)+dev_num);
	miiStationWrite(port_num, MMD_addr_data_register, reg_num);
	miiStationWrite(port_num, MMD_Control_register, (1<<14)+dev_num);
	value = miiStationRead(port_num, MMD_addr_data_register);
...
int eth_phy_init(macAdapter_t *mac_p){
	uint32	efuse_bit31_0_val, efuse_bit63_32_val;
	int i, err = 0;
	if (isFPGA){
		printf("fpga mode, phy init quit\n");
	}
	for ( mac_p->enetPhyAddr = 0; mac_p->enetPhyAddr < 32; mac_p->enetPhyAddr++ ) {
	...
	if(isEN751627)				// allen_20170810, efuse[33:32] 
	{
		efuse_bit31_0_val	= read_reg_word(0xbfbf8214);   	// efuse[31:0]
		efuse_bit63_32_val = read_reg_word(0xbfbf8218);		// efuse[63:32]
		
		if((efuse_bit31_0_val&0x1) == 0x0)		  
		{
			if((efuse_bit63_32_val&0x1) == 0x0)				// efuse[32]
				GECal_Rext(8, 100);							// for 7516, p8
		}
		else   												// efuse[0]=1, remarked
		{
			if(((efuse_bit63_32_val>>1)&0x1) == 0x0)		// efuse[33]
				GECal_Rext(8, 100);							// for 7516, p8
		}

		/* EN7516 phy disable giga capability in bootloader */
		for (i = 8; i <= 12; i++)
		{
			miiStationWrite(i, 9, 0x0000); // disable Giga
			miiStationWrite(i, 13, 0x0007);
			miiStationWrite(i, 14, 0x003c);
			miiStationWrite(i, 13, 0x4007);
			miiStationWrite(i, 13,0x0000); // disable EEE
		}
	}
	...
	if (isFPGA){
		for (i = 1; i <= 4; i++)
		{
			miiStationWrite(i, 29, 5); // setting mii timing
			miiStationWrite(i, 30, 0x2d47);
			write_reg_word(0xbfb5f01c, 0xba050005 + i<<19);
			write_reg_word(0xbfb5f01c, 0xbc052d47 + i<<19);
		}
	}
...
int eth_phy_init_7520(macAdapter_t *mac_p){
...
int eth_phy_init_tc3162l2(macAdapter_t *mac_p){
	int err = 0;
	macPhyReset_tc3162l2();
	pause(100);
...
int eth_phy_init_tc3262g(macAdapter_t *mac_p){
	int err = 0;
	macPhyReset_tc3262g();
	pause(100);
