...
DRAMC_CTX_T DramCtx =
{
  ...
  //BOOL arfgWriteLevelingInitShif;
#if (DUAL_RANK_ENABLE == 1) //YMC add to avoid warning
  {0,0},
#else
  0,
...
static void vCalibration_Flow_DDR(DRAMC_CTX_T *p)
{
    ...
    for(s1RankIdx=RANK_0; s1RankIdx<u1RankMax; s1RankIdx++)
    {
	...
	#endif
        {
            ...
            if ((p->dram_type != TYPE_LPDDR2) && (p->dram_type != TYPE_PCDDR2))
            {
	...
	#if 0//cc move later below RxPerbit. DE suggests K RX first
    ...
    #if 0//cc mark to do it for each rank. DUAL_RANK_ENABLE
    #ifndef DUAL_RANK_RX_K
        if(s1RankIdx==RANK_0 || p->dram_type==TYPE_LPDDR2)
    #endif
    #endif
        {
...
#if GATING_ADJUST_TXDLY_FOR_TRACKING
	mcSHOW_DBG_MSG(("##timeinfo: RxDqsGatingPostProcess begin...\n"));
	timestamp_show();
	//cc add. only PC4 may need this process?? confirm with DE.
	if (p->dram_type == TYPE_PCDDR4)
    	DramcRxdqsGatingPostProcess(p);	
	mcSHOW_DBG_MSG(("##timeinfo: RxDqsGatingPostProcess end\n"));
	timestamp_show();
#endif
	...
	/* cc notes: porting from Boar(MT5887) */
#if RX_EYE_SCAN
	int i;
...
int Init_DRAM(EMI_SETTINGS *emi_set)
{
    ...
    if ((p->pinmux == PIN_MUX_TYPE_LPDDR3_168BALL) || (p->pinmux == PIN_MUX_TYPE_DDR3X8) || (p->pinmux == PIN_MUX_TYPE_LPDDR3_211BALL))
        p->en_4bitMux = ENABLE;
...
DRAMC_CTX_T g_dramc_config;

#if COMPILE_PCDDR3//FOR_DV_SIMULATION		//YMC change for real chip use
void DPI_SW_main_PCDDR3(void)
{
    DRAM_INFO_BY_MRR_T DramInfo;
	DRAMC_CTX_T *p;
	...
	DramConfig = &g_dramc_config;
	
	DramConfig->support_rank_num = RANK_SINGLE; //RANK_DUAL;		//YMC change for EN7580
	// DramRank
	DramConfig->rank = RANK_0;
	// DRAM type
	DramConfig->dram_type = TYPE_PCDDR3;
	// ODT config
	DramConfig->odt_onoff = ODT_ON;	//set ODT on
	// DRAM CBT mode, only for LP4, useless in LP3
	DramConfig->dram_cbt_mode = CBT_NORMAL_MODE;
	// IC and DRAM read DBI
	DramConfig->DBI_R_onoff = DBI_OFF;		 // only for LP4, uesless in LP3
	// IC and DRAM write DBI
	DramConfig->DBI_W_onoff = DBI_OFF;		 // only for LP4, uesless in LP3
	// IC and DRAM package
...
#if COMPILE_PCDDR3_1866
	DramConfig->frequency = DDR_DDR1866;
#else
	DramConfig->frequency = DDR_DDR1333;
#endif
	...
	// Disable SSC, Only use integer mode
	DramConfig->ssc_en = DISABLE;
#if DUAL_RANK_ENABLE
	DramConfig->pinmux = PIN_MUX_TYPE_DDR3X16x2;
#else
#if FOR_DV_SIMULATION //cc notes: For simulation, seems that always two ranks are provided.
	DramConfig->pinmux = PIN_MUX_TYPE_DDR3X16x2;
#else
	//YMC add for EN7523 ASIC.
	if(pkg_type != BGA)
	{
		DramConfig->pinmux = PIN_MUX_TYPE_DDR3KGD;
	}
	else
	{
	...
	if(xtal_sel == PLL_Fin_25M)
	{
		DramConfig->clk_src = PLL_Fin_25M;
	}
	else
	{
		DramConfig->clk_src = PLL_Fin_20M;
	}
#endif
#endif
	...
	for (ii = 0; ii < RANK_MAX; ii++) {
		DramConfig->arfgWriteLevelingInitShif[ii] = FALSE;
		DramConfig->fgTXPerbifInit[ii] = FALSE;
	}
	DramConfig->asymmetric = 0;
	DramConfig->min_winsize = 0;
	DramConfig->sum_winsize = 0;

        /* For lower speed, disable ODT */
	if (DramConfig->frequency < DDR_DDR1600)
		DramConfig->odt_onoff = ODT_OFF;

	p = DramConfig;

	mcSHOW_DBG_MSG(("ctx = %p, dram_type = %d\n", p, p->dram_type));

	#if 1//dbg_print
	prom_puts("dram_type = ");
	prom_print_dec(p->dram_type);
	prom_puts(", speed = ");
	...
	mcSHOW_DBG_MSG(("Start Selftest Now...\n"));
	#if 1//dbg_print
    prom_puts("Start Selftest Now...\n");
	#endif
	timestamp_show();
	DramcSelftestRun(p);
#endif
...
#if RUN_TIME_CONFIG_ENABLE //cc add
    mcSHOW_DBG_MSG(("\nSettings after calibration ...\n"));
    DramcRunTimeConfig(p);
#endif
    ...
    vDramCPUReadWriteTestAfterCalibration(p);
#endif
    //return 0;	//YMC mark to avoid warning
#endif
}
#endif	//YMC add

#if COMPILE_PCDDR2
void DPI_SW_main_PCDDR2(void)
{
	DRAMC_CTX_T *DramConfig;

	DramConfig = &g_dramc_config;
	
	DramConfig->dram_type = TYPE_PCDDR2;
	DramConfig->frequency = DDR_DDR1066;
#if FOR_DV_SIMULATION //cc notes: For simulation, seems that always two ranks are provided.
	DramConfig->pinmux = PIN_MUX_TYPE_DDR3X16x2;
#else
	//YMC add for EN7523 ASIC.
	if(pkg_type != BGA)
	{
		DramConfig->pinmux = PIN_MUX_TYPE_DDR2KGD;
	}
	else
	{
		DramConfig->pinmux = PIN_MUX_TYPE_DDR2DDR3X16;
	...
	DramConfig->package = PACKAGE_SBS;
	DramConfig->data_width = DATA_WIDTH_16BIT;
	DramConfig->test2_1 = DEFAULT_TEST2_1_CAL;
	DramConfig->test2_2 = DEFAULT_TEST2_2_CAL;
	DramConfig->test_pattern = TEST_XTALK_PATTERN;
	...
	DramConfig->ssc_en = DISABLE;
	DramConfig->en_4bitMux = DISABLE;
	DramConfig->asymmetric = DISABLE;
	DramConfig->min_winsize = 0;
	DramConfig->sum_winsize = 0;
	DramConfig->aru4CalResultFlag[0] = 0;
	DramConfig->aru4CalResultFlag[1] = 0;
	DramConfig->aru4CalExecuteFlag[0] = 0;
	DramConfig->aru4CalExecuteFlag[1] = 0;

	//#if WRITE_LEVELING_MOVE_DQS_INSTEAD_OF_CLK
	DramConfig->arfgWriteLevelingInitShif[0] = FALSE;
	DramConfig->arfgWriteLevelingInitShif[1] = FALSE;
	//#endif
	//#if TX_PERBIT_INIT_FLOW_CONTROL
	DramConfig->fgTXPerbifInit[0] = FALSE;
	DramConfig->fgTXPerbifInit[1] = FALSE;
	//#endif

	DramConfig->enable_rx_scan_vref = DISABLE;	//YMC add  for bring up
	DramConfig->enable_tx_scan_vref = DISABLE;	//YMC add  for bring up

	mcSHOW_DBG_MSG(("\n----->Global_Option_Init begin ...\n"));
	timestamp_show();
	Global_Option_Init(DramConfig);
	timestamp_show();
	mcSHOW_DBG_MSG(("\nGlobal_Option_Init end <-----\n"));

	mcSHOW_DBG_MSG(("\n----->DramcSwImpedanceCal begin ...\n"));
	timestamp_show();
	DramcSwImpedanceCal(DramConfig);
	timestamp_show();
	...
	timestamp_show();
	mcSHOW_DBG_MSG(("\DramcInit end <-----\n"));

#if ENABLE_MIOCK_JMETER
	mcSHOW_DBG_MSG(("\n----->DramcMiockJmeter begin ...\n"));
	timestamp_show();
	DramcMiockJmeter_CLK(DramConfig);
	timestamp_show();
	mcSHOW_DBG_MSG(("\DramcMiockJmeter end <-----\n"));
#endif

	mcSHOW_DBG_MSG(("\n----->vApplyConfigBeforeCalibration begin ...\n"));
	timestamp_show();
    vApplyConfigBeforeCalibration_PC2(DramConfig);
	timestamp_show();
	mcSHOW_DBG_MSG(("\vApplyConfigBeforeCalibration end <-----\n"));

	vDramCalibrationAllChannel(DramConfig);

	mcSHOW_DBG_MSG(("\n----->vApplyConfigAfterCalibration begin ...\n"));
	timestamp_show();
	vApplyConfigAfterCalibration(DramConfig);
	timestamp_show();
	mcSHOW_DBG_MSG(("\vApplyConfigAfterCalibration end <-----\n"));

#if 0
	mcSHOW_DBG_MSG(("\n----->DramcRunTimeConfig begin ...\n"));
	timestamp_show();
	DramcRunTimeConfig(DramConfig);
	timestamp_show();
	mcSHOW_DBG_MSG(("\DramcRunTimeConfig end <-----\n"));
#endif
}
