...
#elif (fcFOR_CHIP_ID == EN7523)
const U32 arRegBaseAddrList[4] =
{
   DRAMC_NAO_BASE_ADDRESS,
   DRAMC_AO_BASE_ADDRESS,
   DDRPHY_B01_BASE_ADDRESS, 
   DDRPHY_CMD_BASE_ADDRESS
};
#endif
...
U32 u4RegBaseAddrTraslate(U32 u4reg_addr)
{
	...
	U32 u4RegType = ((u4reg_addr-DRAMC_NAO_BASE_VIRTUAL) >> POS_BANK_NUM) & 0xf;

    if (u4reg_addr < DRAMC_NAO_BASE_VIRTUAL || u4reg_addr >= MAX_BASE_VIRTUAL) return u4reg_addr;

#if DUAL_RANK_ENABLE //cc add. DDR3 may support DUAL rank.
    if (u4RegType == 0)
    {
        // NAO Register
        if (u4Offset >= (DRAMC_REG_RK0_DQSOSC_STATUS-DRAMC_NAO_BASE) && u4Offset < (DRAMC_REG_RK1_DQSOSC_STATUS-DRAMC_NAO_BASE))
        {
    ...
    {
        // AO Register
        if (u4Offset >= (DRAMC_REG_RK0_DQSOSC-DRAMC_AO_BASE) && u4Offset < (DRAMC_REG_RK1_DQSOSC-DRAMC_AO_BASE))
        {
            u4Offset += pgCurrDramCtx->rank * 0x100;
        }
        else if (u4Offset >= (DRAMC_REG_SHURK0_DQSCTL-DRAMC_AO_BASE) && u4Offset < (DRAMC_REG_SHURK1_DQSCTL-DRAMC_AO_BASE))
        {
            u4Offset += pgCurrDramCtx->rank * 0x100;
        }
    }
    ...
    {
        if (u4Offset >= (DDRPHY_SHU1_R0_B0_DQ0-DDRPHY_B01_BASE) && u4Offset < (DDRPHY_SHU1_R1_B0_DQ0-DDRPHY_B01_BASE))
        {
            u4Offset += pgCurrDramCtx->rank * 0x100;
        }      
        else if (u4Offset >= (DDRPHY_MISC_STBERR_RK0_R-DDRPHY_B01_BASE) && u4Offset < (DDRPHY_MISC_STBERR_RK1_R-DDRPHY_B01_BASE))
        {
            u4Offset += pgCurrDramCtx->rank * 0x8; //cc changed from *8 -> *4
        }
...
U8 ucDram_Register_Write(U32 u4reg_addr, U32 u4reg_value)
{
    ...
    writel(u4reg_value,u4reg_addr);
    //dsb();		//YMC mark for no define
#endif
...
void vIO32Write4B_All(U32 reg32, U32 val32)
{
    ...
    if(u4RegType >=DDRPHY_B01_BASE_VIRTUAL)//PHY 
    {
        reg32 &= 0xffff;
        u1AllCount =2;
        reg32 += DDRPHY_B01_BASE_VIRTUAL;
    }
