...
#elif (defined(CONFIG_MTK_ATF) && defined(TCSUPPORT_CPU_ARMV8_64))
#if defined(CONFIG_ECNT)
	extern void jumparch64_smc(ulong addr, ulong arg1, ulong arg2, ulong arg3);
#else
...
#if defined(CONFIG_ECNT)
	jumparch64_smc(images->ep, (unsigned long)images->ft_addr, 0, KERNEL_64BITS);
#else
    jumparch64_smc(images->ep, (unsigned long)images->ft_addr, 0);
#endif
#else /* should be here for ARM32 kernel */
