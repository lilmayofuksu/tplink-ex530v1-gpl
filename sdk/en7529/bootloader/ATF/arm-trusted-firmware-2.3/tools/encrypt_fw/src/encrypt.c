		       ...
		       const char *op_name)
{
	...
	int bytes, enc_len = 0, i, j, ret = 0, key_size = KEY_SIZE, buffer_size = BUFFER_SIZE;
	struct fw_enc_hdr header;

	memset(&header, 0, sizeof(struct fw_enc_hdr));
	
	bytes = strlen(key_string);
	if (bytes != KEY_STRING_SIZE)
	{
		if (bytes != KEY_STRING_SIZE_128)
		{		
			ERROR("Unsupported key size: %lu\n", strlen(key_string));
			return -1;
		}
		key_size = KEY_SIZE_128;
		buffer_size = BUFFER_SIZE_128;
