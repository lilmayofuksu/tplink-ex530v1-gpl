...
BOOL SendKeepAlive(PHGlobal *phglobal, int opCode)
{
	...
	ret = phSend(phglobal->m_udpsocket, p2, KEEPALIVE_PACKET_LEN,0);
...
int ExecuteUpdate(PHGlobal *phglobal)
{
	...
	ret = phSend(phglobal->m_tcpsocket, (char*)COMMAND_AUTH,sizeof(COMMAND_AUTH),0);
    LOG(1) (("OK.\n"));
	...
	ret = phSend(phglobal->m_tcpsocket, sendbuffer,strlen(sendbuffer),0);
    LOG(1) (("OK\n"));
	...
	if (strcmp(buffer,"536") == 0) //find redirected server address, and let us connect again to new server
	{
		...
		if (pos0)
		{
			...
			{
				*pos1 = '\0';
				strncpy(phglobal->szHost, pos0 + 1,sizeof(phglobal->szHost)-1);
	...
	for (i=0;;i++)
    {
        ...
        ret = phSend(phglobal->m_tcpsocket,regicommand,strlen(regicommand),0);
    }

	//////////////////////////////////////////////////////////////////////////
	//send confirm
	LOG(1) (("SEND CNFM DATA..."));
    ret = phSend(phglobal->m_tcpsocket,(char*)COMMAND_CNFM,strlen(COMMAND_CNFM),0);
    LOG(1) (("OK\n"));
	...
	ret = phSend(phglobal->m_tcpsocket,(void *)COMMAND_STAT_USER,sizeof(COMMAND_STAT_USER),0);
	...
	ret = phSend(phglobal->m_tcpsocket,(void *)COMMAND_STAT_DOM,sizeof(COMMAND_STAT_DOM),0);
	memset(buffer, 0, 1024);
	len = phReadOneLine(phglobal->m_tcpsocket, buffer,sizeof(buffer));
	buffer[3] = 0;
	if (len <= 0 || strcmp(buffer,"250")!=0)
	...
	ret = phSend(phglobal->m_tcpsocket,(char*)COMMAND_QUIT,sizeof(COMMAND_QUIT),0);
    LOG(1) (("OK.\n"));
