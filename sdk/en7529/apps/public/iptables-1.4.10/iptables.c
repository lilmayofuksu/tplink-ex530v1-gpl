	       ...
	       struct iptc_handle *const handle)
{
	...
	if (fw->ip.smsk.s_addr == 0L && !(format & FMT_NUMERIC))
		...
		if (format & FMT_NUMERIC)
			strncpy(buf, xtables_ipaddr_to_numeric(&fw->ip.src), sizeof(buf) - 1);
		else
			strncpy(buf, xtables_ipaddr_to_anyname(&fw->ip.src), sizeof(buf) - 1);
		strncat(buf, xtables_ipmask_to_numeric(&fw->ip.smsk),sizeof(buf) -strlen(buf)- 1);
	...
	if (fw->ip.dmsk.s_addr == 0L && !(format & FMT_NUMERIC))
		...
		if (format & FMT_NUMERIC)
			strncpy(buf, xtables_ipaddr_to_numeric(&fw->ip.dst), sizeof(buf) - 1);
		else
			strncpy(buf, xtables_ipaddr_to_anyname(&fw->ip.dst), sizeof(buf) - 1);
		strncat(buf, xtables_ipmask_to_numeric(&fw->ip.dmsk),sizeof(buf) -strlen(buf)- 1);
		printf(FMT("%-19s ","-> %s"), buf);
	}
