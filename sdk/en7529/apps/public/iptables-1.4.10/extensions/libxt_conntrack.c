	...
	case '4': /* --ctorigdst */
		...
		if (naddrs > 1)
			xtables_error(PARAMETER_PROBLEM,
			           "multiple IP addresses not allowed");
		if (naddrs == 1)
			memcpy(&info->origdst_addr.in6, addr, sizeof(*addr));
		info->match_flags |= XT_CONNTRACK_ORIGDST;
		if (invert)
			info->invert_flags |= XT_CONNTRACK_ORIGDST;
		if(addr != NULL){
	...
	case '5': /* --ctreplsrc */
		...
		if (naddrs > 1)
			xtables_error(PARAMETER_PROBLEM,
			           "multiple IP addresses not allowed");
		if (naddrs == 1)
			memcpy(&info->replsrc_addr.in6, addr, sizeof(*addr));
		info->match_flags |= XT_CONNTRACK_REPLSRC;
		if (invert)
			info->invert_flags |= XT_CONNTRACK_REPLSRC;
		if(addr != NULL){
	...
	case '6': /* --ctrepldst */
		...
		if (naddrs > 1)
			xtables_error(PARAMETER_PROBLEM,
			           "multiple IP addresses not allowed");
		if (naddrs == 1)
			memcpy(&info->repldst_addr.in6, addr, sizeof(*addr));
		info->match_flags |= XT_CONNTRACK_REPLDST;
		if (invert)
			info->invert_flags |= XT_CONNTRACK_REPLDST;
		if(addr != NULL){
           ...
           int inv, int numeric)
{
	...
	if (mask->s_addr == 0L && !numeric)
		...
		if (numeric)
			strncpy(buf, xtables_ipaddr_to_numeric(addr), sizeof(buf) - 1);
		else
			strncpy(buf, xtables_ipaddr_to_anyname(addr), sizeof(buf) - 1);
		strncat(buf, xtables_ipmask_to_numeric(mask),sizeof(buf)-strlen(buf) - 1);
		printf("%s ", buf);
	}
}
