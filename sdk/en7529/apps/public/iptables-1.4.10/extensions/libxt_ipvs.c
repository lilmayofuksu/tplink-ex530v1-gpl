			      ...
			      unsigned int family, bool numeric)
{
	...
	if (family == NFPROTO_IPV4) {
		...
		if (numeric)
			...
			strncpy(buf, xtables_ipaddr_to_anyname(&addr->in), sizeof(buf) - 1);
		...
		if (numeric)
			strncpy(buf, xtables_ip6addr_to_numeric(&addr->in6), sizeof(buf) - 1);
		else
			strncpy(buf, xtables_ip6addr_to_anyname(&addr->in6), sizeof(buf) - 1);
