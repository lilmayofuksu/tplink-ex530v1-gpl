...
#define PROCFILE_BUFSIZ	1024
	...
	if (ret) {
		...
		res = read(procfile, ret, PROCFILE_BUFSIZ - 1);
...
static int compatible_revision(const char *name, u_int8_t revision, int opt)
{
	...
	strncpy(rev.name, name, sizeof(rev.name) - 1);
...
const char *xtables_ip6mask_to_numeric(const struct in6_addr *addrp)
{
	...
	if (l == -1) {
		strcpy(buf, "/");
		strncat(buf, xtables_ip6addr_to_numeric(addrp), sizeof(buf) - strlen(buf) - 1);
		return buf;
	}
	sprintf(buf, "/%d", l);
	return buf;
		      ...
		      struct in6_addr **maskpp, unsigned int *naddrs)
{
	...
	unsigned int len = 0, i = 0, j = 0, n = 0, count = 1;
	const char *loop = name;
