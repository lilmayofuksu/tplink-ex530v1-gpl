	       ...
	       struct ip6tc_handle *const handle)
{
	...
	if (!memcmp(&fw->ipv6.smsk, &in6addr_any, sizeof in6addr_any)
		...
		if (format & FMT_NUMERIC)
			strncpy(buf, xtables_ip6addr_to_numeric(&fw->ipv6.src), sizeof(buf) - 1);
		else
			strncpy(buf, xtables_ip6addr_to_anyname(&fw->ipv6.src), sizeof(buf) - 1);
		strncat(buf, xtables_ip6mask_to_numeric(&fw->ipv6.smsk), sizeof(buf) - strlen(buf)- 1);
	...
	if (!memcmp(&fw->ipv6.dmsk, &in6addr_any, sizeof in6addr_any)
		...
		if (format & FMT_NUMERIC)
			strncpy(buf, xtables_ip6addr_to_numeric(&fw->ipv6.dst), sizeof(buf) - 1);
		else
			strncpy(buf, xtables_ip6addr_to_anyname(&fw->ipv6.dst), sizeof(buf) - 1);
		strncat(buf, xtables_ip6mask_to_numeric(&fw->ipv6.dmsk),sizeof(buf) - strlen(buf)- 1);
		printf(FMT("%-19s ","-> %s"), buf);
	}
...
int do_command6(int argc, char *argv[], char **table, struct ip6tc_handle **handle)
{
	...
	while ((c = getopt_long(argc, argv,
	   "-A:D:R:I:L::S::M:F::Z::N:X::E:P:Vh::o:p:s:d:j:i:bvnt:m:xc:g:",
		...
		case 'N':
			if (optarg && (*optarg == '-' || *optarg == '!'))
				xtables_error(PARAMETER_PROBLEM,
					   ...
					   "with `%c'\n", *optarg);
			if (optarg && xtables_find_target(optarg, XTF_TRY_LOAD))
				xtables_error(PARAMETER_PROBLEM,
		...
		case 'p':
			...
			for (protocol = optarg; protocol&&*protocol; protocol++)
		...
		case 'c':
			...
			if (pcnt&&sscanf(pcnt, "%llu", &cnt) != 1)
				xtables_error(PARAMETER_PROBLEM,
					"-%c packet counter not numeric",
					opt2char(OPT_COUNTERS));
			fw.counters.pcnt = cnt;

			if (bcnt&&sscanf(bcnt, "%llu", &cnt) != 1)
				xtables_error(PARAMETER_PROBLEM,
	...
	if (command == CMD_APPEND
	    ...
	    || command == CMD_INSERT
	    || command == CMD_REPLACE) {
		if (chain&&(strcmp(chain, "PREROUTING") == 0
		...
		if (chain&&(strcmp(chain, "POSTROUTING") == 0
