...
int set_authfile (char *word, char *value, int context, void *item)
{
    ...
    case CONTEXT_GLOBAL:
        ...
        strncpy (((struct global *) item)->authfile, value,
                 sizeof (((struct global *)item)->authfile)-1);
        break;
    default:
        snprintf (filerr, sizeof (filerr), "'%s' not valid in this context\n",
                  word);
...
struct iprange *set_range (char *word, char *value, struct iprange *in)
{
    ...
    if (c)
    {
		...
		if (count < 3) {
			strcpy(ip_hi, value);
			for (e = ip_hi + strlen(ip_hi) -1; e >= ip_hi; e--) {
				if (*e == '.') count--;
				if (count < 0) {
					e++;
					break;
...
int set_lns (char *word, char *value, int context, void *item)
{
    ...
    case CONTEXT_LAC:
        ...
        strncpy (ipr->hostname, value, sizeof (ipr->hostname)-1);
        if (d)
            ipr->port = atoi (d);
        else
            ...
            else if (!strcasecmp (s, "lns"))
            {
                ...
                if (d)
                    strncpy (((struct lns *) data)->entname,
                             d, sizeof (((struct lns *) data)->entname)-1);
#ifdef DEBUG_FILE
