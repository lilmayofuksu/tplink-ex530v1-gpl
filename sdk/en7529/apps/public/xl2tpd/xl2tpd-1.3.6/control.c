 ...
 * don't ask for sequencing */
        ...
        x_add_opt (po, "nodetach");
        if (c->lac)
        {
            if (c->lac->defaultroute)
                x_add_opt (po, "defaultroute");
            strncpy (ip1, IPADDY (c->lac->localaddr), sizeof (ip1) - 1);
            strncpy (ip2, IPADDY (c->lac->remoteaddr), sizeof (ip2) - 1);
#ifdef IP_ALLOCATION
			if ( po )
	            add_opt (po, "%s:%s", c->lac->localaddr ? ip1 : "",
	                          c->lac->remoteaddr ? ip2 : "");
#endif
            if (c->lac->authself)
            {
                if (c->lac->pap_refuse)
                    x_add_opt (po, "refuse-pap");
                if (c->lac->chap_refuse)
                    x_add_opt (po, "refuse-chap");
            }
            else
            {
                x_add_opt (po, "refuse-pap");
            ...
            if (c->lac->authpeer)
            {
                ...
                if (c->lac->pap_require)
                    x_add_opt (po, "require-pap");
                if (c->lac->chap_require)
                    x_add_opt (po, "require-chap");
            }
            if (c->lac->authname[0])
            {
                x_add_opt (po, "name");
                x_add_opt (po, c->lac->authname);
            }
            if (c->lac->debug)
                x_add_opt (po, "debug");
            if (c->lac->password[0])
            {                    
                ...
                x_add_opt (po, "plugin");
                x_add_opt (po, "passwordfd.so");
                x_add_opt (po, "passwordfd");
                snprintf (passwdfd_buf, 32, "%d", pppd_passwdfd[0]);
                x_add_opt (po, passwdfd_buf);
            }
            if (c->lac->pppoptfile[0])
            {
                x_add_opt (po, "file");
                x_add_opt (po, c->lac->pppoptfile);
            }
        };
        if ( c->lac && c->lac->pass_peer)
        {
            x_add_opt (po, "ipparam");
            x_add_opt (po, IPADDY (t->peer.sin_addr));
        }
        start_pppd (c, po);
        opt_destroy (po);
        if (c->lac)
            c->lac->rtries = 0;
	if (c->lac && c->lac->password[0])
    ...
    case ICCN:
	    ...
	    */
        ...
        c->state = ICCN;
        strncpy (ip1, IPADDY (c->lns->localaddr), sizeof (ip1) - 1);
        strncpy (ip2, IPADDY (c->addr), sizeof (ip2) - 1);
        po = NULL;
        po = add_opt (po, "passive");
        x_add_opt (po, "nodetach");
		if ( po )
        	add_opt (po, "%s:%s", c->lns->localaddr ? ip1 : "", ip2);
        if (c->lns->authself)
        {
            if (c->lns->pap_refuse)
                x_add_opt (po, "refuse-pap");
