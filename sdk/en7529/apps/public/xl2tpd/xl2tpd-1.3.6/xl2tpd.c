...
int start_pppd (struct call *c, struct ppp_opts *opts)
{
    ...
    if (kernel_support)
    {
       ...
       stropt[pos++] = strdup ("/lib/pppol2tp.so");
...
#endif
    {
        if ((c->fd = getPtyMaster (tty, sizeof(tty) - 1)) < 0)
        {
...
void destroy_tunnel (struct tunnel *t)
{
    ...
    struct call *c = NULL, *me = NULL, *tmp = NULL;
    struct tunnel *p = NULL;
    struct timeval tv;
    if (!t)
        return;
...
struct call *lac_call (int tid, struct lac *lac, struct lns *lns)
{
    ...
    while (t)
    {
        if (t->ourtid == tid)
        {
            ...
            strncpy (tmp->dial_no, dial_no_tmp, sizeof(tmp->dial_no) - 1); /*  jz: copy dialnumber to tmp->dial_no  */
    ...
    while (!done)
    {
        ...
        case 'c':
            ...
            if(lac) {
                ...
                lac->rtries = 0;
                if (authname != NULL)
                    strncpy (lac->authname, authname, STRLEN - 1);
                if (password != NULL)
                    strncpy (lac->password, password, STRLEN - 1);
                if (!lac->c)
                {
                    magic_lac_dial (lac);
                    write_res (resf, "%02i OK\n", 0);
       ...
       case 'o':          /* jz: option 'o' for doing a outgoing call */
        ...
        case 'a':
            ...
            {               
                ...
                strncpy (lac->entname, tunstr, sizeof (lac->entname) - 1);
...
void init (int argc,char *argv[])
{
    ...
    unlink(gconfig.controlfile);
    ret = mkfifo (gconfig.controlfile, 0600);
