...
int main (int argc, char *argv[])
{
    ...
    ret = mkfifo (result_filename, 0600);
	...
	fseek(mesf,0L,SEEK_END);
	file_len = ftell (mesf);
    if ( file_len > 0 && write (control_fd, buf, file_len) < 0)
    {
      int errorno = errno;
      print_error (ERROR_LEVEL,
    ...
    int command_result_code = read_result (
        result_fd, rbuf, CONTROL_PIPE_MESSAGE_SIZE - 1
    );
    printf ("%s", rbuf);
