...
#if defined(TCSUPPORT_CMCCV2)
#define FILE_DSTLIST_PATH "/tmp/tmp_vpndstip_list"
struct dest_addr_group {
	unsigned int ipv4_addr;
	struct dest_addr_group *next;
};
/* vpn dest address  group header */
struct dest_addr_group *gDstGrp = NULL;

int add_destip_to_list(unsigned int ipaddr)
{
	struct dest_addr_group *cur_dstaddr_group = NULL;
	struct dest_addr_group **new_dstaddr_group = NULL;

	/* check IP exist. */
	for ( cur_dstaddr_group = gDstGrp, new_dstaddr_group = &gDstGrp;
			cur_dstaddr_group;
			  new_dstaddr_group = &cur_dstaddr_group->next
			, cur_dstaddr_group = cur_dstaddr_group->next )
	{
		if ( cur_dstaddr_group->ipv4_addr == ipaddr )
			return 0;
	}

	/* add new entry */
	if ( !(*new_dstaddr_group) )
		*new_dstaddr_group =
		malloc(sizeof(struct dest_addr_group));

	/* malloc group failed. */
	if ( !(*new_dstaddr_group) )
		return -1;
	(*new_dstaddr_group)->ipv4_addr = ipaddr;
	(*new_dstaddr_group)->next = NULL;

	return 0;
}

extern int dump_destip_list(void);
int dump_destip_list(void)
{
	struct dest_addr_group *cur_dstaddr_group = NULL;
	FILE *fp_dst = NULL;
	int is_first = 1;

	fp_dst = fopen(FILE_DSTLIST_PATH, "a+");
	if ( NULL == fp_dst )
		return -1;

	for ( cur_dstaddr_group = gDstGrp;
			cur_dstaddr_group;
			cur_dstaddr_group = cur_dstaddr_group->next )
	{
		if ( is_first )
		{
			fprintf(fp_dst, "\"%s\""
				, inet_ntoa(*((struct in_addr *)&cur_dstaddr_group->ipv4_addr)));
			is_first = 0;
		}
		else
			fprintf(fp_dst, ", \"%s\""
...
int handle_forward_pkt(unsigned char *buf)
{
	struct vpn_ppp_hdr *vpn_hdr = NULL;
	struct iphdr *iph = NULL;

	if ( NULL == buf )
		return -1;

	vpn_hdr = (struct vpn_ppp_hdr *)buf;
	if ( vpn_hdr->protocol != 0x0021 ) /* IPv4 */
		return -2;

	iph = (struct iphdr *)(vpn_hdr + 1);

	/*printf("\n source ip=[%s] \n", inet_ntoa(*((struct in_addr *)&iph->saddr)));*/

	add_destip_to_list(iph->saddr);
...
int init_network (void)
{
    ...
    flags = 1;
    ret = setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, &flags, sizeof(flags));
#ifdef SO_NO_CHECK
    ret = setsockopt(server_socket, SOL_SOCKET, SO_NO_CHECK, &flags, sizeof(flags));
#endif
    ...
    arg |= O_NONBLOCK;
    ret = fcntl (server_socket, F_SETFL, arg);
...
void network_thread ()
{
    ...
    for (;;)
    {
		...
		if ( !CTBIT (*((_u16 *) buf->start)))
		{
			...
			{
				handle_forward_pkt((unsigned char*)buf->start);
			}
		}
#endif
...
int connect_pppol2tp(struct tunnel *t) {
        if (kernel_support) {
            ...
            flags=1;
            ret = setsockopt(ufd, SOL_SOCKET, SO_REUSEADDR, &flags, sizeof(flags));
#ifdef SO_NO_CHECK
            ret = setsockopt(ufd, SOL_SOCKET, SO_NO_CHECK, &flags, sizeof(flags));
#endif
