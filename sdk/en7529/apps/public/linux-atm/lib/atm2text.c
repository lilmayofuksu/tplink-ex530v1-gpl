  ...
  int flags)
{
    ...
    int *fmt, fmt_len, fmt_ofs; //COV, CID 69239
    ...
    if (*addr->sas_addr.prv) {
	fmt = pure;
	fmt_len = sizeof(pure)/sizeof(pure[0]); //COV, CID 69239
	i = 0;
	if (flags & A2T_PRETTY)
		...
		case ATM_AFI_ICD_GROUP:
		    fmt = bin;
		    fmt_len = sizeof(bin)/sizeof(bin[0]); //COV, CID 69239
		    break;
		case ATM_AFI_LOCAL:
		case ATM_AFI_LOCAL_GROUP:
		    fmt = local;
		    fmt_len = sizeof(local)/sizeof(local[0]); //COV, CID 69239
		    break;
		case ATM_AFI_E164:
		case ATM_AFI_E164_GROUP:
		    ...
		    *buffer++ = ':';
		    i = 9;
		    fmt = e164;
		    fmt_len = sizeof(e164)/sizeof(e164[0]); //COV, CID 69239
		    break;
		default:
		    break;
	    }
	fmt_ofs = 0; //COV, CID 69239
	for (left = *fmt++; i < ATM_ESA_LEN; i++) {
	    if (!left--) {
		if (!length--) return FATAL;
		*buffer++ = '.';
		fmt_ofs++; //COV, CID 69239
		if (fmt_ofs >= fmt_len)
			return FATAL; 
		left = *fmt++-1;
	    }
