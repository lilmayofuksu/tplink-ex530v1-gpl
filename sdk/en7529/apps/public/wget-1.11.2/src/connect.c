...
/* Fill SA as per the data in IP and PORT.  SA shoult point to struct
   ...
   otherwise.  */
extern char speed_test_cmd_array[3][128];
  ...
  if (print)
    {
	  ...
	  if((access("/proc/tc3162/speed_test",F_OK))!=-1)
	  {
		  memset(speed_test_cmd_array[0], 0, sizeof(speed_test_cmd_array[0]) );
		  sprintf(speed_test_cmd_array[0], "echo wgetdestport=%d > /proc/tc3162/speed_test", port);
		  
		  memset(speed_test_cmd_array[1], 0, sizeof(speed_test_cmd_array[1]) );
		  sprintf(speed_test_cmd_array[1], "echo wgetdestip=%s > /proc/tc3162/speed_test", txt_addr);

		  memset(speed_test_cmd_array[2], 0, sizeof(speed_test_cmd_array[2]) );
		  if (print && 0 != strcmp (print, txt_addr))
		  	sprintf(speed_test_cmd_array[2], "echo wgethost=%s > /proc/tc3162/speed_test", escnonprint (print));
		  else
		  	sprintf(speed_test_cmd_array[2], "echo wgethost=%s:%d > /proc/tc3162/speed_test", txt_addr, port);
	  }
...
#ifdef SO_REUSEADDR
  res = setsockopt (sock, SOL_SOCKET, SO_REUSEADDR, setopt_ptr, setopt_size);
#endif
