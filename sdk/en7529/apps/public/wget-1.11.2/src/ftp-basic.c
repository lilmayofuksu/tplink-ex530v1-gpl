...
ftp_port (int csock, int *local_sock)
{
  ...
  *local_sock = bind_local (&addr, &port);
  ...
  if((access("/proc/tc3162/speed_test",F_OK))!=-1)
  {
	...
	sprintf(speed_test_cmd, "echo localport=%d > /proc/tc3162/speed_test", port);
	printf("%s\n", speed_test_cmd);
	system(speed_test_cmd);

	memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
	sprintf(speed_test_cmd, "echo localip=%d.%d.%d.%d > /proc/tc3162/speed_test", 
	  ptr[0], ptr[1], ptr[2], ptr[3]);
	printf("%s\n", speed_test_cmd);
	system(speed_test_cmd);

	//Ssaptr = (void*)&(addr,data);
	memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
	sprintf(speed_test_cmd, "echo action=FGET_ACTV > /proc/tc3162/speed_test");
	printf("%s\n", speed_test_cmd);
	system(speed_test_cmd);
  }
...
ftp_pasv (int csock, ip_address *addr, int *port)
{
  ...
  *port = ((tmp[4] << 8) & 0xff00) + tmp[5];
	...
	{
		memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
		sprintf(speed_test_cmd, "echo destport=%d > /proc/tc3162/speed_test", *port);
		printf("%s\n", speed_test_cmd);
		system(speed_test_cmd);
		
		memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
		sprintf(speed_test_cmd, "echo destip=%d.%d.%d.%d > /proc/tc3162/speed_test", 
		...
		//Ssaptr = (void*)&(addr,data);
		memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
		sprintf(speed_test_cmd, "echo action=FGET_PASV %d.%d.%d.%d %d > /proc/tc3162/speed_test",
