...
char speed_test_cmd_array[3][128];
...
int speedtest_using_wget(void){
  FILE *fp;
  char buf[512] = {0};
  int count = 0;
  fp = fopen("/proc/tc3162/speedtest_wget_switch", "r");
  if(!fp){
	  return 0;
  }
...
File `%s' already there; not retrieving.\n\n"), hs->local_file);
  ...
  if(((access("/proc/tc3162/speed_test",F_OK))!=-1)&&speedtest_using_wget())
  {
	  if(statcode == 200){
		  CLOSE_INVALIDATE(sock);
		  memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
		  sprintf(speed_test_cmd, "echo wgetaction=GET /%s 0 0 > /proc/tc3162/speed_test", conn->path);
		  system(speed_test_cmd_array[0]);
		  system(speed_test_cmd_array[1]);
		  system(speed_test_cmd_array[2]);
          ...
          if (*dt & RETROKF)
            {
			  ...
			  download_end(hstat.local_file, hstat.len);
...
digest_authentication_encode (const char *au, const char *user,
                              ...
                              const char *path)
{
  static char *realm = NULL, *opaque = NULL, *nonce = NULL;
  static struct {
    const char *name;
    char **variable;
  } options[] = {
