				  ...
				  struct current_user *user, WERROR *perr)
{
	...
	*perr = WERR_OK;
	...
	if (driver->driverpath[0] != '\0' && strlen(driver->driverpath)) {
	...
	if (driver->datafile[0] != '\0' && strlen(driver->datafile)) {
	...
	if (driver->configfile[0] != '\0' && strlen(driver->configfile)) {
	...
	if (driver->helpfile[0] != '\0' && strlen(driver->helpfile)) {
...
static int add_new_printer_key( NT_PRINTER_DATA *data, const char *name )
{
	...
	if ( (data->num_keys+1) < 0 )
		return -1;
	/* allocate another slot in the NT_PRINTER_KEY array */
	
	d = Realloc2( data->keys, sizeof(NT_PRINTER_KEY)*(data->num_keys+1) );
	if ( d )
		data->keys = d;
