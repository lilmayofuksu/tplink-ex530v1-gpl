...
*/
	...
	q_u->user_ctr.ptr=1;
	q_u->user_ctr.user1.size=((clientname!=NULL)?strlen(clientname):0)+((user_name!=NULL)?strlen(user_name):0)+10;
				...
				PRINTER_INFO_2 *info)
{
	...
	memset(inf, 0, sizeof(SPOOL_PRINTER_INFO_LEVEL_2));
	if(info == NULL){
		*spool_info2 = inf;
		return True;
	}
