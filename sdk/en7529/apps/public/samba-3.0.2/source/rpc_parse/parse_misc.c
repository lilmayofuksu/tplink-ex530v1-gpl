...
void init_buffer3_hex(BUFFER3 *str, const char *buf)
{
	...
	str->buf_max_len = str->buf_len = strhex_to_str((char *)str->buffer, 
		(strlen(buf) < MAX_BUFFERLEN) ? MAX_BUFFERLEN : strlen(buf), buf);
}
...
BOOL smb_io_bufhdr2(const char *desc, BUFHDR2 *hdr, prs_struct *ps, int depth)
{
	...
	res = prs_align(ps);
	res = prs_uint32("info_level", ps, depth, &(hdr->info_level));
	res = prs_uint32("length    ", ps, depth, &(hdr->length    ));
	res = prs_uint32("buffer    ", ps, depth, &(hdr->buffer    ));
...
BOOL smb_io_buffer4(const char *desc, BUFFER4 *buf4, uint32 buffer, prs_struct *ps, int depth)
{
	...
	res = prs_align(ps);
	res = prs_uint32("buf_len", ps, depth, &(buf4->buf_len));
	...
	res = prs_uint8s(True, "buffer", ps, depth, buf4->buffer, buf4->buf_len);
