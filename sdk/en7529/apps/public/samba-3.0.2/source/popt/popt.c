...
poptContext poptGetContext(const char * name, int argc, const char ** argv,
    ...
    if (name)
	con->appName = malloc(strlen(name) + 1);
	if(con->appName)
		strncpy(con->appName, name, strlen(name));
		...
		/*@unused@*/ int flags)
{
	...
	if(newAlias.longName){
		alias->longName = malloc(strlen(newAlias.longName) + 1);
		if(alias->longName)
				strncpy(alias->longName, newAlias.longName, strlen(newAlias.longName));
	}else{
		alias->longName = NULL;
	}
