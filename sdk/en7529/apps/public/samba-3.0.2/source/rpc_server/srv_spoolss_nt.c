...
static uint32 size_of_notify_info_data(uint16 type, uint16 field)
{
	...
	for (i = 0; i < sizeof(notify_info_data_table)/sizeof(notify_info_data_table[0]); i++) 
	{
				  ...
				  TALLOC_CTX *mem_ctx)
{
	...
	res = get_printer_snum(p, hnd, &snum);
	...
	switch (version) {
		...
		*/
		case 2:
		{
			...
			memset(&driver1, 0, sizeof(driver1));
