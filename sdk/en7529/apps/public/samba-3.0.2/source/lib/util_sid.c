...
void generate_wellknown_sids(void)
{
	...
	res = string_to_sid(&global_sid_NULL, "S-1-0-0");

	/* SECURITY_WORLD_SID_AUTHORITY */
	res = string_to_sid(&global_sid_World_Domain, "S-1-1");
	res = string_to_sid(&global_sid_World, "S-1-1-0");

	/* SECURITY_CREATOR_SID_AUTHORITY */
	res = string_to_sid(&global_sid_Creator_Owner_Domain, "S-1-3");
	res = string_to_sid(&global_sid_Creator_Owner, "S-1-3-0");
	res = string_to_sid(&global_sid_Creator_Group, "S-1-3-1");

	/* SECURITY_NT_AUTHORITY */
	res = string_to_sid(&global_sid_NT_Authority, "S-1-5");
	res = string_to_sid(&global_sid_Network, "S-1-5-2");
	res = string_to_sid(&global_sid_Anonymous, "S-1-5-7");
	res = string_to_sid(&global_sid_Authenticated_Users, "S-1-5-11");
	res = string_to_sid(&global_sid_System, "S-1-5-18");

	/* SECURITY_BUILTIN_DOMAIN_RID */
	res = string_to_sid(&global_sid_Builtin, "S-1-5-32");
	res = string_to_sid(&global_sid_Builtin_Administrators, "S-1-5-32-544");
	res = string_to_sid(&global_sid_Builtin_Users, "S-1-5-32-545");
	res = string_to_sid(&global_sid_Builtin_Guests, "S-1-5-32-546");
	res = string_to_sid(&global_sid_Builtin_Power_Users, "S-1-5-32-547");
	res = string_to_sid(&global_sid_Builtin_Account_Operators, "S-1-5-32-548");
	res = string_to_sid(&global_sid_Builtin_Server_Operators, "S-1-5-32-549");
	res = string_to_sid(&global_sid_Builtin_Print_Operators, "S-1-5-32-550");
	res = string_to_sid(&global_sid_Builtin_Backup_Operators, "S-1-5-32-551");
	res = string_to_sid(&global_sid_Builtin_Replicator, "S-1-5-32-552");
