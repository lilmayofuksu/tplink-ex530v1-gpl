					  ...
					  const char *pass, const char *domain)
{
	...
	memset(&blob, 0, sizeof(DATA_BLOB));
			      ...
			      const char *pass, const char *domain)
{
	...
	/* the server sent us the first part of the SPNEGO exchange in the negprot 
	   reply */
	if (!spnego_parse_negTokenInit(blob, OIDs, &principal) || principal == NULL) {
		data_blob_free(&blob);
		if(principal != NULL){
			free(principal);
		}
		    ...
		    const char *share, const char *dev, const char *pass, int passlen)
{
	...
	p = smb_buf(cli->outbuf);
	memcpy(p, pword, passlen > sizeof(pword) ? sizeof(pword) : passlen);
	p += passlen;
			     ...
			     BOOL *retry) 
{
	...
	if (cli && !cli_session_setup(cli, user, password, strlen(password)+1, 
	...
	if (service) {
		if (cli && !cli_send_tconX(cli, service, service_type,
				    password, strlen(password)+1)) {
			nt_status = cli_nt_error(cli);
