...
#endif
{
  ...
  extern int errno;
  char filename[128]={0}, mode[12], fullpath[196], *bufindex, ackbuf[512], string[64];
  #if defined(TCSUPPORT_IPV6_TFTP)
				...
				if(FD_ISSET(sock, &readfd)) {
					...
					(tid == ntohs (client.sin_port))) {
						...
						if(packetbuf[0] == NULLSTART && opcode == DATA && rcount == count) {
							...
							rt = blapi_system_get_max_receive_file_size(&size);
							if (file_size > size) {
...
#endif
{
  ...
  unsigned char filebuf[MAXDATASIZE + 1];
  static unsigned char packetbuf[MAXACKFREQ][MAXDATASIZE + 12],
    recvbuf[MAXDATASIZE + 12];
  char filename[128]={0}, mode[12], fullpath[196], *bufindex;
  #if defined(TCSUPPORT_IPV6_TFTP)
