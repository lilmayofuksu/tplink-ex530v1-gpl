...
/*
return active session num which mac match.
*/
static int checkLastSession(unsigned char *mac)
{
	PPPoESession *cur = NULL;
	int sess_num = 0;

	cur = ActiveSessions;
	while ( cur )
	{
		if ( !memcmp(mac, cur->acHash->peerMac, ETH_ALEN) )
			sess_num ++;
...
/*
add/delete a mac address to/from bridge table.
*/
static int update_br_addr(int action, unsigned char *mac)
{
	FILE *fp = NULL;
	int i_sess_num = 0;
	...
	/* need check last session when delete it. */
	if ( 0 == action )
	{
		i_sess_num = checkLastSession(mac);
		if ( i_sess_num > 1 )
			return 0;
	}

	fp = fopen("/proc/tc3162/br_static_mac", "w");
	if ( fp )
	{
...
usage(char const *argv0)
{
    ...
    fprintf(stderr, "   -m mode        -- PPPoE relay mode\n");
...
main(int argc, char *argv[])
{
    ...
    while((opt = getopt(argc, argv, "hC:S:B:n:i:Fm:")) != -1) {
	switch(opt) {
	case 'h':
	    usage(argv[0]);
	    break;
	...
	case 'S':
		printf("\n****filename is 111 %s\n", optarg);
		strncpy(nasName, optarg,sizeof(nasName)-1);
    ...
    case 'm':
		...
		fprintf(stderr, "Illegal argument to -i: should be -m mode\n");
		exit(EXIT_FAILURE);
	    }
        printf("\nMain mode  %d\n", mode);
	    break;
...
relayLoop()
{
	...
	if ((setsockopt(sock, SOL_SOCKET, SO_TYPE_COPY_SKB_MARK, (const void*)&sock_opt, sizeof(sock_opt))) < 0) 
	{
...
relayGotDiscoveryPacket(PPPoEInterface const *iface)
{
	...
	size = ntohs(packet.length) + HDR_SIZE;
	 ...
	 switch ( packet.code )
	 {
		case CODE_PADI:
		case CODE_PADR:
			if ( 0 != check_ppp_packets(iface->discoverySock, ntohs(packet.ethHdr.h_proto)) )
			{
...
relayGotSessionPacket(PPPoEInterface const *iface)
{
    ...
    blapi_traffic_get_foe_info_from_sock(iface->sessionSock, &sockinfo);
		...
		int size)
{
	...
	if ( BRIDGE_MODE == mode )
	{
		/* remove pppoe server mac from bridge table. (PADT from bras) */
		update_br_addr(0, packet->ethHdr.h_source);
	}
#endif
    /* Destroy the session */
		...
		int size)
{
    ...
    /* Broadcast the PADI on all AC-capable interfaces except the interface
       on which it came */
    for (i=0; i < NumInterfaces; i++) {
        if (iface == &Interfaces[i]) 
            continue;
    	if (!Interfaces[i].acOK) 
            continue;
        if(mode == ROUTE_MODE)
		...
		int size)
{
    ...
    if((mode == ROUTE_MODE) || (mode == RELAY_MODE))
        memcpy(packet->ethHdr.h_source, Interfaces[ifIndex].mac, ETH_ALEN);
#if defined(TCSUPPORT_CT)
	else
	{
		/* set pppoe server mac to bridge table. */
		update_br_addr(1, packet->ethHdr.h_source);
	}
#endif
		...
		int size)
{
    ...
    if (ifIndex < 0 || ifIndex >= NumInterfaces ||
	!Interfaces[ifIndex].acOK || iface == &Interfaces[ifIndex]) {
	syslog(LOG_ERR,
		...
		int size)
{
	...
	if (!ses) {
        ...
        //tcdbg_printf("PADS packet session change from %d to %d \n", packet->session, ses->sesNum);

    	/* Replace session number in route mode only */
        if(mode == ROUTE_MODE)
        	packet->session = ses->sesNum;
    }
