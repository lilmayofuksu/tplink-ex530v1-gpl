...
extern const size_t CMD_IGMP_LEN;
...
#if defined(TCSUPPORT_CT_JOYME4)
	ssize_t max_buf_used_joyme4 = 10485760;
#endif
    size_t buflen = 0;
...
#if defined(TCSUPPORT_CT_JOYME4)
	buflen = (env_snd_buflen > 0) ? (size_t)env_snd_buflen : (size_t)max_buf_used_joyme4;
#else
...
udp_relay( int sockfd, struct server_ctx* ctx )
{
    ...
    size_t      rcvbuf_len = 0;
#if defined(TCSUPPORT_CT_JOYME4)
	char ppe_localin_cmd[128];
	char cpu_bind_cmd[128];
#endif
...
#if defined(TCSUPPORT_CT_JOYME4)
		if(CPU_BIND_CNT == 1){
			memset(cpu_bind_cmd, 0, sizeof(cpu_bind_cmd) );
			sprintf(cpu_bind_cmd, "taskset -p 0x8 %d", getpid());
			system(cpu_bind_cmd);
...
#if defined(TCSUPPORT_CT_JOYME4)
			memset(ppe_localin_cmd, 0, sizeof(ppe_localin_cmd) );
			sprintf(ppe_localin_cmd, "/userfs/bin/ppe localin add dport %d", (int)port);
			system(ppe_localin_cmd);
#endif
...
accept_requests (int sockfd, tmfd_t* asock, size_t* alen)
{
    ...
    while (naccepted < nmax) {
        ...
        */
        ...
        set_sendbuf( new_sockfd, 2097152 );
