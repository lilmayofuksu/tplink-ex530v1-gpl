...
unsigned char shortoldmac[ETH_ALEN]={0};
int shortoldsid;
int sendPADTflag = 0;
int shortflag = 0;
#endif
...
PPPOEInitDevice(void)
{
    ...
    sprintf(nodeName, "SysInfo_Entry");	
    memset(tmp, 0 , sizeof(tmp));
    tcapi_get(nodeName, "ShortFlag", tmp);
    if(strlen(tmp) != 0)
        shortflag = atoi(tmp);	
	
    if(shortflag){	
		memset(nodeName, 0, sizeof(nodeName));
		sprintf(nodeName, "Wan_PVC0"); 
        memset(tmp, 0 , sizeof(tmp));
        tcapi_get(nodeName, "PPPSessionId", tmp);
        if(strlen(tmp) == 0){
            tcdbg_printf("\r\n PPPOEInitDevice not get PPPSessionId node");
            sprintf(tmp, "00:21:05:6b:fc:1b:1");
        }		
        ...
        memset(tmp, 0 , sizeof(tmp));
        tcapi_get(nodeName, "PADITimeout", tmp);
        if(strlen(tmp) == 0){
            timeout = 2;
            tcdbg_printf("\r\n PPPOEInitDevice not get PADITimeout node");
        }
        else
            timeout = atoi(tmp);

        memset(tmp, 0 , sizeof(tmp));
        tcapi_get(nodeName, "PADTFlag", tmp);
        if(strlen(tmp) != 0)
            sendPADTflag = atoi(tmp);
    }
#endif
...
PPPOEConnectDevice(void)
{
...
#if defined(TCSUPPORT_PADT_BEFORE_PADI) || defined(TCSUPPORT_STARTUP_OPTIMIZATION) 
	FILE *fp=NULL;
	char tmpPath[32];
	char tmp[256];
	char nodeName[64];		
	...
	if(shortflag){	
		memset(nodeName, 0, sizeof(nodeName));
		sprintf(nodeName, "Wan_PVC0");
		tcapi_set(nodeName, "PPPSessionId", tmp);
		tcapi_save();
	}
#endif
#endif	
