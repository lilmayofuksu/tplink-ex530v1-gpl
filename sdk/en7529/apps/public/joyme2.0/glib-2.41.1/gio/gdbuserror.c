...
static const GDBusErrorEntry g_dbus_error_entries[] =
{
  ...
  {G_DBUS_ERROR_SETMULTI_FAILED,                  "org.freedesktop.DBus.Error.SetMultiError"},
  {G_DBUS_ERROR_SETMULTI_ROLLBACK_FAILED,         "org.freedesktop.DBus.Error.SetMultiRollbackError"},
  {G_DBUS_ERROR_SET_FAILED,                       "org.freedesktop.DBus.Error.SetError"}
};

GQuark
g_dbus_error_quark (void)
{
  G_STATIC_ASSERT (G_N_ELEMENTS (g_dbus_error_entries) - 1 == G_DBUS_ERROR_PROPERTY_READ_ONLY + 3);
  static volatile gsize quark_volatile = 0;
