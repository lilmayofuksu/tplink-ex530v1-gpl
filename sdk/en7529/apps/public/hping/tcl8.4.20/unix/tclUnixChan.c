...
#   define GETCONTROL(fd, intPtr)	int io_get_ret = ioctl((fd), TIOCMGET, (intPtr))
#   define SETCONTROL(fd, intPtr)	int io_set_ret = ioctl((fd), TIOCMSET, (intPtr))
    /*
...
#   ifdef FIONREAD
#	define GETREADQUEUE(fd, int)	int = ioctl((fd), FIONREAD, &(int))
#   elif defined(FIORDCHK)
#	define GETREADQUEUE(fd, int)	int = ioctl((fd), FIORDCHK, NULL)
#   endif /* FIONREAD */
#   ifdef TIOCOUTQ
#	define GETWRITEQUEUE(fd, int)	int = ioctl((fd), TIOCOUTQ, &(int))
#   endif /* TIOCOUTQ */
...
#	define SETBREAK(fd, flag) \
		if (flag) {				\
		    int io_i6 = ioctl((fd), TIOCSBRK, NULL);	\
		} else {				\
		    int io_i7 = ioctl((fd), TIOCCBRK, NULL);	\
		}
    ...
    int *errorCodePtr;		/* To store error code. */
{
	...
	*errorCodePtr = EOVERFLOW;
	ret = TclOSseek(fsPtr->fd, (Tcl_SeekOffset) oldLoc, SEEK_SET);
	return -1;
    } else {
...
{
    int status, sock, asyncConnect, curState, origState, ret = 0;
    ...
    ret = fcntl(sock, F_SETFD, FD_CLOEXEC);
				 ...
				 * wait forever. */
{
	...
	if (timeout > 0) {
	    ...
	    if (/*blockTime.tv_usec*/(abortTime.usec - now.usec) < 0) {
		blockTime.tv_sec -= 1;
		blockTime.tv_usec = (abortTime.usec - now.usec)+1000000;
	    }
		else
			blockTime.tv_usec = abortTime.usec - now.usec;
