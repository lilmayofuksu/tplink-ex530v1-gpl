				  ...
				  * subexpression is stored here. */
{
    ...
    ret = TclFixupForwardJump(envPtr, &shortCircuitFixup, dist, 127);
    *endPtrPtr = tokenPtr;
				 ...
				 * subexpression is stored here. */
{
    ...
    Tcl_Token *tokenPtr;
    int elseCodeOffset, dist, code, ret = 0;
    int savedStackDepth = envPtr->currStackDepth;
    ...
    ret = TclFixupForwardJump(envPtr, &jumpAroundThenFixup, dist, 127);
    *endPtrPtr = tokenPtr;
