...
#endif
{
	...
	int is_enlager_dt = 0, is_enlager_ev = 0;
		...
		if ( ++TclDate_ps >= &TclDates[ TclDatemaxdepth ] )	/* room on stack? */
		{
			...
			*/
			...
			if (TclDatemaxdepth == YYMAXDEPTH)	/* first time growth */
			{
				...
				{
					int *TclDates_p0 = YYCOPY(newTclDates, TclDates, int);
					if ( TclDates_p0 )
					{
						if ( is_enlager_dt)
							free(TclDates);
						TclDates = TclDates_p0;
					}
					YYSTYPE *TclDatev_p0 = YYCOPY(newTclDatev, TclDatev, YYSTYPE);
					if ( TclDatev_p0 )
					{
						if ( is_enlager_ev )
							free(TclDatev);
						TclDatev = TclDatev_p0;
					}
				}
				else {
			...
			{
				int *TclDates_p1 = YYENLARGE(TclDates, int);
				if ( TclDates_p1 != TclDates )
					is_enlager_dt = 1;
				else
					is_enlager_dt = 0;
				TclDates = TclDates_p1;
				YYSTYPE *TclDatev_p1 = YYENLARGE(TclDatev, YYSTYPE);
				if ( TclDatev_p1 != TclDatev )
					is_enlager_ev = 1;
				else
					is_enlager_ev = 0;
				TclDatev = TclDatev_p1;
