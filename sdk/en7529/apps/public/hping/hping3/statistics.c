...
void	print_statistics(int signal_id)
{
	...
	fprintf(stdout, "\n--- %s hping statistic ---\n", targetname);
	fprintf(stdout, "%d packets tramitted, %d packets received, "
			"%d%% packet loss\n", sent_pkt, recv_pkt, lossrate);
	if (out_of_sequence_pkt)
		fprintf(stdout, "%d out of sequence packets received\n",
			out_of_sequence_pkt);
	fprintf(stdout, "round-trip min/avg/max = %.1f/%.1f/%.1f ms\n",
