...
int main(int argc, char **argv)
{
    ...
    */
...
#endif
	i = 1;
	ret = setsockopt(fileno(stdin), SOL_SOCKET, SO_OOBINLINE, (void *) &i,
			   sizeof(i));
	ret = setsockopt(fileno(stdin), SOL_SOCKET, SO_KEEPALIVE, (void *) &i,
			   sizeof(i));
#if defined(TCSUPPORT_ACCOUNT_ACL) || defined(TCSUPPORT_FW_UPGRADE_16M)
	...
	ret = getsockname(fileno(stdin), (struct sockaddr *) &name, &my_length);
#ifdef TCSUPPORT_IPV6_FTP	
	fourinsix(&name);
	...
	/* Parse hello message */
	strncpy(str, (char *) config_getoption("HELLO_STRING"), sizeof(str) - 1);
