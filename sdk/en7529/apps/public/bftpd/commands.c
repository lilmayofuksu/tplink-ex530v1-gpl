...
void localin_add_port()
{
    char cmd_str[128];
    int res = -1;
    struct sockaddr_in localsock;
	socklen_t namelen = sizeof(localsock);
    
    memset(&localsock, 0, sizeof(localsock));
    namelen = sizeof(localsock);
    res = getsockname(sock, (struct sockaddr *) &localsock, (socklen_t *) &namelen);

    if(res == 0)
    {
        memset(cmd_str, 0, sizeof(cmd_str));
        sprintf(cmd_str, "/userfs/bin/ppe localin add dport %d", ntohs(localsock.sin_port));
        //snprintf(cmd_str, sizeof(cmd_str), "ppe localin add dport %d", ntohs(local.sin_port));
        system(cmd_str);
    }
}

void localin_del_port()
{
    char cmd_str[128];
    int res = -1;
    struct sockaddr_in localsock;    
    ...
    {
        memset(cmd_str, 0, sizeof(cmd_str));
        sprintf(cmd_str, "/userfs/bin/ppe localin del dport %d", ntohs(localsock.sin_port));
        //snprintf(cmd_str, sizeof(cmd_str), "ppe localin add dport %d", ntohs(local.sin_port));
        system(cmd_str);
    }
}   
...
#endif
#ifdef SO_REUSEADDR
	res = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void *) &on, sizeof(on));
#endif
#ifdef SO_REUSEPORT
...
#ifdef SO_SNDBUF
	on = 65536;
	res = setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (void *) &on, sizeof(on));
#endif
...
void command_pasv(char *foo)
{
	...
	if(pasvsock < 0) return;
	
#ifdef TCSUPPORT_IPV6_FTP
	STORAGE_SIN_ADDR(sa) = INADDR_ANY;
	STORAGE_FAMILY(sa) = AF_INET;
#else
	...
	namelen = sizeof(localsock);
	res = getsockname(pasvsock, (struct sockaddr *) &localsock, (socklen_t *) &namelen);
        ...
        {
#ifdef TCSUPPORT_IPV6_FTP
  	  	 sscanf((char *) inet_ntoa(*storage_sin_addr(&(name))), "%i.%i.%i.%i",&a1, &a2, &a3, &a4);
#else
  	    sscanf((char *) inet_ntoa(name.sin_addr), "%i.%i.%i.%i",&a1, &a2, &a3, &a4);
#endif
        }
...
#ifdef TCSUPPORT_IPV6_FTP
	family_mode = STORAGE_FAMILY(remotename);
	pasvsock = socket(family_mode, SOCK_STREAM, 0);
#else
    pasvsock = socket(AF_INET, SOCK_STREAM, 0);
#endif;
	if(pasvsock < 0) return;
...
void do_stor(char *filename, int flags)
{
    ...
    #endif
	int res = 0;
	unsigned long max_receive_file_size = 0;
    ...
    */
	...
	if (!((i = recv(sock, buffer, my_buffer_size - 1, 0))))
		...
		blapi_system_get_max_receive_file_size(&max_receive_file_size);
		if (bytes_recvd > max_receive_file_size) {
			tcdbg_printf("Upload file size is out of range\n");
			control_printf(SL_FAILURE, "452 Error: Upload file size is out of range.");
			if (fd >= 0)
	          		close(fd);
			if (buffer)
				free(buffer);	
...
void command_retr(char *filename)
{
	...
	res = stat(mapped, (struct stat *) &statbuf);
	...
	if ((((double)(statbuf.st_size - offset) * ratio_send) / ratio_recv > bytes_recvd
	...
	switch (whattodo) {
		...
		case DO_NORMAL:
            ...
            localin_add_port();
                        alarm(0);
			res = lseek(phile, offset, SEEK_SET);
			offset = 0;
