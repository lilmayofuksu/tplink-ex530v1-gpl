...
axel_t *axel_new( conf_t *conf, int count, void *url )
{
	...
	*axel->conf = *conf;
	...
	if( *axel->filename == 0 )	/* Index page == no fn		*/
		strncpy( axel->filename, axel->conf->default_filename, MAX_STRING - 1 );
...
int axel_open( axel_t *axel )
{
	...
	{
		count = read( fd, &axel->conf->num_connections, sizeof( axel->conf->num_connections ) );
		...
		count = read( fd, &axel->bytes_done, sizeof( axel->bytes_done ) );
		for( i = 0; i < axel->conf->num_connections; i ++ )
			count = read( fd, &axel->conn[i].currentbyte, sizeof( axel->conn[i].currentbyte ) );
