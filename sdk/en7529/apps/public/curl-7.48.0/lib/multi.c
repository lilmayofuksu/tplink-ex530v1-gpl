...
extern struct timeval g_sysTimeROM;
extern struct timeval g_sysTimeBOM;
extern struct timeval g_sysTimeEOM;
extern unsigned int tr143_uldrdsize;
extern int tr143_ulddiag_flg;
extern unsigned int g_diagstate;
extern const unsigned char State_Complete;
extern const unsigned char State_Error_NoResponse;
extern const unsigned char State_Error_TransferFailed;
#endif
  ...
  if(curlfds) {
    ...
    while(data) {
      ...
      for(i=0; i< MAX_SOCKSPEREASYHANDLE; i++) {
        ...
        if((bitmap & GETSOCK_READSOCK(i)) && ufds) {
          ufds[nfds].fd = sockbunch[i];
          ufds[nfds].events = POLLIN;
          ++nfds;
          s = sockbunch[i];
        }
        if((bitmap & GETSOCK_WRITESOCK(i)) && ufds) {
          ufds[nfds].fd = sockbunch[i];
  ...
  do {
    ...
    case CURLM_STATE_DO:
      if(data->set.connect_only) {
		...
		{
			gettimeofday(&g_sysTimeROM, NULL);
			g_diagstate = State_Error_NoResponse;
		}
#endif
    ...
    case CURLM_STATE_PERFORM:
    {
...
#if defined(TCSUPPORT_TR143_CURL_UPLOAD)
	  unsigned int rdsize = tr143_uldrdsize;
#endif
      result = Curl_readwrite(data->easy_conn, data, &done);
#if defined(TCSUPPORT_TR143_CURL_UPLOAD)
	  if(tr143_ulddiag_flg)
	  {
		  if(0 == rdsize && tr143_uldrdsize > rdsize)
		  {
			  gettimeofday(&g_sysTimeBOM, NULL);
			  g_diagstate = State_Error_TransferFailed;
		  }
	  }
#endif
      ...
      if(result) {
		...
		if(tr143_ulddiag_flg)
		{
			gettimeofday(&g_sysTimeEOM, NULL);
			g_diagstate = State_Complete;
		}
#endif
          rc = CURLM_CALL_MULTI_PERFORM;
        }
