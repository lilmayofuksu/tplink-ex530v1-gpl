...
static int write_percen_to_file(char *path, curl_off_t parameter)
{
	FILE *fp = NULL;
...
int Curl_pgrsUpdate(struct connectdata *conn)
{
  ...
  if(!(data->progress.flags & PGRS_HIDE)) {
    ...
    fflush(data->set.err);
    if(data->set.transfer_percent == 1)
    	write_percen_to_file("/tmp/transfer_percent",total_percen);
