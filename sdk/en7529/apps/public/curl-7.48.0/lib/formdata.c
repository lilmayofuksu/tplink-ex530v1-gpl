                           ...
                           size_t size)
{
  ...
  if(callback) {
	...
	if(tr143_ulddiag_flg)
	{
		if(tr143_uldrdsize + size <= tr143_uldwantsize)
		{
			memset(buffer, 0x01, size);
			tr143_uldrdsize += size;
			nread = size;
		}
		else if(tr143_uldrdsize < tr143_uldwantsize)
		{
			int left = tr143_uldwantsize - tr143_uldrdsize;
			memset(buffer, 0x01, left);
			tr143_uldrdsize += left;
			nread = left;
		}
		else
		{
