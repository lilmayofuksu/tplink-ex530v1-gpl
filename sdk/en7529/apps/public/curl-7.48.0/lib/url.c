...
extern struct timeval g_sysTimeTCPOpenRequest;
                     ...
                     va_list param)
{
  ...
  case CURLOPT_SOCKCONNECT_PROGRESS:
	/*
	* socket callback function: called after socket connect()
	*/
	data->set.sock_conn_progress = va_arg(param, curl_sockconn_callback);
    break;

  case CURLOPT_SOCKCONNECT_MODE:
  	data->set.connect_mode = va_arg(param, long);
  	break;

  case CURLOPT_SPEEDTEST_MODE:
	  data->set.speedtest_mode = va_arg(param, long);
	  break;

  case CURLOPT_FILE_TRANSFER_PERCENT:
	  data->set.transfer_percent = va_arg(param, long);
	  break;
                         ...
                         bool *protocol_done)
{
  ...
  *protocol_done = FALSE; /* default to not done */
  ...
  if(CURL_SOCKET_BAD == conn->sock[FIRSTSOCKET]) {
	...
	{
		gettimeofday(&g_sysTimeTCPOpenRequest, NULL);
