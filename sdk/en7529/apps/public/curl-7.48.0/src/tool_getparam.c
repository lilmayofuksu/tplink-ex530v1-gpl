...
extern int tr143_ulddiag_flg;
extern unsigned int tr143_uldwantsize;
extern unsigned int tr143_uldrdsize;
extern char tr143_uldoutdoc[128];
extern char tr143_uldpidfile[128];
extern char tr143_uldlogfile[128];
extern char tr143_uldinterface[64];
extern int tr143_uldethpri;
extern unsigned int g_BOMOutOctet;
extern struct timeval g_sysTimeBOM;
#endif

extern char tr069_uploadpidfile[128];
extern char tr069_uploadlogfile[128];
extern int tr069_upload_flg;
extern unsigned int g_tr069uldStatus;
extern struct timeval g_tr069uldStartTime;
extern struct timeval g_tr069uldCompleteTime;
                          ...
                          argv_item_t argv[])
{
  ...
  char speedtest_mode[16] = {'0'};
  char transfer_percent[16] = {'0'};
  ...
  for(i = 1, stillflags = TRUE; i < argc && !result; i++) {
...
#if defined(TCSUPPORT_TR143_CURL_UPLOAD)
	if(curl_strequal("-tr143-ulddiag", orig_opt))
	{
		if(i + 1 < argc)
		{
	  		tr143_ulddiag_flg = (atoi(argv[i + 1]) != 0);
			if(tr143_ulddiag_flg)
			{
				g_BOMOutOctet = 0;
			}
			tr143_uldrdsize = 0;
	...
	else if(curl_strequal("-tr143-uldfilelen", orig_opt))
	{
		if(i + 1 < argc)
		{
			tr143_uldwantsize = atoi(argv[i + 1]);
			i++;
		}
		continue;
	}
	else if(curl_strequal("-tr143-uldoutdoc", orig_opt))
	{
		if(i + 1 < argc)
		{
			if(strlen(argv[i + 1]) < sizeof(tr143_uldoutdoc))
			{
				strcpy(tr143_uldoutdoc, argv[i + 1]);
			}			
			i++;
		}
		continue;
	}
	else if(curl_strequal("-tr143-uldpidfile", orig_opt))
	{
		if(i + 1 < argc)
		{
			if(strlen(argv[i + 1]) < sizeof(tr143_uldpidfile))
			{
				strcpy(tr143_uldpidfile, argv[i + 1]);
				if(tr143_uldpidfile[0] && tr143_ulddiag_flg)
				{
					/* write statistics to temp file. */
					FILE* fp = fopen(tr143_uldpidfile, "w");
					if(fp)
					{
					  fprintf(fp, "%d\n", (int)getpid());
					  fclose(fp);
	...
	else if(curl_strequal("-tr143-uldethpri", orig_opt))
	{
		if(i + 1 < argc)
		{
	  		tr143_uldethpri = atoi(argv[i + 1]);
	  		i++;
		}	
	  	continue;	
	}
	else if(curl_strequal("-tr143-uldlogfile", orig_opt))
	{
		if(i + 1 < argc)
		{
			if(strlen(argv[i + 1]) < sizeof(tr143_uldlogfile))
			{
				strcpy(tr143_uldlogfile, argv[i + 1]);
			}			
			i++;
		}
		continue;
	}
	else if(curl_strequal("-tr143-uldinterface", orig_opt))
	{
		if(i + 1 < argc)
		{
			if(strlen(argv[i + 1]) < sizeof(tr143_uldinterface))
			{
				strcpy(tr143_uldinterface, argv[i + 1]);
			}			
			i++;
		}
		continue;
...
#endif
	if(curl_strequal("-tr069-upload", orig_opt))
	{
		if(i + 1 < argc)
		{
			tr069_upload_flg = (atoi(argv[i + 1]) != 0);
			i++;
		}	
		continue;
	}

	else if(curl_strequal("-tr069-uldpidfile", orig_opt))
	{
		if(i + 1 < argc)
		{
			if(strlen(argv[i + 1]) < sizeof(tr069_uploadpidfile))
			{
				strncpy(tr069_uploadpidfile, argv[i + 1], sizeof(tr069_uploadpidfile) - 1);
				if(tr069_uploadpidfile[0] && tr069_upload_flg)
				{
					/* write statistics to temp file. */
					FILE* tr069fp = fopen(tr069_uploadpidfile, "w");
					if(tr069fp)
					{
						fprintf(tr069fp, "%d\n", (int)getpid());
						fclose(tr069fp);
	...
	else if(curl_strequal("-tr069-uldlogfile", orig_opt))
	{
		if(i + 1 < argc)
		{
			if(strlen(argv[i + 1]) < sizeof(tr069_uploadlogfile))
			{
				strncpy(tr069_uploadlogfile, argv[i + 1], sizeof(tr069_uploadlogfile) - 1);
			}			
			i++;
		}
		continue;
	}
	else if(curl_strequal("-speed-test", orig_opt))
	{
		if(i + 1 < argc)
		{
			if(strlen(argv[i + 1]) < sizeof(speedtest_mode))
			{
				strncpy(speedtest_mode, argv[i + 1], sizeof(speedtest_mode) - 1);
				if(!strncmp(speedtest_mode, "enable", 6)){
					operation->speedtest_mode = 1;
				}else{
					operation->speedtest_mode = 0;
				}
			}			
			i++;
		}
        ...
        else if(curl_strequal("-transfer-percent", orig_opt))
	{
		if(i + 1 < argc)
		{
			if(strlen(argv[i + 1]) < sizeof(transfer_percent))
			{
				strncpy(transfer_percent, argv[i + 1], sizeof(transfer_percent) - 1);
				if(!strncmp(transfer_percent, "enable", 6)){
					operation->transfer_percent = 1;
				}else{
					operation->transfer_percent = 0;
				}
			}			
			i++;
		}
