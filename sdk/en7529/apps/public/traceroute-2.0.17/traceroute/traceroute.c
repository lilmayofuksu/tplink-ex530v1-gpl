...
void tune_socket (int sk) {
	...
	res = fcntl (sk, F_SETFL, O_NONBLOCK);
...
void use_timestamp (int sk) {
	...
	res = setsockopt (sk, SOL_SOCKET, SO_TIMESTAMP, &n, sizeof (n));
	/*  foo on errors...  */
}
...
void use_recv_ttl (int sk) {
	...
	if (af == AF_INET)
		res = setsockopt (sk, SOL_IP, IP_RECVTTL, &n, sizeof (n));
	else if (af == AF_INET6)
		res = setsockopt (sk, SOL_IPV6, IPV6_RECVHOPLIMIT, &n, sizeof (n));
	/*  foo on errors   */
}
