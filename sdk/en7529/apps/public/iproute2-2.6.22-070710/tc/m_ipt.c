...
static void set_revision(char *name, u_int8_t revision)
{
	...
	*  but we stole a byte from name. */
	name[IPT_FUNCTION_MAXNAMELEN - 3] = '\0';
	name[IPT_FUNCTION_MAXNAMELEN - 2] = revision;
}
...
build_st(struct iptables_target *target, struct ipt_entry_target *t)
{
	...
	if (target) {
		...
		strncpy(target->t->u.user.name, target->name, sizeof(target->t->u.user.name) - 1);
		return 0;
	}
