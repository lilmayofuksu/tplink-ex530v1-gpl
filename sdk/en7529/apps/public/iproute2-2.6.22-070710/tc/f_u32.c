...
static int u32_parse_opt(struct filter_util *qu, char *handle, int argc, char **argv, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		...
		if (matches(*argv, "match") == 0) {
			...
			unsigned handle1;
			NEXT_ARG();
			if (get_u32_handle(&handle1, *argv)) {
				fprintf(stderr, "Illegal \"link\"\n");
				return -1;
			}
			if (handle1 && TC_U32_NODE(handle1)) {
				fprintf(stderr, "\"link\" must be a hash table.\n");
				return -1;
			}
			addattr_l(n, MAX_MSG, TCA_U32_LINK, &handle1, 4);
		} else if (strcmp(*argv, "ht") == 0) {
			unsigned handle2;
			NEXT_ARG();
			if (get_u32_handle(&handle2, *argv)) {
				fprintf(stderr, "Illegal \"ht\"\n");
				return -1;
			}
			if (handle2 && TC_U32_NODE(handle2)) {
				fprintf(stderr, "\"ht\" must be a hash table.\n");
				return -1;
			}
			if (sample_ok)
				htid = (htid&0xFF000)|(handle2&0xFFF00000);
			else
				htid = (handle2&0xFFFFF000);
