...
struct msdos_dir_entry
  {
    char	name[11], ext[3];	/* name and extension */
...
setup_tables (void)
{
  ...
  if (!atari_format) {
	...
	((long long) bs.cluster_size * sector_size + nr_fats*2);
	...
	( (long long)bs.cluster_size * sector_size + nr_fats*4);
...
write_tables (void)
{
  ...
  if (size_fat == 32)
    {
      seekto( (long long)(CF_LE_W(bs.fat32.info_sector)*sector_size), "info sector" );
