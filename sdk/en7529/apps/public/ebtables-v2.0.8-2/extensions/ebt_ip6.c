...
#endif
#if defined(TCSUPPORT_CT_JOYME4)
	{ "ip6-flabel"		 , required_argument, 0, IP_FLABEL	 },
#endif
	{ 0 }
};
...
#if defined(TCSUPPORT_CT_JOYME4)
	case IP_FLABEL:
		ebt_check_option2(flags, OPT_FLABEL);
		ipinfo->bitmask |= EBT_IP6_FLABEL;
		if (ebt_check_inverse2(optarg))
			ipinfo->invflags |= EBT_IP6_FLABEL;
		ipinfo->flowlabel = strtol(optarg, &end, 16);
		break;
#endif
   ...
   const struct ebt_entry_match *match)
{
	...
	if (ipinfo->bitmask & EBT_IP6_SOURCE) {
		...
		if (ipinfo->invflags & EBT_IP6_SOURCE)
			printf("! ");
		printf("%s", (char *)ebt_ip6_to_numeric(&ipinfo->saddr));
		printf("/%s ", (char *)ebt_ip6_to_numeric(&ipinfo->smsk));
	}
	if (ipinfo->bitmask & EBT_IP6_DEST) {
		printf("--ip6-dst ");
		if (ipinfo->invflags & EBT_IP6_DEST)
			printf("! ");
		printf("%s", (char *)ebt_ip6_to_numeric(&ipinfo->daddr));
		printf("/%s ", (char * )ebt_ip6_to_numeric(&ipinfo->dmsk));
	}
	...
	if (ipinfo->bitmask & EBT_IP6_PROTO) {
		...
		if(ipinfo->protocol[0] == ipinfo->protocol[1]){
			...
			bzero(proto_name, sizeof(proto_name));
			pe = getprotobynumber(ipinfo->protocol[0]);
			snprintf(proto_name, sizeof(proto_name), "%s", pe->p_name);
			pe2 = getprotobynumber(ipinfo->protocol[1]);
			if((pe == NULL) || (pe2 == NULL))
				printf("%d:%d ", ipinfo->protocol[0], ipinfo->protocol[1]);
			else
				printf("%s:%s ", proto_name, pe2->p_name);
		}
#endif
	}
...
#if defined(TCSUPPORT_CT_JOYME4)
	if (ipinfo->bitmask & EBT_IP6_FLABEL) {
		printf("--ip6-flabel ");
		if (ipinfo->invflags & EBT_IP6_FLABEL)
			printf("! ");
		printf("0x%X ",ipinfo->flowlabel);
	}
#endif
   ...
   const struct ebt_entry_match *m2)
{
	...
	#endif
#if defined(TCSUPPORT_CT_JOYME4)
	if (ipinfo1->bitmask & EBT_IP6_FLABEL) {
		if (ipinfo1->flowlabel != ipinfo2->flowlabel)
			return 0;
	}
#endif
	return 1;
