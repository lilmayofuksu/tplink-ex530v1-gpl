...
struct all_addr addr_host;
int ipver = 0;
int chkSamev6ip(struct all_addr *addrL, struct all_addr *addrR)
{
	int idx = 0;

	if ( NULL == addrL || NULL == addrR )
		return 0;

	for ( idx = 0; idx < 4; idx ++ )
	{
		if ( addrL->addr.addr6.s6_addr32[idx] != addrR->addr.addr6.s6_addr32[idx] )
			return 0;
	}
...
int extract_addresses(HEADER *header, size_t qlen, char *name, time_t now, union mysockaddr *source_addr)
#else
...
#if defined(TCSUPPORT_CT_JOYME4) || defined(TCSUPPORT_CT_UBUS)
			if(source_addr)
				URLFilterMatch(source_addr, (struct all_addr *)p1, flags);
#else
			if (match)
			{
				tcdbg_printf("set iptables rule!!\n");
				doSetRule(&match, (struct all_addr *)p1, flags);
				rule_set_flag++;
			}
#endif
#endif
...
#if defined(TCSUPPORT_CMCCV2)
			if(ntohs(header->ancount) > 1 && !search_flag)/*search ip just once*/
			{
				search_flag = 1;
				if( T_A == aqtype )
					sendMsg.action = SEARCH_V4_IP;
				else if( T_AAAA == aqtype )
					sendMsg.action = SEARCH_V6_IP;

				send2dnshost(&sendMsg);
				if(sendMsg.exists_flag == 2)/*do not find this name*/
					search_flag = 2;
				memcpy(&addr_host, &sendMsg.addr, sizeof(addr));
			}
			if( 1 == search_flag && chkSamev6ip(&addr_host,&addr) || ( 2 != search_flag && 1 == j ))
			{
				search_flag = 2;
				memcpy(&sendMsg.addr, p1, addrlen);
				ipver = 0;
				if( T_A == aqtype )
				{
					sendMsg.action = ADD_V4_IP;
					ipver |= F_IPV4;
				}
				else if( T_AAAA == aqtype )
				{
					sendMsg.action = ADD_V6_IP;
					ipver |= F_IPV6;
				}
				...
				if(sendMsg.exists_flag == 1)
					memcpy(&addr_host, p1, sizeof(addr_host));
				else
...
#if (defined(TCSUPPORT_CT_JOYME2) || defined(TCSUPPORT_CT_UBUS)) && defined(TCSUPPORT_CFG_NG)
#if !defined(TCSUPPORT_CT_JOYME4) && !defined(TCSUPPORT_CT_UBUS)
	if(match && rule_set_flag)
		match->chain_rule_add_flag = 1;
#endif
#endif
