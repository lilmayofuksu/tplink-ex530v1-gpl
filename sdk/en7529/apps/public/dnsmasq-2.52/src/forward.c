...
#if defined(TCSUPPORT_CT) || defined(TCSUPPORT_DNSBIND_DEV)
static int get_url_info(struct url *request_url, char *namebuf);
#endif
//#ifdef TCSUPPORT_DNSEACHPVC
#if defined(TCSUPPORT_DNSEACHPVC) || defined(TCSUPPORT_TR69_BIND_PVC) || defined(TCSUPPORT_DNSBIND_DEV)
...
extern int ponstate;
#endif
#if defined(TCSUPPORT_CT_PON) || defined(TCSUPPORT_ANDLINK)
...
#if (defined(TCSUPPORT_CT_JOYME2) || defined(TCSUPPORT_CT_UBUS)) && defined(TCSUPPORT_CFG_NG)
...
int get_vpn_url_info(struct url *request_url, int vpn_idx)
{
	char interface_name[IF_NAMESIZE + 1] = {0};
	
	snprintf(interface_name, sizeof(interface_name), "ppp%d", vpn_idx+100);
	request_url->url_type = URL_VPN;
	strncpy(request_url->interface, interface_name, IF_NAMESIZE);
...
#endif
#endif
#if defined(TCSUPPORT_TR69_BIND_PVC) || defined(TCSUPPORT_DNSBIND_DEV)
	char active[4]={0};
	char bindactive[4]={0};
#endif
...
#if defined(TCSUPPORT_CT_DNSBIND)
#if defined(TCSUPPORT_MULTI_USER_ITF)
	unsigned int lan_mark = GET_LAN_ITF_MARK(mark);
#else
	unsigned int lan_mark = (mark & 0xf0000000) >> 28;
#endif
#else
	unsigned int lan_mark = 0;
#endif

#if defined(TCSUPPORT_CT_DNSBIND)
  unsigned int ret = 0;
ret = checkUrlFilter(daemon->namebuff,mark);
	DT("dnsmasq C1:%s() ==> checkUrlFilter ret=[%d], mark=[%x], name=[%s].\n", __FUNCTION__, ret, mark, daemon->namebuff );
...
if(!strcmp(active,"Yes")&&!strcmp(bindactive,"Yes"))
...
#if defined(TCSUPPORT_CT_DNSBIND)
#if defined(TCSUPPORT_CT_L2TP_VPN) ||  defined(TCSUPPORT_CT_VPN_PPTP)
	vpn_mark = ((mark & 0x7f0000) >> 16); 
	vpn_idx = vpn_mark - 0x6e;
	if ((vpn_idx >= 0) && (vpn_idx < 16))
	{
		get_vpn_url_info(&request_url, vpn_idx);
	}
	else
#endif
#endif
...
#if (defined(TCSUPPORT_CT) || defined(TCSUPPORT_DNSBIND_DEV)) && defined(TCSUPPORT_VOIP)
...
#endif
#if defined(TCSUPPORT_DNSBIND_DEV) && defined(TCSUPPORT_VOIP)
	if ( URL_VOIP == request_url.url_type )
		iTR69Flag = 2;
#endif
  ...
  if (!daemon->servers)
  {
	DT("dnsmasq C1:%s() ==> NO server for [%s].\n", __FUNCTION__, daemon->namebuff );
    forward = NULL;
    ...
    {
		DT("dnsmasq C1:%s() ==> retry on existing query for [%s].\n", __FUNCTION__, daemon->namebuff );
	...
	flags = search_servers(now, &addrp, gotname, daemon->namebuff, &type, &domain);
		...
		DT("dnsmasq C1:%s() ==> gotname=[%d], flags=[%x], name=[%s].\n", __FUNCTION__, gotname, flags, daemon->namebuff );
      
      if (!flags && !(forward = get_new_frec(now, NULL)))
      {
		DT("dnsmasq C1:%s() ==> table full - server failure [%s].\n", __FUNCTION__, daemon->namebuff );
  ...
  if (!flags && forward)
	...
	{ 
	  ...
	  /* only send to servers dealing with our domain.
	     ...
	     must be NULL also. */
	  DT("dnsmasq C1:%s() ==> choose server ok [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);
	  if (type == (start->flags & SERV_TYPE) &&
		...
		for ( fwdidx = 0; fwdidx < AWN_IP_NUM; fwdidx ++)
		{
			if ( request_url.addr_used[fwdidx] )
			{
				...
				DT("dnsmasq C1:%s() ==> isfwd sendto server [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);

				if ( -1 != sendto(fd, (char *)header, plen, 0,
				 &(request_url.addr[fwdidx].sa),
				 sa_len(&(request_url.addr[fwdidx]))) )
				{
					DT("dnsmasq C1:%s() ==> isfwd sendto server fail [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);
					start->queries++;
					forwarded = 1;
		...
		DT("dnsmasq C1:%s() ==> sendto server [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);
	      if (sendto(fd, (char *)header, plen, 0,
			 &start->addr.sa,
			 sa_len(&start->addr)) == -1)
		{
			DT("dnsmasq C1:%s() ==> sendto server fail [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);
		  if (retry_send())
		    continue;
		}
	      else
		{
		  ...
		  DT("dnsmasq C1:%s() ==> sendto server OK [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);
		  if (!gotname)
...
#if defined(TCSUPPORT_CT) || defined(TCSUPPORT_TR69_BIND_PVC) || defined(TCSUPPORT_DNSBIND_DEV)
next_server:	  
#endif
	  if (!(start = start->next))
	   ...
	   if (start == firstsentto)
	  	{
			DT("dnsmasq C1:%s() ==> loop all server done for [%s].\n", __FUNCTION__, daemon->namebuff );
			break;
	  	}
	}
	...
	while (1)
	{
			...
			|| ds_frec_delay )
		{
			if ( NULL == start->sfd )
				continue;
			svrfd = start->sfd->fd;
			 DT("dnsmasq C1:%s() ==> delay send to server [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);
			if (sendto(svrfd, (char *)header, plen, 0,
			&start->addr.sa,
			sa_len(&start->addr)) == -1)
			{
				DT("dnsmasq C1:%s() ==> delay send to server fail [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);
				if (retry_send())
					continue;
			}
			else
			{
				...
				daemon->srv_save = start;
				daemon->packet_len = plen;
				DT("dnsmasq C1:%s() ==> delay send to server OK [%s], iface=[%s].\n", __FUNCTION__, daemon->namebuff, start->interface);
...
forward_end_pos1:
  ...
  if (udpfd != -1)
    {
    	DT("dnsmasq C1:%s() ==> return empty answer or address if known for whole domain [%s].\n", __FUNCTION__, daemon->namebuff);
...
notFindServer:  
  ...
  if (udpfd != -1)
    {
    	DT("dnsmasq C1:%s() ==> notFindServer return empty answer or address if known for whole domain [%s].\n", __FUNCTION__, daemon->namebuff);
      plen = setup_reply(header, plen, addrp, flags, daemon->local_ttl);
      send_from(udpfd, daemon->options & OPT_NOWILD, (char *)header, plen, udpaddr, dst_addr, dst_iface);
    }
#endif
...
static size_t process_reply(HEADER *header, time_t now, 
			    struct server *server, size_t n, union mysockaddr *addr)
#else
static size_t process_reply(HEADER *header, time_t now, 
...
#if defined(TCSUPPORT_CT_JOYME4) || defined(TCSUPPORT_CT_UBUS)
      if (extract_addresses(header, n, daemon->namebuff, now, addr))
#else
...
extern struct all_addr addr_host;
extern int ipver;
#endif
	  ...
	  if ((nn = resize_packet(header, (size_t)n, pheader, plen)))
	    {
	      header->qr = 0;
	      header->tc = 0;
		  DT("dnsmasq C2:%s() ==> for broken servers, attempt to send to another one [%s].\n", __FUNCTION__, daemon->namebuff);
...
#endif
	bzero(rc_name, sizeof(rc_name));
	snprintf(rc_name, sizeof(rc_name), "%s", daemon->namebuff);
	DT("dnsmasq C2:%s() ==> will do process_reply [%s].\n", __FUNCTION__, rc_name);

#if defined(TCSUPPORT_CT_JOYME4) || defined(TCSUPPORT_CT_UBUS)
      if ((nn = process_reply(header, now, server, (size_t)n, &forward->source)))
#else
      if ((nn = process_reply(header, now, server, (size_t)n)))
#endif
	{
#if defined(TCSUPPORT_CT_VLAN_BIND)
		DT("dnsmasq C2:%s() ==> forward->mark-[%x] name=[%s].\n", __FUNCTION__, forward->mark, rc_name);
#endif
#if defined(TCSUPPORT_CT_VLAN_BIND)
#if defined(TCSUPPORT_MULTI_USER_ITF)
		if(GET_LAN_ITF_MARK(forward->mark))
#else
	  ...
	  header->ra = 1; /* recursion if available */
		...
		{
#endif
		DT("dnsmasq C2:%s() ==> send reply to LAN [%s].\n", __FUNCTION__, rc_name);

#if defined(TCSUPPORT_CMCCV2)
	  if( 0 != addr_host.addr.addr6.s6_addr32[0] )
	  {
		  nn = setup_reply(header, (size_t)n, &addr_host, ipver, daemon->local_ttl);
...
void receive_query(struct listener *listen, time_t now)
{
  ...
  __u32			skb_mark = 0;
#if defined(TCSUPPORT_CMCCV2) || defined(TCSUPPORT_CT_FJ) || defined(TCSUPPORT_CT_REMOTE_UPG_WEBREDIRECT)
...
char control[CMSG_SPACE(sizeof(struct in_pktinfo)) + 
  ...
  {
	DT("dnsmasq C1:%s() ==> recvmsg error %d.\n", __FUNCTION__, errno);
    return;
  }
  ...
  if (n < (int)sizeof(HEADER) || 
      ...
      header->qr)
  {
	DT("dnsmasq C1:%s() ==> check head err.\n", __FUNCTION__ );
    return;
  }
  ...
  if (!(daemon->options & OPT_NOWILD))
      ...
      if (listen->family == AF_INET6)
	{
      ...
      {
      	DT("dnsmasq C1:%s() ==> iface_check err.\n", __FUNCTION__ );
	return;
      }
      ...
      {
      	DT("dnsmasq C1:%s() ==> SIOCGIFNETMASK err.\n", __FUNCTION__ );
	return;
      }
	...
	{
	  	case T_A:
			sendMsg.action = SEARCH_V4_IP;
			break;
		case T_AAAA:
			sendMsg.action = SEARCH_V6_IP;
			break;
		default:
			sendMsg.action = SEARCH_V4_IP;
			break;
	}
	sendMsg.bitflag = BIT_DNS;
	strncpy(sendMsg.domain_name,daemon->namebuff,sizeof(sendMsg.domain_name)-1);
	send2dnshost(&sendMsg);
	if( 1 == sendMsg.exists_flag ){
		if (SEARCH_V4_IP == sendMsg.action)
		{
			rep_addr.addr.addr4.s_addr = htonl(sendMsg.addr.addr4.s_addr);
			t_plen = setup_reply(header, (size_t)n, &rep_addr, F_IPV4, daemon->local_ttl);
		}
		else if(SEARCH_V6_IP == sendMsg.action)
		{
			memcpy(&rep_addr.addr.addr6,&sendMsg.addr.addr6,sizeof(rep_addr.addr.addr6));
	...
	{
		res = DNSFilterMatch(header, n, &source_addr, &dst_addr, if_index, listen->family, listen->fd);
		DT("dnsmasq C1:%s() ==> DNSFilterMatch ret=%d, name[%s].\n", __FUNCTION__, res , daemon->namebuff);
#if defined(TCSUPPORT_CT_JOYME4) || defined(TCSUPPORT_CT_UBUS) 
		ret_val = doURLMatch(&source_addr, listen->family);
		if(res || ret_val)
			return;
#else
		if(res)
		...
		if(0 == tcapi_get("UrlFilter_Common", "Activate", activate) &&  0 != atoi(activate))
		{
			memset (policy, 0, sizeof(policy));
			if(0 == tcapi_get("UrlFilter_Common", "Filter_Policy", policy) &&  0 != atoi(policy))
		{
			match = doURLMatch();
			DT("dnsmasq C1:%s() ==> DNSFilterMatch match=%X.\n", __FUNCTION__, match );
			if(!match)
			{
				return;
			}	
...
#if defined(RA_PARENTALCONTROL)
		memset (activate, 0, sizeof(activate));
		tcapi_get("Parental_Common", "Active", activate);
		if ( !strcmp(activate, "Yes"))
		{
			ret_val2 = doParentalMatch(&source_addr, listen->family);
			if(ret_val2)
				return;
		}
#endif
  ...
  if (extract_request(header, (size_t)n, daemon->namebuff, &type))
    {
      char types[20];
		...
		/* Fix nessus scan issue 35450:
		DNS Server spoofed request ampilfication ddos.
		Reply nothing when query root zone.
		*/
		if ( 0 == daemon->namebuff[0] )
		{
			DT("dnsmasq C1:%s() ==> EMPTY dns name.\n", __FUNCTION__ );
	...
	{
		DT("dnsmasq C1:%s() ==> checkDNS_SpeedLimit drop for [%s].\n", __FUNCTION__, daemon->namebuff );
		return; /* drop the query. */
	}
#endif
...
#ifdef CT_COM_DEVICEREG 
   DT("dnsmasq C1:%s() ==> devreg=%d, name=[%s].\n", __FUNCTION__, devreg, daemon->namebuff );

   if(/*registerFlag*/ devreg)
  {
#if defined(TCSUPPORT_MULTI_USER_ITF)
	if( !tr69_flag && (GET_LAN_ITF_MARK(skb_mark)) )
#else
...
#endif
#endif
	DT("dnsmasq C1:%s() ==> will cache local for=[%s].\n", __FUNCTION__, daemon->namebuff );
  	cache_reload();
     }
  }
#endif
...
/* there is no TR69 or dnsbind in ALINK VER. */
#if defined(TCSUPPORT_ANDLINK)
	if ( strstr(daemon->namebuff, "cmhi.10086.cn") )
	{
		snprintf(dnsBuf, sizeof(dnsBuf), "%s %s def\n"
									, gLANIP, daemon->namebuff);
		doValPut("/etc/hosts", dnsBuf);
		cache_reload();
	}
#endif
  ...
  DT("dnsmasq C1:%s() ==> answer_request m=[%d], skb_mark=[%x], name=[%s].\n", __FUNCTION__, m, skb_mark, daemon->namebuff );
...
#if defined(TCSUPPORT_CT_JOYME4) || defined(TCSUPPORT_CT_UBUS)
		    m = process_reply(header, now, last_server, (unsigned int)m, NULL);
#else
...
static int checkTR69Url(int *flag, char *packetUrl)
{
  ...
  if(fp == NULL){
	  ...
	  while(fgets(dnsValue, sizeof(dnsValue), fp)){
...
#if defined(TCSUPPORT_CT_L2TP_VPN) || defined(TCSUPPORT_CT_VPN_PPTP)
	extern struct vpn_dns_group *gVPNDNSGrp;
	struct vpn_dns_group *cur_vpndns_group = NULL;
	void * value = NULL;
	char itf_name[IF_NAMESIZE + 1] = {0};
#endif
#endif
...
#if defined(TCSUPPORT_CT_JOYME2)
#if defined(TCSUPPORT_CT_L2TP_VPN) || defined(TCSUPPORT_CT_VPN_PPTP)
	fp = fopen("/etc/vpn_dnsInfo.conf", "r");

	if(fp == NULL){
	  //tcdbg_printf("\n dnsInfo.conf not exit or open file failed");
	}
	else 
	{	
		if(fgets(dnsValue, MAXSIZE - 1, fp))
		{
			if(strlen(dnsValue) == 0)
			{
				tcdbg_printf("\n content is empty.");
			...
			else
			{
				...
				{
					if((strcmp(cur_vpndns_group->domain, namebuf) == 0)
						|| (domain_reg_match(cur_vpndns_group->domain, namebuf) == 0))
					{
						request_url->url_type = URL_VPN;
						snprintf(itf_name, sizeof(itf_name), "ppp%d", 100+(cur_vpndns_group->vpn_tunnel_idx));
						strncpy(request_url->interface, itf_name, IF_NAMESIZE);
						break;
					}
				}
			}
	...
	if(1 == iTR69Flag) //request come from tr69
	{
	    ...
	    }else //request come from other application
	    {
	    	while(NULL != *start)
	    	{
		    	...
		    	while(NULL != currNode)
		    	{
				if ((0 == strcmp(currNode->strInterface, (*start)->interface)) 
					&& (strcmp(currNode->strApplication, "TR69") != 0))
				{	    		
					if (AF_INET == (*start)->addr.sa.sa_family)
					{
						/* dns server ip is ipv4 */
						if (0 == strcmp(currNode->strDNS, inet_ntoa((*start)->addr.in.sin_addr)))
						{
							iMatch = 1;
							break;
					...
					else if (AF_INET6 == (*start)->addr.sa.sa_family)
					{	
						/* dns server ip is ipv6 */
						if (0 == strcmp(currNode->strDNS, inet6_ntoa((*start)->addr.in6.sin6_addr)))
						{
							iMatch = 1;
							break;
						}
...
{	
	...
	while(currNode!=NULL)
	{
		if(0 == strcmp(currNode->strDNS, dnsIpAddr) && 
					0 == strcmp(currNode->strInterface, (*start)->interface))
		{
#if defined(TCSUPPORT_DNSBIND_DEV)
		if ( 2 == iTR69Flag ) /* VOICE */
		{
			if ( strstr(currNode->strApplication, "VOICE")
				|| 0 == strcmp(currNode->strApplication, "ALL") )
				break;
		}
#if defined(TCSUPPORT_TR69_BIND_PVC)
		if ( 1 == iTR69Flag )
		{
			if ( strstr(currNode->strApplication, "TR069")
				|| 0 == strcmp(currNode->strApplication, "ALL") )
				break;
		}
#endif
		...
		{
			if ( strstr(currNode->strApplication, "DEFAULT")
				|| 0 == strcmp(currNode->strApplication, "ALL") )
				break;
		}
#else
...
{	
	...
	char dnsIpAddr[40] = {0};
#if defined(TCSUPPORT_CT_L2TP_VPN) || defined(TCSUPPORT_CT_VPN_PPTP)
	char vpn_intf_name[IF_NAMESIZE] = {0}; 
	int vpn_mark = -1; 
	int vpn_idx = -1;
#endif

	if((*start)->addr.sa.sa_family == AF_INET) {//dns server ip is ipv4
			strncpy(dnsIpAddr, inet_ntoa((*start)->addr.in.sin_addr), sizeof(dnsIpAddr) - 1);
	}
	#ifdef HAVE_IPV6
	else {//dns server ip is ipv6
		strncpy(dnsIpAddr, inet6_ntoa((*start)->addr.in6.sin6_addr), sizeof(dnsIpAddr) - 1);
	}
	#endif
					...
					0 == strcmp(currNode->strInterface, (*start)->interface) )
    		{
...
#if defined(TCSUPPORT_CT_JOYME2)
#if defined(TCSUPPORT_CT_L2TP_VPN) || defined(TCSUPPORT_CT_VPN_PPTP)
			else if(request_url->url_type == URL_VPN){			
				if(0 == strcmp(request_url->interface, (*start)->interface)){
						allowed = 1;
						break;			
				}	
				...
				if(0 != strcmp(currNode->strApplication, "TR069")){					
					...
					if (bind_reval == 0){/*1.lan if is binded, and wan interface match*/						
		    				allowed = 1;
						DT("dnsmasq C1:%s() ==> lan iface is binded, and wan interface match, name=[%s].\n", __FUNCTION__, daemon->namebuff );
						break;
					}				
						...
						if(0 != strcmp(currNode->strApplication, "OTHER")){	
							...
							{
								DT("dnsmasq C1:%s() ==> lan iface is not binded or from CPE, select internet wan iface, name=[%s].\n", __FUNCTION__, daemon->namebuff );
								allowed = 1;
								break;
							}
						}
	...
	DT("dnsmasq C1:%s() ==> allowed=[%d], name=[%s].\n", __FUNCTION__, allowed, daemon->namebuff );
...
static int check_bind(char *wan_if, __u32 mark)
{
	...
	DT("dnsmasq C1:%s() ==> pt_mark=[%x] name=[%s].\n", __FUNCTION__, pt_mark, daemon->namebuff );
...
#if defined(TCSUPPORT_CT_VLAN_BIND)
		if ( 1 == get_VlanBindByPort(pt_mark - 1) )
			snprintf(vbindActive, sizeof(vbindActive), "%s", "Yes");
		else
			snprintf(vbindActive, sizeof(vbindActive), "%s", "No");
		DT("dnsmasq C1:%s() ==> pt_mark=[%x], vbindActive=[%s], name=[%s].\n", __FUNCTION__, pt_mark, vbindActive, daemon->namebuff );
#else
		snprintf(wan_node, sizeof(wan_node), "VlanBind_Entry%d", pt_mark-1);
	...
	DT("dnsmasq C1:%s() ==> pt_mark=[%x], switchOn=[%d], vbindActive=[%s], name=[%s].\n", __FUNCTION__, pt_mark, switchOn, vbindActive, daemon->namebuff );
...
#endif
	{
		...
		bind_flag = 0;
		if ( 1 == get_PortBindByPort(pt_mark - 1, wan_if_val, sizeof(wan_if_val)) )
			bind_flag = 1;
		DT("dnsmasq C1:%s() ==> pt_mark=[%x], bind_flag=[%d], wan_if_val=[%s], name=[%s].\n"
				, __FUNCTION__, pt_mark, bind_flag, wan_if_val, daemon->namebuff );
#else
#if defined(TCSUPPORT_MULTI_USER_ITF)
	...
	DT("dnsmasq C1:%s() ==> pt_mark=[%x], bind_flag=[%d], name=[%s].\n", __FUNCTION__, pt_mark, bind_flag, daemon->namebuff );
	if (1 == bind_flag) {
		/* check wan if */
		if (!strcmp(wan_if_val, wan_if)) {
			DT("dnsmasq C1:%s() ==> lan bind, wan interface match, name=[%s].\n", __FUNCTION__, daemon->namebuff );
			return 0;
		}
	}
	else {
		/* the lan if no bind */
		DT("dnsmasq C1:%s() ==> lan not bind, name=[%s].\n", __FUNCTION__, daemon->namebuff );
		return 1;
	}
	}

	DT("dnsmasq C1:%s() ==> wan interface NOT match, name=[%s].\n", __FUNCTION__, daemon->namebuff );
