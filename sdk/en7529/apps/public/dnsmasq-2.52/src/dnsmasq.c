	...
	*/
	for ( idx = 0; idx < MAX_DNS_LIMIT_RULE; idx ++)
	{
			...
			&& 0 != domain[0] )
		{
			...
			bzero(*new_dns_speed_limit_group, sizeof(struct dns_speed_limit_group));
...
#if defined(TCSUPPORT_CT_JOYME2) || defined(TCSUPPORT_CT_UBUS)
			(*new_dns_speed_limit_group)->request_times = 0;
#endif
...
#if defined(TCSUPPORT_CT_JOYME2) || defined(TCSUPPORT_CT_UBUS)
				cur_limit_group->request_times = 0;
#endif
				cur_limit_group->time = dnsmasq_time();
			}

#if defined(TCSUPPORT_CT_JOYME2) || defined(TCSUPPORT_CT_UBUS)
			cur_limit_group->request_times++;
			if(cur_limit_group->request_times > cur_limit_group->limit_count)
			{
				...
				bzero(new_client_group, sizeof(struct dns_speed_limit_client_group));
...
int writeDeviceInfoReport()
{
	...
	{
#if defined(TCSUPPORT_CT_JOYME2) || defined(TCSUPPORT_CT_UBUS)
		if ( cur_limit_group->request_times > 
						cur_limit_group->limit_count )
#else
...
#define DNS_LIMIT_ALERT 21
	...
	DT("dnsmasq C1:%s() ==> gDNSSpeedLimitMode=%d.\n", __FUNCTION__, gDNSSpeedLimitMode );
	...
	if ( DNS_SPEED_LIMIT_ACTION_ALERT == ret )
	{
		...
		if(-1 == ecnt_event_send(ECNT_EVENT_DBUS,
						ECNT_EVENT_DBUS_DNSSPEEDLIMIT,
						NULL,
						0))
			tcdbg_printf("\nsend message to dbus error!\n");						
#endif
	}
...
gDNS_Param.pHead = (struct DNS_Info*)malloc(sizeof(struct DNS_Info));
  ...
  if ( 1 == ponstate )
	...
	{	
		ParentalFilterDB_init();
		atexit(ParentalFilterDB_exit);
		initParentalFilterInfo();
	}
#endif
...
#endif
#if defined(TCSUPPORT_CT_VLAN_BIND)
	if ( 0 != stat("/tmp/wan_startup_optimization", &fbuf) )
	{
		initVlanBindGroup();
		initPortBindGroup();
	}
...
#  endif
		...
		/* check switch every loop, 
		some issue cannot reproduce when dnsmasq restart. */
		DT_SW();
      ...
      if (select(maxfd+1, &rset, &wset, &eset, tp) < 0)
	{
      ...
      now = dnsmasq_time();
      memcpy(&tmp_t, &now, sizeof(now));
      DT("dnsmasq:%s() ==> fd data changed at %s ", __FUNCTION__, asctime(localtime(&tmp_t)) );
...
static void check_dns_listeners(fd_set *set, time_t now)
{
  ...
  for (serverfdp = daemon->sfds; serverfdp; serverfdp = serverfdp->next)
    ...
    {
		DT("dnsmasq C2:%s() ==> recv wan reply, do reply_query.\n", __FUNCTION__ );
  ...
  for (listener = daemon->listeners; listener; listener = listener->next)
    {
      ...
      {
		DT("dnsmasq C1:%s() ==> recv lan query, do receive_query.\n", __FUNCTION__ );
      ...
      if (listener->tcpfd != -1 && FD_ISSET(listener->tcpfd, set))
	{
	  ...
	  if (!iface)
		...
		res = fcntl(confd, F_SETFL, flags & ~O_NONBLOCK);
...
//#ifdef TCSUPPORT_DNSEACHPVC
#if defined(TCSUPPORT_DNSEACHPVC) || defined(TCSUPPORT_TR69_BIND_PVC) || defined(TCSUPPORT_DNSBIND_DEV)
static void resolve_dnsServerConf_core(char* path)
{
	FILE* fp = NULL;
...
static void resolve_dnsServerConf(){
	resolve_dnsServerConf_core("/etc/dnsInfo.conf");
#if defined(TCSUPPORT_CT_JOYME2)
#if defined(TCSUPPORT_CT_L2TP_VPN) || defined(TCSUPPORT_CT_VPN_PPTP)
	resolve_dnsServerConf_core("/etc/vpn_dnsInfo.conf");
#endif
#endif
}
