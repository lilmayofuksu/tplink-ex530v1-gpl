...
#define ECNT_WLANAC_PORT_OFFSET		(8)
#endif

#if defined(TCSUPPORT_CT_JOYME2) && defined(TCSUPPORT_CFG_NG)
#include <hashmap.h>
#endif
...
#if defined(TCSUPPORT_CT_L2TP_VPN) || defined(TCSUPPORT_CT_JOYME2)
#define VPN_ENTRY 			"VPN_Entry"
#define VPN_DELENTRY		"VPN_DelDomainEntry"
#define VPN_ATTACH_NUM		2048
#define VPN_INSTANCE_NUM	16
#define VPNDOAMIN_ENTRY		"VPNDomain_Entry"
#define VPNIPS_ENTRY		"VPNIPS_Entry"
#define VPNMAC_ENTRY		"VPNMAC_Entry"
...
int checkDomainType(char *domain, int tunnel, int domain_name_id);
#if defined(TCSUPPORT_CT_JOYME2)
...
int extract_addresses(HEADER *header, size_t qlen, char *namebuff, time_t now, union mysockaddr *addr);
#else
...
struct wan_portbind_group
{
	int portBindGroup[MAX_LAN_PORT_NUM];
	char wan_iface[32];
};
...
#define MAX_MAC_LEN			512
struct dns_filter_entry{	
	char *key;
	int action; //0:reject 1:Lan IP 2:Error	
...
extern hashmap_map *DNSFilterMap;

struct url_filter_entry{	
	char *key;
	char url[MAX_HOSTNAME_LEN];	
	char Mac[MAX_MAC_LEN];
	unsigned int entry_idx;
	int forward_rule_add_flag;
	int chain_rule_add_flag;
	int v4_add_flag;
	int v6_add_flag;
};
extern hashmap_map *URLFilterMap;
...
#define	DT_SW		dnsmasq_trace_switch_check
#define DT			dnsmasq_trace
