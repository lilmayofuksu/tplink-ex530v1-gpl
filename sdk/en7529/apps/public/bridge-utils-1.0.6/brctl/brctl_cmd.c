...
#ifdef TCSUPPORT_MACDUPDETECT
	printf("port no\tmac addr\t\tis local?\tipv4\tageing timer\n");
#else
...
static int br_cmd_igmpsnoop(int argc, char *const* argv)
{
	...
	igmpsnoop |= (1<<BRCTL_IGMPSNOOPING_OFFSET);
	#endif
...
#else
#ifdef TCSUPPORT_SNOOPING_SEPERATION	
	printf("igmpsnooping enabled: %s\n", info.igmpsnoop_enabled?"yes":"no");
	printf("mldsnooping enabled: %s\n", info.mldsnoop_enabled?"yes":"no");
#else
...
#else
#ifdef TCSUPPORT_SNOOPING_SEPERATION
static int br_cmd_mldsnoop(int argc, char *const* argv)
{
	int mldsnooping = 1, err;	
	...
	mldsnooping |= (1<<BRCTL_MLDSNOOPING_OFFSET);
	err = br_mldsnoop_set_state(argv[1], mldsnooping);
	if(err)
		fprintf(stderr, "set mld snooping state failed\n");
...
#ifdef TCSUPPORT_SNOOPING_SEPERATION
{ 2, "igmpsnoop", br_cmd_igmpsnoop,
  "<bridge> {on|off}\tturn igmpsnooping on/off" },
{ 2, "mldsnoop", br_cmd_mldsnoop,
