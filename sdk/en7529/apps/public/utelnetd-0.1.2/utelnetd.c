...
{
	printf("Usage: telnetd [-p port] [-l loginprogram] [-d] [-a address] [-i ifname]\n");
	printf("\n");
	printf("   -p port          specify the tcp port to connect to\n");
	printf("   -l loginprogram  program started by the server\n");
	printf("   -d               daemonize\n");
	printf("   -a address       specify the local address to connect\n");
	printf("   -i ifname        specify the interface to bind\n");
	printf("\n");         
	exit(1);
}
...
#endif
{
	...
	/*zero allock buffer*/
	memset(ts->buf1, 0, BUFSIZE);
	memset(ts->buf2, 0, BUFSIZE);
...
static int init_start_time(void)
{
...
static int checkTimeOut(void)
{
	...
	ret = tcapi_get("Account_TelnetEntry", "Timeout", tmp);

	/*if timeout is not set, default timeout is 24h*/
	if(ret < 0)
		tmout_hour = 24;
	else
		tmout_hour = atoi(tmp);

	/*if timeout is set to 0, do not exit*/
	if(!tmout_hour)
		return 0;

	bzero(&cur_t, sizeof(cur_t));
	clock_gettime(CLOCK_MONOTONIC, &cur_t);
	if ( cur_t.tv_sec - start_time.tv_sec >= (tmout_hour*60*60) )
		exit(1);
...
int main(int argc, char **argv)
{
...
#if defined(TCSUPPORT_CT_JOYME2)
	FILE*fp_multip_login = NULL;
#endif

	/* check if user supplied a port number */

	for (;;) {
		c = getopt( argc, argv, "p:l:hda:i:");
		if (c == EOF) break;
		switch (c) {
			case 'p':
				portnbr = atoi(optarg);
	...
	if(ifname)
	{
		if ( -1 == setsockopt(master_fd, SOL_SOCKET, SO_BINDTODEVICE, ifname, 32))
		{
...
#if defined(TCSUPPORT_CT_JOYME2)
				argv_init[1] = "-t";
				fp_multip_login = fopen("/var/run/multip_login", "r");
				if(fp_multip_login == NULL)
				{
				/* only keep one session. */
				if ( 0 != sessions )
					argv_init[2] = "-d";
				else
					argv_init[2] = NULL;
