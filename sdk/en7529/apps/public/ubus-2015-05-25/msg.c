...
static int ubus_cli_send_ext(struct ubus_context *ctx, int argc, char **argv)
{
	...
	voip_evt_t voip_evt;
	mesh_evt_t mesh_evt;
	wan_related_evt_t wan_related_evt;
	...
	memset(&wan_related_evt, 0, sizeof(wan_related_evt));
	...
	switch (type)
	{
		...
		{
			snprintf(evt_type, sizeof(evt_type), "%s", EVT_WAN_RELATED_INTERNAL);
			break;
		}
		default:
		{
	...
	switch (evt_id)
	{
		...
		{
			snprintf(wan_related_evt.buf, sizeof(wan_related_evt.buf), "%s", argv[2]);
			blob_buf_init(&b, 0);
			blob_put(&b, 1, &evt_id, sizeof(evt_id));
			blob_put(&b, 2, &wan_related_evt, sizeof(wan_related_evt));
			break;
		}
		default:
		{
...
static int usage(const char *prog)
{
	fprintf(stderr,
		...
		" - send_ext [type 1 wan internal / 2 wan external / 3 voip internal / 5 mesh internal / 7 wan related internal] [evt_id ] [message] send an evt\n"
		"\n", prog);
	return 1;
}
