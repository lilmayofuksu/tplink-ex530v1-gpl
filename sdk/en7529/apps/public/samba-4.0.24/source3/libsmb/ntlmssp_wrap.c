...
static NTSTATUS gensec_ntlmssp3_client_start(struct gensec_security *gensec_security)
{
	...
	if (gensec_security->want_features & GENSEC_FEATURE_SESSION_KEY) {
		gensec_ntlmssp->ntlmssp_state->required_flags |= NTLMSSP_NEGOTIATE_SIGN;
	}
	...
	if (gensec_security->want_features & GENSEC_FEATURE_SEAL) {
		gensec_ntlmssp->ntlmssp_state->required_flags |= NTLMSSP_NEGOTIATE_SIGN;
		gensec_ntlmssp->ntlmssp_state->required_flags |= NTLMSSP_NEGOTIATE_SEAL;
	}

	gensec_ntlmssp->ntlmssp_state->neg_flags |= gensec_ntlmssp->ntlmssp_state->required_flags;
