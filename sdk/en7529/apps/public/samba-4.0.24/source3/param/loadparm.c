...
static void init_globals(bool reinit_globals)
{
	...
	Globals.bAllowDcerpcAuthLevelCconnect = false; /* we don't allow this by default */
...
*/
	...
	Globals.client_ipc_signing = SMB_SIGNING_REQUIRED;
...
#define MNT_FAT		"vfat"
#define MNT_NTFS	"fuseblk"
#define REPLY_FAT	"FAT"
#define REPLY_NTFS	"NTFS"
#define REPLY_SMB	"Samba"

void set_fstype(char *path, int serv_num)
{
	...
	if ((fp = fopen("/proc/mounts", "r")) != NULL){
		while ((fgets(buf, sizeof(buf), fp)) != NULL){			
			...
			if ((tmp_str = strstr(buf, path)) != NULL){
				if ((tmp_str = strstr(buf, MNT_FAT)) != NULL){ 
					string_set(&ServicePtrs[serv_num]->fstype, REPLY_FAT);
				} else if ((tmp_str = strstr(buf, MNT_NTFS)) != NULL) {				
					string_set(&ServicePtrs[serv_num]->fstype, REPLY_NTFS);
				} else {
					string_set(&ServicePtrs[serv_num]->fstype, REPLY_SMB);
				}
				break;
			}
			else {
				DEBUG(3, ("In set_fstype, readline=%s\n", buf));
			}
		}		
		fclose(fp);
	}
