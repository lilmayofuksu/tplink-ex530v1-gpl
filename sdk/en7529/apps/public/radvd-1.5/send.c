...
extern unsigned int g_skb_mark;
...
extern int get_interface_linkmtu(const char *iface, uint32_t *mtu);
int
send_ra(int sock, struct Interface *iface, struct in6_addr *dest)
{
	...
	ret = get_interface_linkmtu(iface, &iface->AdvLinkMTU);
	if(ret != 0)
		printf("send_ra get_interface_linkmtu failed!\n");
	...
	{
		skb_len = sizeof(int);
		if( 0 != setsockopt(sock, SOL_IPV6, IPV6_SKB_MARK, &g_skb_mark, skb_len) )
		{
			tcdbg_printf("%s: L%d, setsockopt(SOL_IPV6): %s\n", __func__, __LINE__, strerror(errno));
			return -1;
		}
	}
