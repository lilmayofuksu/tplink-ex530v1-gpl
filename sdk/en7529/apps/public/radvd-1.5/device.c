...
int
get_interface_var(const char *iface,
		  const char *var, const char *name,
		  uint32_t *val)
{
	FILE *fp;
	...
	if(fgets(line_string, sizeof(line_string)-1, fp) == NULL){
		fclose(fp);
		return -1;
	}

	*val = atoi(line_string);
	fclose(fp);
...
int
get_interface_linkmtu(const char *iface, uint32_t *mtu)
{
	int ret=0;

	ret = get_interface_var(iface,
			 PROC_SYS_IP6_LINKMTU, "LinkMTU",
			 mtu);
