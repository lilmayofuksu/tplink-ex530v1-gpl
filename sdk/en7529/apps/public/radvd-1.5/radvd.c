...
main(int argc, char *argv[])
{
	...
	/* parse args */
#ifdef HAVE_GETOPT_LONG
	while ((c = getopt_long(argc, argv, "d:C:l:m:p:t:u:vhs:M:", prog_opt, &opt_idx)) > 0)
#else
	while ((c = getopt(argc, argv, "d:C:l:m:p:t:u:vhs:M:")) > 0)
#endif
	{
		switch (c) {
		case 'C':
	...
	if((long)getpid() == (long)atoi(pidstr))
		unlink(pidfile);
	exit(0);
}
