...
unsigned int unknow_mul_pkt_v6 = 0;
unsigned long long unknow_mul_byte_v6 = 0;

/* Prototypes, to forward some functions */
void update_interfaces(struct intnode *intn);
void l2_ethtype(struct intnode *intn, const uint8_t *packet, const unsigned int len, const unsigned int ether_type, const unsigned char* mac);
...
void mld1_send_reduction(struct intnode *intn, const struct in6_addr *mca);
void mld1_send_reduction(struct intnode *intn, const struct in6_addr *mca)
{
	struct mld_report_packet
	{
	...
	/* Report -> Multicast address */
#if 1
	/*  link-scope all-nodes (ff02::2) */
	packet.ip6.ip6_dst.s6_addr[0]	= 0xff;
	packet.ip6.ip6_dst.s6_addr[1]	= 0x02;
	packet.ip6.ip6_dst.s6_addr[15]	= 0x02;
#else
	memcpy(&packet.ip6.ip6_dst, mca, sizeof(*mca));
#endif
	...
	dolog(LOG_DEBUG, "Sending MLDv1 Reduction on %s/%u\n", intn->name, intn->ifindex);
	sendpacket6(intn, (const struct ip6_hdr *)&packet, sizeof(packet));
...
void mld_send_reduction_all(struct intnode *interface, const struct in6_addr *mca);
void mld_send_reduction_all(struct intnode *interface, const struct in6_addr *mca)
{
	unsigned int		i;
	struct intnode		*intn;
...
void l4_ipv6_icmpv6_mld1_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen, const unsigned char* mac);
void l4_ipv6_icmpv6_mld1_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen, const unsigned char* mac)
{
	...
	struct grpclientlistnode *grpclient = NULL;
	...
	/*check multicast_drop list*/	
	memcpy(ipv6_addr,mld1->mca.s6_addr,16); 
	blapi_traffic_del_hwdrop_ipv6_unknown_mc(ipv6_addr);
	...
	if ( g_conf->quick_leave )
	{
		/* Find the groupnode */
		groupn = group_find(&mld1->mca);

		if ( groupn && groupn->clients )
		{
			if ( 0 == groupn->clients->count )
				isnew = 1;
			else
				isnew = 0;

			/* Find the clients in this group */
			grpclient = grpclient_find(groupn->clients, mac);

			if ( NULL == grpclient )
			{
				/* Create the groupinterface node */
				grpclient = grpclient_create(mac);

				/* Add the group to the list */
				if (grpclient) listnode_add(groupn->clients, (void *)grpclient);
			}
	...
	if (isnew) 
	{
		...
		system("echo  > /proc/tc3162/hwnat_del_mlddrop");
	}

	return;
}

void l4_ipv6_icmpv6_mld1_reduction(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen, const unsigned char* mac);
void l4_ipv6_icmpv6_mld1_reduction(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen, const unsigned char* mac)
{
	struct groupnode	*groupn = NULL;
	struct grpintnode	*grpintn = NULL;
	...
	if ( g_conf->quick_leave )
	{
		if ( groupn && groupn->clients )
		{
			...
			/* Remove client */
			if ( grpclient )
				listnode_delete(groupn->clients, grpclient);

			if ( 0 == groupn->clients->count )
				mld_send_reduction_all(intn, &mld1->mca);
		}
	}
#endif
...
void l4_ipv6_icmpv6_mld2_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld2_report *mld2r, const uint16_t plen)
{
	...
	while (ngrec > 0)
	{
		...
		else
		{
			...
			/* here we send mldv2 query with the only one src address or with all source */
			if(grec->grec_type == MLD2_BLOCK_OLD_SOURCES){
				if(1 == ntohs(grec->grec_nsrcs))
					mld_send_query(intn, &grec->grec_mca, first_src, false);
				else
					mld_send_query(intn, &grec->grec_mca, NULL, false);
			}
		}
...
#ifdef TCSUPPORT_IGMP_QUICK_LEAVE
				if (0 == groupn->clients->count) continue;
#else
				if (grpintn->ifindex != intn->ifindex) continue;
#endif
...
void send_packet_to_interface(unsigned int	ifindex, struct iphdr *iph, const uint16_t len)
{
	struct intnode		*interface = NULL;
	unsigned int pvc_index = 0, entry_index = 0,iface_idx = 0;
	unsigned char wan_type=0,wan_index = 0;

	/* Get the interface */
	interface = int_find(ifindex);
	if (!interface) return;

#if defined(TCSUPPORT_CT_PON)
	if ( 0 != g_conf->upstream[0] )
	{
		if ( 'p' == g_conf->upstream[0] )
		{
			sscanf(g_conf->upstream, "ppp%d", &iface_idx);
			wan_type = 1;
		}
		else
		{
			sscanf(g_conf->upstream, "nas%d_%d",
					&pvc_index, &entry_index);
			wan_type = 2;
			iface_idx = pvc_index * MAX_SMUX_NUM + entry_index;
		}
		wan_index = (unsigned char)iface_idx;
	}
#endif

	#ifdef TCSUPPORT_MULTICAST_SPEED
	blapi_traffic_ipv6_mc_set_wan_info_to_sock(g_conf->rawsocket,wan_type,wan_index);
	#endif
...
D(
	...
	if (!groupn
#ifdef TCSUPPORT_IGMP_QUICK_LEAVE
			|| (groupn && groupn->clients && (0 == groupn->clients->count))
#endif
		)
	{
...
#endif
		unknow_mul_byte_v6 += (len+18);
		unknow_mul_pkt_v6++;

		blapi_traffic_hwdrop_ipv6_unknown_mc_by_sock(g_conf->rawsocket,iph->ip6_dst.s6_addr);
	...
	LIST_LOOP(groupn->interfaces, grpintn, in)
	{
		...
		/* the include/exclude operation is implemented in kernel */
		send_packet_to_interface(grpintn->ifindex, iph, len);
#else
 ...
 */
void l4_ipv6_icmpv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, struct icmp6_hdr *icmpv6, const uint16_t plen, const unsigned char* mac);
void l4_ipv6_icmpv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, struct icmp6_hdr *icmpv6, const uint16_t plen, const unsigned char* mac)
{
	uint16_t		csum;
	...
	if(!allowGlobalSource)
	{
		...
		if (icmpv6->icmp6_type == ICMP6_MEMBERSHIP_REPORT)
		{
			l4_ipv6_icmpv6_mld1_report(intn, iph, len, (struct mld1 *)icmpv6, plen, mac);
		}
		else if (icmpv6->icmp6_type == ICMP6_MEMBERSHIP_REDUCTION)
		{
			l4_ipv6_icmpv6_mld1_reduction(intn, iph, len, (struct mld1 *)icmpv6, plen, mac);
		}
#ifdef ECMH_SUPPORT_MLD2
...
void l3_ipv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, const unsigned char* mac);
void l3_ipv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, const unsigned char* mac)
{
	struct ip6_ext		*ipe;
	uint8_t			ipe_type;
	...
	{
		/* Take care of ICMPv6 */
		l4_ipv6_icmpv6(intn, iph, len, (struct icmp6_hdr *)ipe, plen, mac);
		return;
	}
...
void l2_ethtype(struct intnode *intn, const uint8_t *packet, const unsigned int len, const unsigned int ether_type, const unsigned char* mac)
{
	...
	{
		l3_ipv6(intn, (struct ip6_hdr	*)packet, len, mac);
	}
...
void update_interfaces(struct intnode *intn)
{
	...
	if (getifaddrs(&ifap) == 0)
	{
		for (ifa=ifap;ifa;ifa=ifa->ifa_next)
		{
			...
			{
				memcpy(&addr,
					&((struct sockaddr_in *)ifa->ifa_addr)->sin_addr, sizeof(((struct sockaddr_in *)ifa->ifa_addr)->sin_addr));
			}
...
void sigusr1(int i)
{
	...
	/* Truncate the file */
	ret = ftruncate(fileno(g_conf->stat_file), (off_t)0);
	...
	sprintf(cmd, "echo %d %llu > /proc/tc3162/unknow_multicast_v6_cnt", unknow_mul_pkt_v6, unknow_mul_byte_v6);
	system(cmd);
	unknow_mul_pkt_v6 = 0;
	unknow_mul_byte_v6 = 0;
	/* Reset the signal */
	signal(i, &sigusr1);
}
...
#ifdef TCSUPPORT_MULTICAST_SPEED
	blapi_traffic_ipv6_mc_clear_sock_info(g_conf->rawsocket);
#endif
...
void timeout(void)
{
	...
	LIST_LOOP2(g_conf->groups, groupn, ln, ln2)
	{
		...
		LIST_LOOP2_END
#ifdef TCSUPPORT_IGMP_QUICK_LEAVE
			//list_delete_all_node(groupn->clients);
#endif
