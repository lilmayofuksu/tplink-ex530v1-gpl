			...
			*stream_name = NULL;
			...
			if (res == 0){
				...
				*stream_name = AT_UNNAMED;
				//COV, CID56606 No Local, CID56608 No Local, CID56618 No Local, CID56626 No Local, CID56629 No Local, 
				//COV, CID57048 No Local, CID57052 No Local, CID57053 No Local, CID57054 No Local, CID57057 No Local, 
			}
			return res;
		}
	} else
		...
		const char *target, struct fuse_file_info *fi)
{
	...
	char *dir_path;
	le32 securid;
	char *path = NULL; //COV, CID 56614
	ntfschar *stream_name;
	int stream_name_len = 0; //COV, CID 56616
...
exit:
	if (path)
		free(path); //COV, CID 56614
	if (stream_name_len > 0)
		free(stream_name); //COV, CID 56616
	free(uname);
