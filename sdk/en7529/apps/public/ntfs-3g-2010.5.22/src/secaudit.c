...
void showhex(FILE *fd)
{
	...
	memset(line, 0, sizeof(line));//COV, CID188380, No Local
	do {
...
BOOL restore(FILE *fd)
{
	...
	memset(line, 0, sizeof(line));//COV, CID188346, No Local
	do {
...
int audit_sds(BOOL second)
{
	...
	if (size != 20) {
		...
		do {
			...
			if ((entryalsz >= (MAXATTRSZ + 20)) || (size < (int)(entrysz - 20))) //COV, CID 187723
				done = TRUE;
			else {
				if (opt_v) {
...
int main(int argc, char *argv[])
{
	...
	warnings = 0;
	xarg = getoptions(argc,argv);
	argv[xarg][strlen(argv[xarg])] = '\0'; //COV, CID 188275
	if (xarg) {
		for (i=0; i<(MAXSECURID + (1 << SECBLKSZ) - 1)/(1 << SECBLKSZ); i++)
			securdata[i] = (struct SECURITY_DATA*)NULL;
#if POSIXACLS
