...
fuse_req_t req_g; //COV, CID59959, CID59954, CID59960
                            ...
                            struct fuse_file_info *fi)
{
    ...
    char *path;
    int err;
    int fuse_req_s = sizeof_fuse_req();
    ...
    if (!err) {
        ...
        pthread_mutex_unlock(&f->lock);
        req_g = (fuse_req_t)malloc(fuse_req_s); //COV, CID59959, No Local 
        memcpy(req_g, req, fuse_req_s); //COV, CID59959, No Local 
        if (fuse_reply_create(req, &e, fi) == -ENOENT) {
            /* The open syscall was interrupted, so it must be cancelled */
            fuse_prepare_interrupt(f, req_g, &d);  //COV, CID59959, No Local 
            fuse_do_release(f, e.ino, path, fi);
            fuse_finish_interrupt(f, req_g, &d);  //COV, CID59959, No Local 
            forget_node(f, e.ino, 1);
        } else if (f->conf.debug) {
            fprintf(stderr, "  CREATE[%llu] flags: 0x%x %s\n",
                    (unsigned long long) fi->fh, fi->flags, path);
        }
        free(req_g); //COV, CID59959, No Local 
    } else
        reply_err(req, err);
                          ...
                          struct fuse_file_info *fi)
{
    ...
    if (!err) {
        ...
        pthread_mutex_unlock(&f->lock);
        req_g = (fuse_req_t)malloc(fuse_req_s); //COV, CID59954, No Local 
        memcpy(req_g, req, fuse_req_s); //COV, CID59954, No Local 
        if (fuse_reply_open(req, fi) == -ENOENT) {
            /* The open syscall was interrupted, so it must be cancelled */
            fuse_prepare_interrupt(f, req_g, &d); //COV, CID59954, No Local 
            fuse_do_release(f, ino, path, fi);
            fuse_finish_interrupt(f, req_g, &d); //COV, CID59954, No Local 
        } else if (f->conf.debug) {
            fprintf(stderr, "OPEN[%llu] flags: 0x%x %s\n",
                    (unsigned long long) fi->fh, fi->flags, path);
        }
        free(req_g); //COV, CID59954, No Local 
    } else
        reply_err(req, err);
                       ...
                       struct fuse_file_info *llfi)
{
    ...
    if (!err) {
        req_g = (fuse_req_t)malloc(fuse_req_s); //COV, CID59960, No Local
        memcpy(req_g, req, fuse_req_s); //COV, CID59960, No Local
        if (fuse_reply_open(req, llfi) == -ENOENT) {
            /* The opendir syscall was interrupted, so it must be cancelled */
            fuse_prepare_interrupt(f, req_g, &d); //COV, CID59960, No Local
            fuse_fs_releasedir(f->fs, path, &fi);
            fuse_finish_interrupt(f, req_g, &d); //COV, CID59960, No Local
            pthread_mutex_destroy(&dh->lock);
            free(dh);
        }
        free(req_g); //COV, CID59960, No Local
    } else {
        reply_err(req, err);
        pthread_mutex_destroy(&dh->lock);
        free(dh);
