...
static void printosclassificationoutput(const struct
  ...
  const char *types[MAX_OS_CLASSMEMBERS] = {0};
  const char *cpes[MAX_OS_CLASSMEMBERS] = {0};
  char fullfamily[MAX_OS_CLASSMEMBERS][128] = {0};    /* "[vendor] [os family]" */
  double familyaccuracy[MAX_OS_CLASSMEMBERS] = {0};   /* highest accuracy for this fullfamily */
...
void printosscanoutput(Target *currenths) {
  ...
  if (currenths->seq.lastboot) {
    ...
    struct timeval tv;
    double uptime = 0;
    strncpy(tmbuf, ctime(&currenths->seq.lastboot), sizeof(tmbuf)-1);
    chomp(tmbuf);
