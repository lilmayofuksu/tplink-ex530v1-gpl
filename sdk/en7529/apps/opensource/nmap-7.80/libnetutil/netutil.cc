...
#ifdef IP_HDRINCL
  int one = 1, ret = 0;
  ret = setsockopt(sd, IPPROTO_IP, IP_HDRINCL, (const char *) &one, sizeof(one));
#endif
}
...
void set_ipoptions(int sd, void *opts, size_t optslen) {
  ...
  ret = setsockopt(sd, IPPROTO_IP, IP_OPTIONS, (const char *) opts, optslen);
#endif
}
...
void set_ttl(int sd, int ttl) {
  ...
  ret = setsockopt(sd, IPPROTO_IP, IP_TTL, (const char *) &ttl, sizeof ttl);
#endif
}
...
#if 0  /* DEAD CODE:  base LINE 2425,   Condition "datalen < 8U", taking false branch. Now the value of "datalen" is at least 8. */
...
icmpbad:
      ...
      /*else
        strncpy(icmpinfo,"type=?/code=?", sizeof(icmpinfo));*/  /* dead code */
