...
int EchoClient::nep_handshake(){
  ...
  u8 *data_n = h->getBinaryBuffer();
  nsock_write(this->nsp, this->nsi, write_done_handler, ECHO_WRITE_TIMEOUT, NULL, (char *)data_n, h->getLen());
  free(data_n);
...
int EchoClient::nep_send_packet_spec(){
  ...
  u8 *data_n = h->getBinaryBuffer();
  nsock_write(this->nsp, this->nsi, write_done_handler, ECHO_WRITE_TIMEOUT, NULL, (const char*)data_n,  h->getLen());
  free(data_n);
...
int EchoClient::nep_recv_echo(u8 *packet, size_t packetlen){
  ...
  EchoHeader *pkt_in = new EchoHeader();
