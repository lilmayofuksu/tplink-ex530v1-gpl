...
int resolveCached(char *host, struct sockaddr_storage *ss, size_t *sslen, int pf) {
  ...
  if( (result=resolve(host, 0, ss, sslen, pf)) == 0 ){
	  ...
	  archive[current_index].hostname[MAX_CACHED_HOSTNAME_LEN - 1] = 0;
...
struct hostent *hostentcpy(struct hostent *src){
  ...
  if(  src->h_addr_list != NULL ){
      ...
      /*st->h_addr=st->h_addr_list[0];*/ /*Self assignment */
  }
  return st;
} /* End of hostentcpy() */
...
int send_packet(NpingTarget *target, int rawfd, u8 *pkt, size_t pktLen){
  ...
  if ( o.sendEth() ){
    ...
    size_rt = eth_send(ethsd, pkt, pktLen);
  }else{
    if( o.ipv6() ){ /* IPv6 */
...
/** This function parses Linux file /proc/net/if_inet6 and returns a list
 ...
 2. Netlink device number (interface index) in hexadecimal (see â€œip addrâ€?, too)
 3. Prefix length in hexadecimal
 4. Scope value (see kernel source â€?include/net/ipv6.hâ€?and â€œnet/ipv6/addrconf.câ€?for more)
 5. Interface flags (see â€œinclude/linux/rtnetlink.hâ€?and â€œnet/ipv6/addrconf.câ€?for more)
 6. Device name
