...
int ProbeMode::start(){
  ...
  u8 *pkt = new u8[MAX_IP_PACKET_LEN];       /**< Holds packets returned by fillpacket */
  ...
  sendpkt_t *pkts2send = NULL;     /**< We have a race condition here but the
  ...
  * nmap-dev mailing list. */
  ...
  pkts2send = (sendpkt_t *)(malloc(MX_PKT * sizeof(sendpkt_t)));
  if ( NULL == pkts2send )
  {
	delete []pkt;
	return OP_FAILURE;
        ...
        case UDP:
            ...
            for( c=0; c < o.getPacketCount(); c++){
                ...
                for (p=0; p < numTargetPorts; p++){
                    ...
                    while( (target=o.targets.getNextTarget()) != NULL && targetPorts ){
 ...
 * takes care of that already. */
  if(o.sendEth() && o.getMode()!=ARP){
    ...
    /*if( buff==NULL || filledlen==NULL)
        nping_fatal(QT_3,"fillPacketARP(): NULL pointer supplied.");*/  /* DEAD CODE: LINE 573, will return IF NULL.*/
  ...
  if( o.spoofSource() )
    ...
    * the target, set localhost address. */
    if ( o.ipv6() ){
        ...
        rt_code = inet_pton(AF_INET6, "::1", &s6->sin6_addr);
    }else{
        s4->sin_family=AF_INET;
        rt_code = inet_pton(AF_INET, "127.0.0.1", &s4->sin_addr);
    }
...
#define BUFF_SIZE 65535
 char *final_output = new char[BUFF_SIZE];
 ...
 if ( NULL == final_output )
 	return;

 memset(final_output, 0, BUFF_SIZE);
       ...
       case NSE_TYPE_PCAP_READ:
            ...
            if(ip){
                ...
                if (proto == IPPROTO_UDP || proto == IPPROTO_TCP){
                    ...
                    opened */
                    snprintf(final_output, BUFF_SIZE, "RCVD (%.4fs) %s\n", o.stats.elapsedRuntime(t), buffer);
                    if( o.getVerbosity() >= VB_3 ){
                        hex=hexdump(packet, packetlen);
                        strncat(final_output, hex, BUFF_SIZE-1);
                        free(hex);
                    }
                    prevtime=pcaptime;
