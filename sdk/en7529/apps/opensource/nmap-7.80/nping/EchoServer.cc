...
int EchoServer::nep_capture_handler(nsock_pool nsp, nsock_event nse, void *param){
  ...
  NEPContext *ctx=NULL;
  EchoHeader *pkt_out = new EchoHeader();
  size_t linklen=0;
  size_t packetlen=0;
  handler_arg_t arg;
  arg.me=this;
  ...
  if( ctx->ready() ){
	  ...
	  u8 *data_n = pkt_out->getBinaryBuffer();
      nsock_write(nsp, clnt_iod, echo_handler, NSOCK_INFINITE, NULL, (const char *)data_n, pkt_out->getLen());
	  free(data_n);
...
int EchoServer::nep_hs_client_handler(nsock_pool nsp, nsock_event nse, void *param){
  ...
  u8 *data_n = pkt_out->getBinaryBuffer();
  nsock_write(nsp, nsi, hs_final_handler, NSOCK_INFINITE, NULL, (const char *)data_n, pkt_out->getLen());
  free(data_n);
  delete pkt_out;
  return OP_SUCCESS;
...
int EchoServer::nep_packetspec_handler(nsock_pool nsp, nsock_event nse, void *param){
  ...
  u8 *data_n = pkt_out->getBinaryBuffer();
  nsock_write(nsp, nsi, ready_handler, NSOCK_INFINITE, NULL, (const char *)data_n, pkt_out->getLen());
  free(data_n);
