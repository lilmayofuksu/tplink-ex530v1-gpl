...
static void initialize_idleproxy(struct idle_proxy_info *proxy, char *proxyName,
  ...
  if (q) {
    ...
    if (ports->syn_ping_count > 0) {
      ...
      getpts_simple(DEFAULT_TCP_PROBE_PORT_SPEC, SCAN_TCP_PORT, &ports_o, &count);
      assert(count > 0);
      proxy->probe_port = ports_o[0];
      free(ports_o);
    }
  }
  ...
  if ((o.sendpref & PACKET_SEND_ETH) && (proxy->host.ifType() == devt_ethernet
    ...
    rtcode = unblock_socket(proxy->rawsd);
...
static void adjust_idle_timing(struct idle_proxy_info *proxy,
  ...
  if (testcount < realcount) {
    ...
    proxy->current_groupsz = MAX(proxy->min_groupsz, MIN(proxy->current_groupsz, (double)500000 / (proxy->senddelay + 1)));
    ...
    if (proxy->senddelay < 500)
      proxy->senddelay = 0;
    proxy->current_groupsz = MIN(proxy->current_groupsz * 1.1, (double)500000 / (proxy->senddelay + 1));
  ...
  if (totalfound != expectedopen) {
    if (deepcount1 == -1) {
      ...
      if (retrycount != flatcount1) {
        ...
        flatcount1 = retrycount; /*  flatcount1 == retrycount ==> following code condition will be dead code. */
    ...
    if (deepcount2 == -1) {
      ...
      if (retrycount != flatcount2) {
        ...
        flatcount2 = retrycount;  /*  flatcount2 == retrycount ==> following code condition will be dead code. */
