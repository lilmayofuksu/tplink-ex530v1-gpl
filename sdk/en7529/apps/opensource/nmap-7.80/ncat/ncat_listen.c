...
/* $Id: ncat_listen.c 37640 2019-05-28 21:36:04Z dmiller $ */
...
/* Not sure if this problem exists on Windows, but fcntl and /dev/null don't */
    ...
    for (i = 0; i < num_listenaddrs; i++) {
         ...
         */
        ret = unblock_socket(listen_socket[num_sockets]);
...
static void handle_connection(int socket_accept)
{
    ...
    if (s.fd < 0) {
        ...
        /*close(s.fd);*/ /* s.fd is passed to a parameter that cannot be negative. */
        return;
    }
...
static int ncat_listen_dgram(int proto)
{
    ...
    char *buf = (char *)malloc(DEFAULT_UDP_BUF_LEN);
    char *tempbuf = NULL;
    fd_set read_fds;
	...
	if ( NULL == buf )
		return 1;
	memset(buf, 0, DEFAULT_UDP_BUF_LEN);
...
/* Not sure if this problem exists on Windows, but fcntl and /dev/null don't */
    ...
    while (1) {
        ...
        while (1) {
             ...
             */
            nbytes = recvfrom(socket_n, buf, DEFAULT_UDP_BUF_LEN, MSG_PEEK,
            ...
            nbytes = recv(socket_n, buf, DEFAULT_UDP_BUF_LEN, 0);
            if (nbytes < 0) {
                loguser("%s.\n", socket_strerror(socket_errno()));
                close(socket_n);
				free(buf);
        ...
        while (1) {
            ...
            if (FD_ISSET(STDIN_FILENO, &fds)) {
                nbytes = Read(STDIN_FILENO, buf, DEFAULT_UDP_BUF_LEN);
                if (nbytes <= 0) {
                    if (nbytes < 0 && o.verbose) {
                        logdebug("Error reading from stdin: %s\n", strerror(errno));
...
static int chat_announce_connect(int fd, const union sockaddr_u *su)
{
    ...
    for (i = 0; i <= client_fdlist.fdmax; i++) {
        union sockaddr_u su_l;
        socklen_t len = sizeof(su_l.storage);

        if (i == fd || !FD_ISSET(i, &master_broadcastfds))
            continue;

        if (getpeername(i, &su_l.sockaddr, &len) == -1)
            bye("getpeername for sd %d failed: %s.", i, strerror(errno));

        if (count > 0)
            strbuf_sprintf(&buf, &size, &offset, ", ");

        strbuf_sprintf(&buf, &size, &offset, "%s as <user%d>", inet_socktop(&su_l), i);
