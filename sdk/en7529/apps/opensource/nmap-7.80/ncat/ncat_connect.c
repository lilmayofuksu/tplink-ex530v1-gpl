...
/* $Id: ncat_connect.c 37640 2019-05-28 21:36:04Z dmiller $ */
        ...
        if (o.af == AF_UNIX && o.proto == IPPROTO_UDP)
        {
            if (srcaddr.storage.ss_family != AF_UNIX) {
              ...
              strbuf_sprintf(&tmp_name, &size, &offset, "%s/ncat.XXXXXX",
                  tmpdir ? tmpdir : "/tmp");
              if (tmp_name && mkstemp(tmp_name) == -1) {
              ...
              unlink(tmp_name);
                ...
                strncpy(srcaddr.un.sun_path, tmp_name, sizeof(srcaddr.un.sun_path)-1);
                free (tmp_name);
            }
