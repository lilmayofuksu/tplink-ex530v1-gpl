...
struct model *load_model(const char *model_file_name)
{
	...
	memset(model_, 0, sizeof(model));
	model_->label = NULL;

	char cmd[81];
	while(1)
	{
		ret1 = fscanf(fp,"%80s",cmd);
		if(strcmp(cmd,"solver_type")==0)
		{
			ret2 = fscanf(fp,"%80s",cmd);
			int i;
		...
		else if(strcmp(cmd,"label")==0)
		{
			...
			for(int i=0;i<nr_class;i++)
				ret2 = fscanf(fp,"%d",&model_->label[i]);
		}
		else
		{
	...
	for(i=0; i<w_size; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
			ret3 = fscanf(fp, "%lf ", &model_->w[i*nr_w+j]);
		ret3 = fscanf(fp, "\n");
	}
	if (ferror(fp) != 0)
	{
