...
static int
coap_is_bcast(const coap_address_t *obj)
{
	...
	switch ( obj->addr.sa.sa_family )
	{
		case AF_INET:
			h_addr = ntohl(obj->addr.sin.sin_addr.s_addr);
			p = (unsigned char *)&h_addr; /* check last byte */
			return (*p == 0xff) ? 1 : 0;
		default:
			break;
	}
...
coap_socket_connect_udp(coap_socket_t *sock,
  ...
  sock->flags &= ~(COAP_SOCKET_CONNECTED | COAP_SOCKET_MULTICAST | COAP_SOCKET_BROADCAST);
  ...
  if ( is_bcast
  	&& AF_INET == connect_addr.addr.sa.sa_family
  	&& COAP_DEFAULT_PORT == ntohs(connect_addr.addr.sin.sin_port) ) /* ONLY add for unsecure IPv4 broadcast. */
  {
	if ( COAP_SOCKET_ERROR == setsockopt(sock->fd, SOL_SOCKET, SO_BROADCAST, OPTVAL_T(&on), sizeof(on)) )
		coap_log(LOG_WARNING,
...
#else
	if ( sock->flags & COAP_SOCKET_BROADCAST )
	{
		memcpy(&bcast_addr, &session->remote_addr.addr, sizeof(struct sockaddr_in));
		bytes_written = sendto(sock->fd, data, datalen, 0, (struct sockaddr*)&bcast_addr, sizeof(bcast_addr));
	}
	else
...
coap_network_read(coap_socket_t *sock, coap_packet_t *packet) {
  ...
  if (sock->flags & COAP_SOCKET_CONNECTED) {
	...
	if ( sock->flags & COAP_SOCKET_BROADCAST )
	{
		len = recvfrom(sock->fd, packet->payload, COAP_RXBUFFER_SIZE, 0, (struct sockaddr*)&peer_addr, &addr_len);
		memcpy(&packet->src.addr, &peer_addr, sizeof(packet->src.addr));
	}
	else
