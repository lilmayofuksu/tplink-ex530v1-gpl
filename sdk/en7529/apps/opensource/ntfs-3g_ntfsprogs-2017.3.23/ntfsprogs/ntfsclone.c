...
{
	static char buff[NTFS_MAX_CLUSTER_SIZE]; /* overflow checked at mount time */
	...
	memset(buff, 0, NTFS_MAX_CLUSTER_SIZE);
	if (!opt.restore_image) {
...
{
	s64 i;
	static char buff[NTFS_MAX_CLUSTER_SIZE];
...
static void wipe_mft(char *mrec, u32 mrecsz, u64 mft_no)
{
	...
	ret1 = ntfs_mft_usn_dec((MFT_RECORD*)mrec);
	ret2 = ntfs_mst_pre_write_fixup((NTFS_RECORD*)mrec,mrecsz);
}
