...
static void parse_layer7_protocol(const char *s, struct xt_layer7_info *info)
{
	...
	res = pre_process(info->pattern);
	if(res != NULL){
		strncpy(info->pattern, res, MAX_PATTERN_LEN - 1);
		free(res);
	}

	
	for(i = 1; i < MAX_SUBDIRS; i++)
		if(subdirs[i] != NULL)
			free(subdirs[i]);
      ...
      const void *entry, struct xt_entry_match **match)
{
	...
	case 'd':
		...
		strncpy(l7dir, argv[optind-1], sizeof(l7dir) - 1);
