      ...
      switch (*p)
        ...
        case 'b':
          ...
          if (hquery->want_family == AF_INET6) 
		 {
          ...
          else if(hquery->want_family == AF_INET){
  ...
  if (status == ARES_SUCCESS)
    {
		...
		{
			reply_status = ares_parse_reply(abuf, alen, &rr_type);
			if(reply_status != ARES_SUCCESS){
				end_hquery(hquery, status, NULL);
				return;
			}
			...
			}else if(rr_type == T_AAAA){
				status = ares_parse_aaaa_reply(abuf, alen, &host, NULL, NULL);
				if (host && channel->nsort)
					sort6_addresses(host, channel->sortlist, channel->nsort);
			}
		}
		else if (hquery->sent_family == AF_INET)
        {
          status = ares_parse_a_reply(abuf, alen, &host, NULL, NULL);
          if (host && channel->nsort)
            sort_addresses(host, channel->sortlist, channel->nsort);
      ...
      end_hquery(hquery, status, host);
		...
		status == ARES_ETIMEOUT) && hquery->want_family == AF_UNSPEC)
    {
		end_hquery(hquery, status, NULL);
    }
                       ...
                       struct hostent *host)
{
  ...
  if(hquery->flag == 1)
  	return; /*if want_family is AF_UNSPEC and first enter, skip ares_free()*/
