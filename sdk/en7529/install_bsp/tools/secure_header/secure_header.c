...
int main(int argc, char **argv) 
{
	...
	char signature_file[128] = {0};
	char signature_source_file[128] = {0};
	char rsa_cipher_file[128] = {0};
	char spram_cipher_file[128] = {0};
	char lzma_cipher_file[128] = {0};
	char verify_cipher_file[128] = {0};
	char bootram_cipher_file[128] = {0};
	char aes_img_cipher_file[128] = {0};
	char *buf = NULL;
	unsigned long fill_addr = 0;
    unsigned long i2c_flash_magic=0;
    int is_i2c_flash_magic=0;
	...
	while ((c = getopt(argc, argv, "i:a:s:k:v:d:l:m:e:y:b:c:")) != -1) {
		switch (c) {
			/* input file */
			case 'i':
			...
			case 'a':
				...
				isAddr = 1;
				break;
            /* i2c or flash otp magic */
			case 'c':
				if(parseNumber(optarg, 16, &i2c_flash_magic) == -1) {
					goto free_source;
				}
				is_i2c_flash_magic = 1;
				break;
...
//#if defined(TCSUPPORT_SECURE_BOOT_V2)
	...
	if(Ver == 2) {
        if(is_i2c_flash_magic== 1) {
            sh->v2.i2c_flash_magic = STORE32_LE(i2c_flash_magic);
        }
