...
static int obj_id = YAFFS_NOBJECT_BUCKETS + 1; //for start chun_id
...
static void shuffle_oob(char *spareData, struct yaffs_packed_tags2 *pt)
{
	...
	memset(&(pt->ecc), 0xff , sizeof(struct yaffs_ecc_other));
	//printf("shuffle_oob:%02X,%02x,%02x,%02x!\n ",sizeof(struct yaffs_ecc_other), pt->ecc.col_parity,pt->ecc.line_parity, pt->ecc.line_parity_prime);

	pt->t.seq_number = SWAP32(pt->t.seq_number);
	pt->t.obj_id = SWAP32(pt->t.obj_id);
	pt->t.chunk_id = SWAP32(pt->t.chunk_id);
	pt->t.n_bytes = SWAP32(pt->t.n_bytes);

	p = &(pt->t);

	//printf("shuffle_oob:oob_layout=%d\n",oob_layout);
	if(oob_layout == 0){		
		for(i=0; i< sizeof(struct yaffs_packed_tags2_tags_only); i++){
			spareData[i]  = p[i];	
		}
	}else{
		j=4;
	...
	//printf("pt:%04X,%04X,%04X \n", sizeof(struct yaffs_packed_tags2),sizeof(struct yaffs_packed_tags2_tags_only),sizeof(struct yaffs_ecc_other));
}
...
//	t.serial_number = 0;
	...
	//printf("oob:\t%04X %04X %04X %04X\n", chunk_id, n_bytes, id, t.seq_number);
...
// added NCB **CHECK**
	...
	if (write(outFile,spareData,sizeof(spareData)) != sizeof(spareData))
		fatal("write");
#if 0
	printf("oob:\t%04X %04X %04X %04X\n", pt.t.seq_number, pt.t.obj_id, pt.t.chunk_id, pt.t.n_bytes);
	for(i = 0; i < spareSize; i++){
		if(!(i % 16)) printf("\n%04x\t",i);
		printf("%02X ", (unsigned char)spareData[i]);
	}
	printf("\n");
#endif	
	return 0;
...
// NCB 12/9/02		oh->yst_gid = s->yst_uid;
	...
	{
		oh->file_size_low = 0 ;//s->st_size;
		oh->file_size_high = 0 ;//( (s->st_size & 0xfffff0000)>> 32);
	}
...
int main(int argc, char *argv[])
{
	...
	if(argc < 6)
	{
		...
		printf("		   image_file the output file to hold the image\n");
		printf("		   'convert'  produce a big-endian image from a little-endian machine\n");
		printf("		   oob_layout	0:no padding\n");
		printf("		   pagesize 	0:default 2KB.\n");
		printf("		   oobsize		0:default 64KB\n");
	...
	{
		convert_endian = 1;
		oob_layout = atoi(argv[4]);
	}else{
		oob_layout = atoi(argv[3]);
	}
    }
