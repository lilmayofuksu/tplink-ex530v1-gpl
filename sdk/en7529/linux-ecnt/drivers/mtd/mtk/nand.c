...
static void fix_ecc_0(struct mtd_info *mtd, int page, int orig_page)
{
	struct nand_chip *chip = mtd->priv;
	u8 *block_buf = NULL;
	u8 *data_buf = NULL;
	...
	// erase the block, use non-mapped block becasue erase_func will check the bmt-mapping by itself.
	status = mt6573_nand_erase_hw(mtd, ((orig_page >> page_per_block_shift) << page_per_block_shift));
	if (status & NAND_STATUS_FAIL)
	{
		mt6573_nand_block_markbad_hw(mtd, (start_page << chip->page_shift), 0);
...
#endif
{
	...
	{
		fix_ecc_0(mtd, (page_in_block + (mapped_block * page_per_block)), page);
	}
#endif
