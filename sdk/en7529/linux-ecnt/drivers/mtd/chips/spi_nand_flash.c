...
extern SPI_NAND_FLASH_RTN_T parallel_nand_dma_read(u32 read_addr, u32 page_number, u32 *p_data);
extern SPI_NAND_FLASH_RTN_T parallel_nand_dma_write(u32 write_addr, u32 page_number, u32 *p_data, u32 oob_len, u8 *ptr_oob);
...
static SPI_NAND_FLASH_RTN_T spi_nand_protocol_read_id ( struct SPI_NAND_FLASH_INFO_T *ptr_rtn_flash_id )
{
	...
	_SPI_NAND_READ_NBYTE( (u8 *)&(ptr_rtn_flash_id->dev_id2), _SPI_NAND_LEN_ONE_BYTE, SPI_CONTROLLER_SPEED_SINGLE);
...
static SPI_NAND_FLASH_RTN_T spi_nand_read_page (u32 page_number, SPI_NAND_FLASH_READ_SPEED_MODE_T speed_mode)
{
	...
	if(_current_page_num != page_number) {
		...
		if(_spi_dma_mode == SPI_DMA_MODE_ENABLE) {	
			...
			if (_parallel_nand_mode)
			{
				rtn_status = parallel_nand_dma_read(read_addr, page_number, (u32 *)rdma_addr);
			}
			else
#endif
			{
				nfi_status = SPI_NFI_Read_SPI_NAND_Page(dma_speed_mode, read_cmd, read_addr, (u32 *)rdma_addr);
...
static SPI_NAND_FLASH_RTN_T spi_nand_dma_program_load(u32 addr, u32 oob_len, SPI_NAND_FLASH_WRITE_SPEED_MODE_T speed_mode)
{
	...
	rtn_status = SPI_NFI_Write_SPI_NAND_page(dma_speed_mode, write_cmd, write_addr, (u32 *)wdma_addr);
											    ...
											    SPI_NAND_FLASH_WRITE_SPEED_MODE_T speed_mode)
{
	...
	if(erase_write_flag == SPI_NAND_FLASH_ERASE_WRITE_LOG_ENABLE) {
		b_write_cnt_per_erase[block_idx % MAX_BLOCK]++;
		b_write_total_cnt[block_idx % MAX_BLOCK]++;
	}
#endif
	...
	if(multi_write_dbg_flag == SPI_NAND_FLASH_MULTI_WRITE_ENABLE) {
		block_idx = page_number / PAGE_CNT_PER_BLOCK;
		if(page_info[block_idx % MAX_BLOCK][page_number % PAGE_CNT_PER_BLOCK].isProgramed != 0) {
			...
			//return SPI_NAND_FLASH_RTN_NO_ERROR;
			if(page_info[block_idx % MAX_BLOCK][page_number % PAGE_CNT_PER_BLOCK].illegal_program < 0x7F) {
				page_info[block_idx % MAX_BLOCK][page_number % PAGE_CNT_PER_BLOCK].illegal_program++;
			}
		}
		page_info[block_idx % MAX_BLOCK][page_number % PAGE_CNT_PER_BLOCK].isProgramed = 1;
	}
#endif
	...
	if (_parallel_nand_mode)
	{
		rtn_status = parallel_nand_dma_write(write_addr, page_number, (u32 *)wdma_addr, oob_len, &_current_cache_page_oob_mapping[0]);
	}
	else
#endif
	{
...
SPI_NAND_FLASH_RTN_T spi_nand_erase_block(u32 block_index)
{
...
#ifdef ERASE_WRITE_CNT_LOG
	if(erase_write_flag == SPI_NAND_FLASH_ERASE_WRITE_LOG_ENABLE) {
		b_erase_cnt[block_index % MAX_BLOCK]++;
		b_write_cnt_per_erase[block_index % MAX_BLOCK] = 0;
	}
#endif
	...
	if(multi_write_dbg_flag == SPI_NAND_FLASH_MULTI_WRITE_ENABLE) {
		for(i = 0; i < 128; i++) {
			page_info[block_index % MAX_BLOCK][i].isProgramed = 0;
		}
	}
#endif
				...
				SPI_ECC_DECODE_CONF_T *decode_conf_t)
{
	...
	/* config ECC */
	if(spi_nfi_conf_t->hw_ecc_t == SPI_NFI_CON_HW_ECC_Enable) {
		if(isEN751627|| isEN7580|| isEN7523) {
...
#if LINUX_VERSION_CODE  >= KERNEL_VERSION(5,4,0)
static struct nand_ecclayout_user spinand_oob_64 = {
#else
...
spinand_write_page_hwecc(struct nand_chip *chip, const uint8_t *buf, int oob_required, int page)
{
    struct mtd_info *mtd = spi_nand_mtd;
#else
spinand_write_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf, int oob_required)
{
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
static void spinand_select_chip(struct mtd_chip *chip, int cs)
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
static int spinand_dev_ready(struct mtd_chip *chip)
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
static int spinand_correct_data(struct nand_chip *chip, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
#else
static int spinand_correct_data(struct mtd_info *mtd, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
#endif
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
    chip->ecc.strength		= 1; /* Driver must set ecc.strength when using hardware ECC */
    /* kernel has nand_read_page_hwecc and nand_write_page_hwecc in nand_base.c */
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
     if (nand_scan_with_ids(chip, 1, spi_nand_flash_ids) == 0) {
	...
	int count, int *eof, void *data)
{
	...
	len = sprintf(page, "flash size=[%lu], SPI NAND DEBUG LEVEL=%d, _SPI_NAND_WRITE_FAIL_TEST_FLAG=%d, _SPI_NAND_ERASE_FAIL_TEST_FLAG=%d\n"
