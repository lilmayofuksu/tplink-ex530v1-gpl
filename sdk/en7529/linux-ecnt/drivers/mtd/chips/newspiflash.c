...
#ifdef TCSUPPORT_CPU_ARMV8
#define SNOR_WRL(reg, data)	(cb_set_spi_ctrl_data(reg, data))
#define SNOR_RDL(reg)		(cb_get_spi_ctrl_data(reg))
#else
#define SNOR_WRL(reg, data)	(VPint(reg) = data)
#define SNOR_RDL(reg)		(VPint(reg))
#endif
...
/* Kernel version */
#define KERNEL_4_4_115 		(LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,115))
...
#ifdef TCSUPPORT_CPU_ARMV8
typedef u32 (*cb_get_func_t)(u32);
typedef void (*cb_set_func_t)(u32, u32);

static cb_get_func_t cb_get_spi_ctrl_data = NULL;
static cb_set_func_t cb_set_spi_ctrl_data = NULL;

static const struct of_device_id ecnt_snor_of_ids[] = {
	{ .compatible = "econet,ecnt-snor"},
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, ecnt_snor_of_ids);
#endif
...
static struct mtd_info *spiflash_probe(struct map_info *map)
{
	...
	if (!mtd) {
		...
		kfree(chip_info); //COV, CID 139948
		return NULL;
	}
...
static int write_test(void *arg)
{
	...
	#else
	printk("write_test: run at cpu:%d\n", cpu);
	#endif
...
static int __init spiflash_probe_init(void)
{
	...
	SNOR_WRL(SF_MACMUX_SEL, MANUAL_MACMUXSEL);
	...
	spi_set_clock_speed(spi_controller_clk);
    #else
	SNOR_WRL(SF_CLK_CHANGE, 6);
    #endif
	// set Tn to 1
	SNOR_WRL(SF_SI_CK_SEL, 9);

	printk("\nSPI Flash Init Finish\n");

	SNOR_WRL(SF_INTERRUPT_EN, SNOR_RDL(SF_INTERRUPT_EN) | AUTO_MANUAL_INTR_EN);
...
/*******************For ARM*******************/
#ifdef TCSUPPORT_CPU_ARMV8
void register_spi_ctrl_data_cb_funcs(cb_get_func_t get_cb_func, cb_set_func_t set_cb_func)
{
    if (cb_get_spi_ctrl_data==NULL)
        cb_get_spi_ctrl_data = get_cb_func;
    if (cb_set_spi_ctrl_data==NULL)
        cb_set_spi_ctrl_data = set_cb_func;
}

static int ecnt_snor_drv_probe(struct platform_device *pdev)
{
	int ret = 0;
...
static int ecnt_snor_drv_remove(struct platform_device *pdev)
{
	return 0;
}

static struct platform_driver ecnt_snor_driver = {
	.probe = ecnt_snor_drv_probe,
	.remove = ecnt_snor_drv_remove,
	.driver = {
		.name = "ecnt-snor",
		//.pm = MTK_NOR_DEV_PM_OPS,
		.of_match_table = ecnt_snor_of_ids,
	},
};
module_platform_driver(ecnt_snor_driver);
MODULE_DESCRIPTION("EcoNet SPI NOR Flash Driver");
#endif
