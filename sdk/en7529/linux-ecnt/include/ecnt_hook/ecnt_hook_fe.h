...
static inline int FE_API_GET_FlOW_PKT_CNT(unchar idx, FE_Dir_t dir, FE_ACNT_Mode_t mode, uint *pkt_cnt)
{
    struct ecnt_fe_data in_data;
	int ret = 0;
    memset(&in_data, 0, sizeof(struct ecnt_fe_data)) ;
...
static inline int FE_API_SET_GDMA_RELEASE_MODE(FE_Gdma_Sel_t _gdm_sel, FE_RLS_MODE_t mode)
{
	struct ecnt_fe_data in_data;
	int ret = 0;
	 
	in_data.function_id = FE_SET_GDMA_RELEASE_MODE;
	in_data.gdm_sel = _gdm_sel;
	in_data.api_data.fe_rls_mode = mode;
	ret = __ECNT_HOOK(ECNT_FE, ECNT_FE_API, (struct ecnt_data *)&in_data);
	if(ret != ECNT_HOOK_ERROR)
		return in_data.retValue;
	else
...
static inline int FE_API_SET_DYN_QDMA_WAN_TXQ_CNGST_ENABLE(FE_Enable_t enable)
{
	struct ecnt_fe_data in_data;
	int ret = 0;
	memset(&in_data, 0, sizeof(struct ecnt_fe_data)) ;
	in_data.function_id = FE_SET_QDMAWAN_TXQ_CNGST_ENABLE;
	in_data.api_data.enable = enable;
	ret = __ECNT_HOOK(ECNT_FE, ECNT_FE_API, (struct ecnt_data *)&in_data);
	if(ret != ECNT_HOOK_ERROR)
		return in_data.retValue;
