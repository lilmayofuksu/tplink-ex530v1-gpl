...
static inline int ECNT_PPTP_GENERIC_HOOK(int subtype, struct net_data_s *data)
{	
	int ret = ECNT_CONTINUE;
	
	if (!list_empty(&ecnt_hooks[ECNT_NET_PPTP][subtype]))
	{

		ret = __ECNT_HOOK(ECNT_NET_PPTP, subtype, (struct ecnt_data *)data);
	}
