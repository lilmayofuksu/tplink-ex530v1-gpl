...
EXPORT_SYMBOL(get_chip_scu_data);
...
EXPORT_SYMBOL(set_chip_scu_data);
...
{
	return get_chip_scu_data(CR_NP_SCU_RSTCTRL1);
}
EXPORT_SYMBOL(GET_SCU_RST_CTRL_1);

void SET_SCU_RST_CTRL_1(u32 val)
{
	set_chip_scu_data(CR_NP_SCU_RSTCTRL1, val);
}
...
static void pbus_timeout_init(void)
{
	if (GET_IS_FPGA()) 
	{ /* FPGA bus_clk: 64MHz */
		set_np_scu_data(CR_NP_SCU_MON_TMR, 0x4000000); /* unit: one bus clock */
		set_np_scu_data(CR_NP_SCU_MON_TMR, 0x44000000); /* enable it */
	}else 
	{ /* ASIC bus_clk: 235MHz */
		set_np_scu_data(CR_NP_SCU_MON_TMR, 0x10000000); 
		set_np_scu_data(CR_NP_SCU_MON_TMR, 0x50000000);
	}
}
