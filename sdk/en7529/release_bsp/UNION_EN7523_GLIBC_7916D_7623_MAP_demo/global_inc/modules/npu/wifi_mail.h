...
static inline int WIFI_MAIL_API_SET_WAIT_DRAM_BA_ENTRY_ADDR(unsigned int interfaceID, unsigned int dramBaEntry_pa)
{
	struct WIFI_MAIL_Data in_data;
	npuMboxInfo_t mboxInfo;
	struct device *dev=NULL;
	...
	in_data.interfaceID = interfaceID;
	in_data.funcType = SET_WAIT;
	in_data.funcId = WIFI_MAIL_FUNCTION_SET_WAIT_DRAM_BA_ENTRY_ADDR;
	in_data.wifi_mail_private.dramBaEntryAddr = dramBaEntry_pa;
...
static inline int WIFI_MAIL_API_GET_WAIT_RXDESC_BASE(unsigned int interfaceID, npu_info_t *pnpuInfo)
{
	...
	va =  (unsigned long)dma_alloc_coherent(dev, sizeof(WIFI_MAIL_Data_t), (void *)&pa, GFP_KERNEL);
#endif
	if (!va)  {
...
static inline int WIFI_MAIL_API_GET_WAIT_COUNTER(unsigned int interfaceID, counter_t *pcount)
{
	...
	if (jiffies < last_time + 20) {//The interval for using this api must be greater than 200*10=2000ms
		return -1;
	}
#endif
...
//	va =  dma_alloc_coherent(NULL, sizeof(WIFI_MAIL_Data_t), &pa, GFP_KERNEL);
	...
	pdbgCount->errCount = in_data->wifi_mail_private.dbgCount.errCount;
//	dma_free_coherent(NULL, sizeof(WIFI_MAIL_Data_t), va, pa);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0) 
	dma_free_coherent(NULL, sizeof(WIFI_MAIL_Data_t), (void *)va, pa);
#else
	dma_free_coherent(dev, sizeof(WIFI_MAIL_Data_t), (void *)va, pa);
