...
int fill_buffer(char *buffer, char filename[][MAX_FILENAME_LEN], char *vmlinux)
{
	...
	/* \B6\D4\C6\EB\B2\D9\D7\F7\A3\AC\B1\C8\C8\E7֮ǰnΪ816645\A3\AC\B2\D9\D7\F7֮\BA\F3n\B1\E4Ϊ816648 */
	if (n & (ROUND-1)) {
	...
	if (fs_len > (l_flash_struct.miscOffset - l_flash_struct.rootfsOffset))
	{
	...
	if(strlen(filename[CONFIG]))
	{
		...
		if (config_len > l_misc_size) /* will it bigger?*/
		{
	...
	boot_tag->kernelAddress = STORE32_LE(0x0);
	boot_tag->kernelLen = STORE32_LE(kernel_len);

	boot_tag->rootfsAddress = STORE32_LE(kernel_len);
    ...
    int up_boot_image_len 
    	= l_flash_struct.rootfsOffset + BOOTLOADER_TAG_LEN;
...
int main(int argc, char **argv)
{
	...
	if (!(buf = malloc(l_flash_size + BOOTLOADER_TAG_LEN))) 
	{
		fprintf(stderr, "malloc failed\n");
		return EXIT_FAILURE;
	}
	memset(buf, 0xFF, l_flash_size + BOOTLOADER_TAG_LEN);
	...
	if (l_boot_size == 0)
	{
		...
		return EXIT_FAILURE;
#else
		l_boot_size = l_flash_size - l_misc_size;
#endif
	}
	...
	l_flash_struct.miscOffset = l_flash_size - l_misc_size;
	...
	fprintf(stderr, "miscOffset   is %x\n", l_flash_struct.miscOffset);
	fprintf(stderr, "=====================================================\n\n");
	/* end added */
	...
	if (fill_buffer(buf, filename, vmlinux_name) != EXIT_SUCCESS)
	{
		return EXIT_FAILURE;
	}
	...
	{
		sprintf(image_name_prefix_full, "%s/%s(%s)v%d",
					image_path, 
					(prefix[0] == 0) ? l_dev_info.model_name : prefix, 
					l_dev_info.build_spec,
					strtoul(l_dev_info.dev_ver, 0, 0));
	}
	else if(customized_name[0] != '\0')
	{
	...
	/* create image */
	sprintf(out_filename, "%s_FLASH.bin", image_name_prefix);
	...
	sprintf(out_filename, "%s_flash(%s)%s.bin", image_name_prefix_full, l_dev_info.build_date, image_name_suffix);
	if (!(out = fopen(out_filename, "w"))) {
		fprintf(stderr, "can not open \"%s\" for writing\n", out_filename);
		usage();
	}
	...
	sprintf(out_filename, "%s_UP%s.bin", image_name_prefix, image_name_suffix);
	if (!(out = fopen(out_filename, "w"))) {
		fprintf(stderr, "can not open \"%s\" for writing\n", out_filename);
		usage();
	}

	if (!fwrite(buf + l_flash_struct.kernelOffset + BOOTLOADER_TAG_LEN, l_flash_struct.appSize, 1, out) || fflush(out)) {
		fprintf(stderr, "fwrite failed\n");
		return EXIT_FAILURE;
	}
	fclose(out);
	
	sprintf(out_filename, "%s_up(%s)%s.bin", image_name_prefix_full, l_dev_info.build_date, image_name_suffix);
	...
	if (l_dev_info.is_datecode==1)
	{
		sprintf(out_filename, "%s_%s.bin",image_name_prefix_full, image_name_suffix);
	}
	else
	{
	sprintf(out_filename, "%s_up_boot(%s)%s.bin", image_name_prefix_full, l_dev_info.build_date, image_name_suffix);
	}
	if (!(out = fopen(out_filename, "w"))) {
		fprintf(stderr, "can not open \"%s\" for writing\n", out_filename);
		usage();
	...
	if (l_kernel_size)
	{
		if (!fwrite(buf, BOOTLOADER_TAG_LEN, 1, out) || fflush(out)) {
			fprintf(stderr, "fwrite tag failed\n");
			return EXIT_FAILURE;
		}

		if (!fwrite(buf+BOOTLOADER_TAG_LEN + l_flash_struct.kernelOffset, file_tag->kernelLen, 1, out) || fflush(out)) {
			fprintf(stderr, "fwrite failed\n");
			return EXIT_FAILURE;
		}
	
		if (!fwrite(buf+BOOTLOADER_TAG_LEN + l_flash_struct.rootfsOffset, file_tag->rootfsLen, 1, out) || fflush(out)) {
			fprintf(stderr, "fwrite failed\n");
			return EXIT_FAILURE;
		}
	}
	...
	{
		if (!fwrite(buf, file_tag->bootLen + BOOTLOADER_TAG_LEN, 1, out) || fflush(out)) {
	...
	if (1 == dual_image) 
	{
		sprintf(out_filename, "%s_flash_%dM(%s).bin", image_name_prefix_full, l_flash_size/0x100000, l_dev_info.build_date);
