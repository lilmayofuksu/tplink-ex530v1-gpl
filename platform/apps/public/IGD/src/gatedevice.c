...
int upnp_advertisementRunning = 0;
...
int AddPortMapping(struct Upnp_Action_Request *ca_event)
{
	...
	if (!g_vars.portMappingEnabled)
	{
		...
		strncpy(ca_event->ErrStr, "Internal Error", LINE_SIZE);
		ca_event->ActionResult = NULL;
		goto end;
	}
...
#endif /* INCLUDE_USB_FTP_SERVER */
#ifdef INCLUDE_AVOID_UPNP_USE_CWMP_PORT
		/* TCP check validation, UDP skip! */
		if ( 0 == strcmp(proto, "TCP") )
		{
			ext_port_num = atoi(ext_port);
			if ((g_vars.portCwmpUsed == ext_port_num) || (g_vars.portStunUsed == ext_port_num))
			{
...
int GetSpecificPortMappingEntry(struct Upnp_Action_Request *ca_event)
{
      ...
      && (proto = GetFirstDocumentItem(ca_event->ActionRequest,"NewProtocol")))
   {
      if ((strcmp(proto, "TCP") == 0) || (strcmp(proto, "UDP") == 0))
      {
			...
			if (temp)
			{
				snprintf(result_param, RESULT_LEN, "<NewInternalPort>%s</NewInternalPort><NewInternalClient>%s</NewInternalClient><NewEnabled>%d</NewEnabled><NewPortMappingDescription><![CDATA[%s]]></NewPortMappingDescription><NewLeaseDuration>%li</NewLeaseDuration>",
            temp->m_InternalPort,
...
int pmAddToKernel()
{
	...
	while (curr)
	{
		...
		pmlist_AddPortMapping_Conntrack(curr->m_PortMappingEnabled,
							  curr->m_PortMappingProtocol,
							  curr->m_ExternalPort,
		     				  curr->m_InternalClient,
		     				  curr->m_InternalPort
...
int exIntfChangedHandler(CMSG_BUFF *pCmsgBuff)
{
	...
	else if (!STR_IS_EMPTY(pMsg->gwName))
	{
#ifdef RENEW_ENTRY
		pmDelFromKernel();
#else /* RENEW_ENTRY */
...
UBOOL32 switchOnUpnp(char *descDocUrl)
{
	...
	/* If advertisement timer is running, no need to start another. */
	if (!upnp_advertisementRunning)
	{
