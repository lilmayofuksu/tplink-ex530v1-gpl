...
#if 1		/* ÂèåËøûÊé•ÁöÑÊÉÖÂÜµ‰∏ãÈúÄË¶ÅÊâìÂºÄÊ≠§ÂÆèÂºÄÂÖ≥ */
#define WAN_DUAL_ACCESS
#endif

#ifndef STR_IS_EMPTY(str)
...
struct GLOBALS {
	...
	char primInterfaceName[IFNAMSIZ]; // The name of the external interface for the second connection of WAN, picked up from the command line

	char primIntfAddr[BUFLEN_16];
#endif /* WAN_DUAL_ACCESS */

	char intInterfaceName[IFNAMSIZ]; // The name of the internal interface, picked from command line

	char extIntfAddr[BUFLEN_16];

	/*≈‰÷√ ˝æ›, Added by LI CHENGLONG , 2011-Nov-02.*/
	UBOOL32 natEnabled;
