...
void
swlib_free_val(struct switch_val *val)
{
	int type = 0;

	if (NULL == val)
	{
		printf("swlib_free_val error val %p\n", val);
		return;
	}

	if (NULL == val->attr)
	{
		printf("swlib_free_val error attr %p\n", val->attr);
		return;
	}
	...
	case SWITCH_TYPE_PORTS:
		if (NULL != val->value.ports)
		{
			free(val->value.ports);
			val->value.ports = NULL;
		}
		break;

	case SWITCH_TYPE_LINK:
		if (NULL != val->value.link)
		{
			free(val->value.link);
			val->value.link = NULL;
		}
	...
	default:
		printf("swlib_free_val error type %d\n", type);
		return;
	}
}
