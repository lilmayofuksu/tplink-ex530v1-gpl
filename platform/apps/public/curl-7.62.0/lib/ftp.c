...
#endif
  if(0 == access("/proc/tc3162/speed_test",F_OK))
  {
      port = ntohs(sa4->sin_port);
      memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
      snprintf(speed_test_cmd, sizeof(speed_test_cmd), "echo localport=%d > /proc/tc3162/speed_test", port);
      printf("\n%s\n", speed_test_cmd);
                                    ...
                                    int ftpcode)
{
  ...
  if(0 == access("/proc/tc3162/speed_test",F_OK))
	{
	  memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
	  snprintf(speed_test_cmd, sizeof(speed_test_cmd), "echo destport=%d > /proc/tc3162/speed_test", ftpc->newport);
	  printf("\n%s\n", speed_test_cmd);
	  system(speed_test_cmd);
	}
  ...
  /* don't wait for data connection */
  if(0 == access("/proc/tc3162/speed_test",F_OK))
  {
      conn->bits.tcpconnect[SECONDARYSOCKET] = TRUE;
      result = CURLE_OK;
                                    ...
                                    ftpstate instate)
{
  ...
  if((ftpcode == 331) && (ftpc->state == FTP_USER)) {
    ...
    if(conn->data->set.str[STRING_FTP_ALTERNATIVE_TO_USER] &&
      ...
      if(0 == access("/proc/tc3162/speed_test",F_OK))
      {
#define SPEEDTEST_STOPFILE	"/var/run/speedtest_stop"
          char speed_test_cmd[128] = {0};
          memset(speed_test_cmd, 0, sizeof(speed_test_cmd) );
          snprintf(speed_test_cmd, sizeof(speed_test_cmd), "echo 1 > %s", SPEEDTEST_STOPFILE);
          printf("\n%s\n", speed_test_cmd);
          system(speed_test_cmd);
      }
    }
