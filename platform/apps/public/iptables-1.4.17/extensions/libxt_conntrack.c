...
struct xt_conntrack_mtinfo3 up_tmp;
static void conntrack1_mt_parse(struct xt_option_call *cb)
{
	struct xt_conntrack_mtinfo1 *info = cb->data;
	

	memset(&up_tmp, 0, sizeof(up_tmp));
	cinfo_transform(&up_tmp, info);
	up_tmp.origsrc_port_high = up_tmp.origsrc_port;
	up_tmp.origdst_port_high = up_tmp.origdst_port;
	up_tmp.replsrc_port_high = up_tmp.replsrc_port;
	up_tmp.repldst_port_high = up_tmp.repldst_port;
	cb->data = &up_tmp;
	conntrack_mt_parse(cb, 3);
	if (up_tmp.origsrc_port != up_tmp.origsrc_port_high ||
	    up_tmp.origdst_port != up_tmp.origdst_port_high ||
	    up_tmp.replsrc_port != up_tmp.replsrc_port_high ||
	    up_tmp.repldst_port != up_tmp.repldst_port_high)
		xtables_error(PARAMETER_PROBLEM,
			"conntrack rev 1 does not support port ranges");
	cinfo_transform(info, &up_tmp);
	cb->data = info;
}
...
#define cinfo2_transform(r, l) \
	...
	memset(&up_tmp, 0, sizeof(up_tmp));
	memcpy(&up_tmp, info, sizeof(*info));
	up_tmp.origsrc_port_high = up_tmp.origsrc_port;
	up_tmp.origdst_port_high = up_tmp.origdst_port;
	up_tmp.replsrc_port_high = up_tmp.replsrc_port;
	up_tmp.repldst_port_high = up_tmp.repldst_port;
	...
	memcpy(info, &up_tmp, sizeof(*info));
	cb->data = info;
