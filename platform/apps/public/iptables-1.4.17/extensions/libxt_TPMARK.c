...
static void MARK_parse_v0(struct xt_option_call *cb)
{
	...
	default:
		xtables_error(PARAMETER_PROBLEM,
			   "TPMARK target: kernel too old for --%s",
			   cb->entry->name);
	}
}
...
static void MARK_check(struct xt_fcheck_call *cb)
{
	if (cb->xflags == 0)
		xtables_error(PARAMETER_PROBLEM,
		           "TPMARK target: Parameter --set/and/or-mark"
			   " is required");
}
...
{
	if (cb->xflags == 0)
		xtables_error(PARAMETER_PROBLEM, "TPMARK: One of the --set-xmark, "
                          ...
                          int numeric)
{
	...
	if (info->mark == 0)
		printf(" TPMARK and 0x%x", (unsigned int)(uint32_t)~info->mask);
	else if (info->mark == info->mask)
		printf(" TPMARK or 0x%x", info->mark);
	else if (info->mask == 0)
		printf(" TPMARK xor 0x%x", info->mark);
	else if (info->mask == 0xffffffffU)
		printf(" TPMARK set 0x%x", info->mark);
	else
		printf(" TPMARK xset 0x%x/0x%x", info->mark, info->mask);
}
...
static struct xtables_target tpmark_tg_reg[] = {
	{
...
void _init(void)
{
	xtables_register_targets(tpmark_tg_reg, ARRAY_SIZE(tpmark_tg_reg));
}
