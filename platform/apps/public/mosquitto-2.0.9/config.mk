...
#MANCOUNTRIES=en_GB
CFLAGS += -I$(PUBLIC_APPS_PATH)/$(INCLUDE_SSL_LIB_NAME)/install/include
CFLAGS += -I$(PUBLIC_APPS_PATH)/cJSON/install/
#LDFLAGS +=  -L$(PUBLIC_APPS_PATH)/$(INCLUDE_SSL_LIB_NAME)/install/lib -lssl -lcrypto -ldl -lpthread  
LDFLAGS +=  -L$(PUBLIC_APPS_PATH)/$(INCLUDE_SSL_LIB_NAME)/install/lib 
LDFLAGS +=  -L$(PUBLIC_APPS_PATH)/cJSON/install/cjson
UNAME:=$(shell uname -s)
$(error, $(UNAME))
ifeq ($(UNAME),SunOS)
	ifeq ($(CC),cc)
...
INSTALL?=install
prefix?=$(PUBLIC_APPS_PATH)/mosquitto-2.0.9/install
