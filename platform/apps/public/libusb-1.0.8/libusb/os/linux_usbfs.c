...
{
#if defined(INCLUDE_ECONET) || defined(INCLUDE_QUALCOMM) || defined(INCLUDE_QUECTEL)
	snprintf(path, PATH_MAX, "%s/%03d/%03d", usbfs_path, dev->bus_number,
		dev->device_address);
#else
	snprintf(path, PATH_MAX, "%s/%03d/%03d", DEV_AUTO_GEN_PATH, dev->bus_number,
		dev->device_address);    //changed by wxy for test
#endif
}
...
static int op_init(struct libusb_context *ctx)
{
	...
	//attention: linux4.1 has the /dev/bus/usb in VR2810 2.5, but linux3.5 XR500v doesn't have it.

	usbfs_path = find_usbfs_path();
#if defined(INCLUDE_ECONET) || defined(INCLUDE_QUALCOMM) || defined(INCLUDE_QUECTEL)
	if (!usbfs_path) {
#else
	if (strncmp(usbfs_path, "/proc/bus/usb", strlen(usbfs_path))) {   //changed by wxy fot kernel linux4.1
#endif
	...
	uint8_t config_index, unsigned char *buffer, size_t len, int *host_endian)
{
	...
	if (!usbfs_path)  
#else 
	if (strncmp(usbfs_path, "/proc/bus/usb", strlen(usbfs_path)))   //if there is no usbfs in the kernel, we create the node.
#endif
	{
...
static int sysfs_create_device_node(struct libusb_device *dev, char *filename, int namelen)
{
	...
	//printf("\nWXY:strarr is %s, major = %d, dev->dev_address=%d, minor = %d.\n", strarr, major, dev->device_address, minor);
	//if (mknod(strarr, S_IFCHR | 0666, MKDEV(major, minor)) < 0)
	if (mknod(strarr, S_IFCHR|0666, makedev(major, minor)) < 0)    //changed by wxy for kernel linux4.1 
	{
