...
int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, int fd, SMB_OFF_T len)
{
	...
	/* According to W. R. Stevens advanced UNIX prog. Pure 4.3 BSD cannot
	   extend a file with ftruncate. Provide alternate implementation
	   for this */
	currpos = SMB_VFS_LSEEK(fsp, fd, 0, SEEK_CUR);	//to check if we could modify the file---wubaijun
	if (currpos == -1) {
		goto done;
	}
	...
	if (st.st_size > len) { //ftruncate should work for shrinking file anyway---wubaijun
		goto done;
	}	

	if(SMB_VFS_PWRITE(fsp, fd, &c, 1, len-1) != 1)	//using pwrite is better---wubaijun
		goto done;
