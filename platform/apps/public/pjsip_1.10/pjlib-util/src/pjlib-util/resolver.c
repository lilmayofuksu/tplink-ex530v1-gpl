					    ...
					    const pj_uint16_t ports[])
{
    ...
    for (i=0; i<count; ++i) {
		...
		af = pj_AF_INET();
		if (pj_strchr(&servers[i], ':'))
		{
			af = pj_AF_INET6();
		}
		status = pj_sockaddr_init(af, &ns->addr, &servers[i],
				  ...
				  pj_dns_async_query *q)
{
	...
	PJ_PERROR(4,(resolver->name.ptr, status,
		  "%s %d bytes to NS %d (:%d): DNS %s query for %s",
 ...
 */
pj_status_t pj_dns_parse_addr_response(
				 ...
				 const pj_time_val *now)
{
    ...
    PJ_LOG(5, (resolver->name.ptr, "Nameserver :%d state changed %s --> %s",
                             ...
                             pj_ssize_t bytes_read)
{
    ...
    PJ_LOG(5,(resolver->name.ptr, 
	      "Received %d bytes DNS response from :%d",
	      (int)bytes_read, 
	...
	PJ_LOG(3,(resolver->name.ptr, 
		  "Error parsing DNS response from :%d: %s",
		  pj_sockaddr_get_port(src_addr),
		  errmsg));
	goto read_next_packet;
    }
    ...
    if (!q) {
	PJ_LOG(5,(resolver->name.ptr, 
		  "DNS response from :%d id=%d discarded",
