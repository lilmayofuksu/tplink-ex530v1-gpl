...
CMSIP_PRINT("--------attach media transport only for single call---------------------");
   ...
   pj_ioqueue_callback rtp_cb, rtcp_cb;
	...
	CMSIP_PRINT("pool[%p], ioqueue[%p], socket[%d]", tp->pool, ioqueue, (int)tp->rtp_sock);
	...
	CMSIP_PRINT("pool[%p], ioqueue[%p], socket[%d]", tp->pool, ioqueue, (int)tp->rtcp_sock);
