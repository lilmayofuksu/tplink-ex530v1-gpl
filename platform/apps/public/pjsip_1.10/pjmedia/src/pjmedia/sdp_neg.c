				  ...
				  pjmedia_sdp_session **p_answer)
{
	...
	{
		const pjmedia_sdp_media *pOm;	/* offer */
		pOm = offer->media[i];
		if (pj_strcmp2(&pOm->desc.media, "image") == 0)
		{
			foundT38 = PJ_TRUE;
			break;
		}
    ...
    for (i=0; i<offer->media_count; ++i)
	 {
		...
		for (im=NULL, j=0; j<initial->media_count; ++j)
		{
	   		im = initial->media[j];
#ifdef INCLUDE_TM
			if ((PJ_TRUE == enableT38) && (PJ_TRUE == foundT38) && 
				(pj_strcmp2(&om->desc.media, "image") != 0))
			{
				continue;
			}
#endif /* INCLUDE_TM */
