					      ...
					      pjmedia_sdp_rtcp_attr *rtcp)
{
	...
	if (!pj_scan_is_eof(&scanner)) {
	    ...
	    /* Get the address */
#if 0
		/* cs_token doens't contain ':' so pj_scan_get can't get ipv6 address */
	    pj_scan_get(&scanner, &cs_token, &rtcp->addr);
#else
		/* we should use pj_scan_get_until_chr to get ip address in rtcp */
				       ...
				       pjmedia_sdp_session **p_sdp)
{
	...
	while (!pj_scan_is_eof(&scanner)) {
		...
		case 'c':
...
#endif
			{
				if (!strncmp(conn->addr_type.ptr, "IP4", 3))
				{
					if (media) {
						media->conn = conn;
					    } else {
			...
			{
				if (!strncmp(conn->addr_type.ptr, "IP6", 3))
				{
					if (media) {
						media->conn = conn;
					    } else {
			...
			if (m->desc.port != 0 && pt >= 96)
			{
			    ...
			    CHECK( a != NULL, PJMEDIA_SDP_EMISSINGRTPMAP);
					...
					if (a == NULL)
					{
						...
						m->desc.fmt_count --;
						for (k = j; k < m->desc.fmt_count; k++)
						{
							pj_strcpy(&m->desc.fmt[k], &m->desc.fmt[k+1]);
						}
						j--;
					}
#endif
...
#ifdef INCLUDE_WISP_DE
		CHECK( m->desc.fmt_count > 0, PJMEDIA_SDP_EMISSINGRTPMAP);
#endif
    }
