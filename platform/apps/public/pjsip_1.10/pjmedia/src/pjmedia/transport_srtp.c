...
/*
 * Get SRTP encryption context from SRTP Transport.
 */
pj_status_t pjmedia_transport_srtp_info(pjmedia_transport *tp,
										pj_uint8_t *pTxUseSrtp, pj_uint8_t *pRxUseSrtp,
										pj_str_t *pTxSuite, pj_str_t *pRxSuite,
										unsigned char *txKeySalt, unsigned char *rxKeySalt)
{
   transport_srtp *srtp = (transport_srtp *)tp;

   *pTxUseSrtp = PJMEDIA_SRTP_MANDATORY == srtp->setting.use ? 1 : 0;
   *pRxUseSrtp = PJMEDIA_SRTP_MANDATORY == srtp->peer_use ? 1 : 0;

   *pTxSuite = pj_str(crypto_suites[get_crypto_idx(&srtp->tx_policy.name)].name);
   *pRxSuite = pj_str(crypto_suites[get_crypto_idx(&srtp->rx_policy.name)].name);

   pj_bzero(txKeySalt, MAX_KEY_LEN);
   pj_bzero(rxKeySalt, MAX_KEY_LEN);
   pj_memcpy(txKeySalt, (uint8_t *)srtp->tx_policy.key.ptr, srtp->tx_policy.key.slen);
   pj_memcpy(rxKeySalt, (uint8_t *)srtp->rx_policy.key.ptr, srtp->rx_policy.key.slen);
}
