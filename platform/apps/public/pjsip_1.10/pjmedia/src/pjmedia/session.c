...
#ifndef INCLUDE_WISP_DE
		pt != PJMEDIA_RTP_PT_TELEPHONE_EVENTS
#else
		pt == PJMEDIA_RTP_PT_PCMA ||
		pt == PJMEDIA_RTP_PT_PCMU ||
		pt == PJMEDIA_RTP_PT_G729 ||
		pt == PJMEDIA_RTP_PT_G721 ||
		pt == PJMEDIA_RTP_PT_G726_32 ||
...
CMSIP_PRINT("-----notify stream Info to CM!");
	...
	pj_uint8_t txUseSrtp = 0;
	pj_uint8_t rxUseSrtp = 0;
	CMSIP_SRTP_SUITE txSuite = CMSIP_SRTP_SUITE_MAX;
	CMSIP_SRTP_SUITE rxSuite = CMSIP_SRTP_SUITE_MAX;
	unsigned char txKeySalt[MAX_SRTP_MASTER_KEY_SALT_LEN];
	memset(txKeySalt, 0, MAX_SRTP_MASTER_KEY_SALT_LEN);
	unsigned char rxKeySalt[MAX_SRTP_MASTER_KEY_SALT_LEN];
	memset(rxKeySalt, 0, MAX_SRTP_MASTER_KEY_SALT_LEN);

	/* Record names of SRTP policies in two directions. */
	pj_str_t txSuiteName;
	memset(&txSuiteName, 0, sizeof(txSuiteName));
	pj_str_t rxSuiteName;
	memset(&rxSuiteName, 0, sizeof(rxSuiteName));
	...
	if (tp)
	{
		pjmedia_transport_udp_local_info(tp, localIp, &localPort, &localRtcpPort, &family);
		pjmedia_transport_udp_remote_info(tp, remoteIp, &remotePort, &remoteRtcpPort, &family);

#ifdef INCLUDE_SRTP
		if (PJMEDIA_SRTP_MANDATORY == pjsua_var.ua_cfg.use_srtp)
		{
			call = &pjsua_var.calls[callIndex];
			pjmedia_transport_srtp_info(call->med_tp,
										&txUseSrtp, &rxUseSrtp,
										&txSuiteName, &rxSuiteName,
										txKeySalt, rxKeySalt);

			if (!pj_strcmp2(&txSuiteName, "AES_CM_128_HMAC_SHA1_80"))
			{
				txSuite = CMSIP_SRTP_SUITE_AES_CM_128_HMAC_SHA1_80;
			}
			else if (!pj_strcmp2(&txSuiteName, "AES_CM_128_HMAC_SHA1_32"))
			{
				txSuite = CMSIP_SRTP_SUITE_AES_CM_128_HMAC_SHA1_32;
			}
			else if (!pj_strcmp2(&txSuiteName, "F8_128_HMAC_SHA1_80"))
			{
				txSuite = CMSIP_SRTP_SUITE_F8_128_HMAC_SHA1_80;
			}
			else
			{
				txSuite = CMSIP_SRTP_SUITE_MAX;
			}

			if (!pj_strcmp2(&rxSuiteName, "AES_CM_128_HMAC_SHA1_80"))
			{
				rxSuite = CMSIP_SRTP_SUITE_AES_CM_128_HMAC_SHA1_80;
			}
			else if (!pj_strcmp2(&rxSuiteName, "AES_CM_128_HMAC_SHA1_32"))
			{
				rxSuite = CMSIP_SRTP_SUITE_AES_CM_128_HMAC_SHA1_32;
			}
			else if (!pj_strcmp2(&rxSuiteName, "F8_128_HMAC_SHA1_80"))
			{
				rxSuite = CMSIP_SRTP_SUITE_F8_128_HMAC_SHA1_80;
			}
			else
			{
				rxSuite = CMSIP_SRTP_SUITE_MAX;
			}
		}
#endif /* INCLUDE_SRTP */
	}
...
#	endif
		...
		*/
		else
		{
			...
			char srcAddr[CMSIP_STR_40];
			char dstAddr[CMSIP_STR_40];
			...
			pjmedia_transport_udp_local_info(session->stream[i]->transport, 
				srcAddr, &localFwPort, &localFwRtcpPort, &family);
			pjmedia_transport_udp_remote_info(session->stream[i]->transport, 
				dstAddr, &remoteFwPort, &remoteFwRtcpPort, &family);

			src = pj_str(srcAddr);
			dst = pj_str(dstAddr);
			status = pj_firewall_set_rule_accept(PJ_FIREWALLCFG_ADD, protocol, 
					&src, NULL, localFwPort, &dst, NULL, remoteFwPort, pj_firewall_get_fwtype(family)
			...
			status = pj_firewall_set_rule_accept(PJ_FIREWALLCFG_ADD, protocol, 
					&src, NULL, localFwRtcpPort, &dst, NULL, remoteFwRtcpPort, pj_firewall_get_fwtype(family)
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
					, family
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
					, tcMark);
...
#	endif
#if defined(PJSIP_HAS_FIREWALL_FILTER)&& PJSIP_HAS_FIREWALL_FILTER!=0
						,PJ_FIREWALLCFG_CTRL fwCtrl, pj_uint32_t tcMark
#endif
																)
{
    unsigned i;
