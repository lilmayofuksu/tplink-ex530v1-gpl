...
#ifdef INCLUDE_TRTC
		/*liuqianhao add: we need to skip G726-32 with payload type 2,otherwise
		 * the server will not response with telephone-event, and RFC2833 can not be used
		 */		
		if (codec_param.info.pt == 2)		
		{		
			continue;		
		}
#endif /*INCLUDE_TRTC*/
...
void pjmedia_endpt_unset_mediaPort(pjmedia_endpt * endpt)
{
	pj_uint16_t	port;
	port = endpt->mediaPort;
	if (port - 2 < PJMEDIA_PORT_LOWER_BOUND)
	{
		port = PJMEDIA_PORT_UPPER_BOUND;
	}
	else
	{
		port -= 2;
...
pj_status_t	pjmedia_endpt_test_media_port(pj_str_t boundIp, pj_uint16_t port, pj_bool_t* avaible, int family)
{
	pj_sock_t	sockfd;
	pj_sockaddr bindaddr;
	pj_status_t	status;
	int on = 1;
	...
	*avaible = PJ_TRUE;
	...
	status = pj_sockaddr_set_str_addr(family, &bindaddr, &boundIp);
	if (status != PJ_SUCCESS)
	{
		pjsua_perror(THIS_FILE, "resolve address error", status);
		return status;
	}
	status = pj_sockaddr_set_port(&bindaddr, port);
	if (status != PJ_SUCCESS)
	{
		pjsua_perror(THIS_FILE, "set port error", status);
		return status;
	}
	status = pj_sock_socket(family, pj_SOCK_DGRAM(), 0, &sockfd);
	if (status != PJ_SUCCESS)
	{
	...
	status = pj_sock_bind(sockfd, &bindaddr, sizeof(bindaddr));
	if (status != PJ_SUCCESS)
	{
		*avaible = PJ_FALSE;
