...
#define _VOIP_DEV_NAME  "/dev/voip"
#define _VOIP_IOC_MAGIC 'v'
#define _VOIP_TIMER_START			_IOW(_VOIP_IOC_MAGIC, 8, int)
#define _VOIP_TIMER_STOP			_IOW(_VOIP_IOC_MAGIC, 9, int)
/**************************************************************************************************/
/*                                           TYPES                                                */
/**************************************************************************************************/
/* 
								...
								*/
								...
								if (access(notifyFilePathName, F_OK) == 0) 
								{
									...
									if ((g_usbvm_recordHandle[recordId].outputFp = fopen(notifyFilePathName, "rb")) == NULL)
									{
										...
										gotoDeinit = TRUE;
										/*Edited by huanglei,�˳���ʹcmǿ�Ƴ�ʱ��
										��������Զ��usbvm�����˳������������Ǳ��߳��˳� 2012/01/11*/
										sipMsgModifyCMTimer(recordId, 100);
			...
			{	
				...
				if (g_usbvm_recordHandle[recordId].fileSize >= currentPos)
				{
					...
					if (len > 0)
					{
						...
						if (!chkRtpStreamReady(call))
						{
							currentPos += len;/* Ӧ�ò��ŵ������ˣ�ʵ�����Ѿ��������ˣ�Ӧ�ü���len*/
							PJSUA_UNLOCK();
							continue;
						}
					...
					else   /* Play finished */
					{
						...
						/*Edited by huanglei, 2012/03/27, ��Ȼ�����Ӧ����Ϊ0*/
						currentPos = 0;		
						switch(statusRead)
						{
						case REMOTE_RECORD_START_NOTIFY:
 ...
 */
				...
				*/
				...
				status = pjmedia_transport_send_rtp(call->session->stream[0]->transport, (pjmedia_rtp_hdr *)&rtpPacket,
				len + sizeof(pjmedia_rtp_hdr));
				if (status != PJ_SUCCESS)
				{
					char errmsg[PJ_ERR_MSG_SIZE];
					pj_strerror(status, errmsg, sizeof(errmsg));
					MARK_PRINT("pjmedia_transport_send_rtp error:%s(error no %d)", errmsg, status);
...
{	
	...
	g_usbvm_started[cid]=TRUE;		 /* �����dtmfʱ�ж��Ƿ�������usbvm  */
	l_exitMsgSent[recordId]=FALSE;
	/* ������ʱ��Ϊtrue��¼��ʱ��Ϊfalse��¼������ĺ���һ���о���Ϊtrue */
...
void usbvm_exitRemoteUSBvoiceMail( int recordId )
{
	...
	{
		/* add by yuanjianxiang at 2022.12.9 */
		/* it seems that, calling pthread_join() on a thread that no longer exists may cause segmentation fault. */
		if (index == 0 && g_usbvm_recordHandle[recordId].playThreadRunning == PJ_FALSE)
		{
			MARK_PRINT("HL_DBG[%s:%d]: pjsua_var.usbvmThread[%d][%d] playThreadRunning[%d] exited!!! \n",
				__FUNCTION__,__LINE__, recordId, index, g_usbvm_recordHandle[recordId].playThreadRunning);
			continue;
		}

		if (index == 1 && g_usbvm_recordHandle[recordId].rcvRtpThreadRunning == PJ_FALSE)
		{
			MARK_PRINT("HL_DBG[%s:%d]: pjsua_var.usbvmThread[%d][%d] rcvRtpThreadRunning[%d] exited!!! \n",
				__FUNCTION__,__LINE__, recordId, index, g_usbvm_recordHandle[recordId].rcvRtpThreadRunning);
			continue;
		}

		if (index == 2 && g_usbvm_recordHandle[recordId].recordThreadRunning == PJ_FALSE)
		{
			MARK_PRINT("HL_DBG[%s:%d]: pjsua_var.usbvmThread[%d][%d] recordThreadRunning[%d] exited!!! \n",
				__FUNCTION__,__LINE__, recordId, index, g_usbvm_recordHandle[recordId].recordThreadRunning);
			continue;
		}
		/* end add by yuanjianxiang at 2022.12.9 */
