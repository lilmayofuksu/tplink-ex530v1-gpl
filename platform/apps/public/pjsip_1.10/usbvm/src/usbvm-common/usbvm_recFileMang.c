...
#if 0
#if 0	/* cxmod 在900V中，我们只能发给client，不过这里为了保持统一，就不修改函数名了 */
    if (0 != msg_connCliAndSend(CMSG_ID_VOIP_SERVER, &msgFd, &msg))
#else
    if (0 != msg_connCliAndSend(CMSG_ID_VOIP_CLIENT_DECT, &msgFd, &msg))
#endif
    {
...
#ifndef INCLUDE_FLASH_VOICEMAIL
...
void usbvm_wavNameFormat( USBVM_REC_FILE_NODE recNode, char *pwavStr )
{
	...
	/* 2022.12.9 edited by yuanjianxiang */
	size_t timestamp = (size_t)recNode.fileTimName;
	
	/*<< BosaZhong@12Oct2013, modify, we use localtime according with web time. */
	#if 1
	ptm = gmtime((time_t *)&(timestamp));
	#else
	ptm = localtime((time_t *)&(timestamp));
	#endif /* 0 */
...
void usbvm_wavNameFormat2(unsigned int fileTimName, char *srcNum, char *dstNum, char *pwavStr )
{
	...
	/* 2022.12.9 edited by yuanjianxiang */
	size_t timestamp = (size_t)fileTimName;
	ptm = gmtime((time_t *)&(timestamp));
...
int usbvm_markNodeReadStatus(int endpt, int allnum, unsigned int fileTime, int status)
{
	FILE* fpIndex = NULL;
	USBVM_REC_FILE_NODE node;
	int lockRet = -1;
	...
	if (lockRet != -1)
	{
		for (i = 0; i < allnum; i++)
		{
			if (fseek(fpIndex, RECLIST_INDEX_AREA_SIZE + i * sizeof(USBVM_REC_FILE_NODE), SEEK_SET) == -1)
			{
				MARK_PRINT("seek index(%d) node failed", i);
				ret = -1;
				break;
			}
			...
			if (fread_USBVM_REC_FILE_NODE(&node, sizeof(USBVM_REC_FILE_NODE), 1, fpIndex) != 1)
			{
				MARK_PRINT("read index(%d) node failed", i);
				ret = -1;
				break;
			}

			if (fileTime == node.fileTimName)
			{
				node.readFlag = status;

				usbvm_calChkSum(&node);
				...
				if (fwrite_USBVM_REC_FILE_NODE(&node, sizeof(USBVM_REC_FILE_NODE), 1, fpIndex) != 1)
				{
					MARK_PRINT("write index(%d) node failed", i);
					ret = -1;
				}
