...
/* 
 * brief	deal --set-mark parameter, 后16位用来在路由时选择路由表，需要用套接字选项设置
 */
#define CMSIP_ROUTE_MARK_MASK	0xFFFF

/* 
 * brief	deal --set-mark parameter, 前16位用来选择TC队列，需要用iptables设置
 */
#define CMSIP_TC_MARK_MASK		0xFFFF0000
#define CMSIP_TC_MARK_OFFSET	16

/*
 * brief	The maximum length of SRTP master key and salt.
 */
#ifdef INCLUDE_SRTP
#define MAX_SRTP_MASTER_KEY_SALT_LEN	32
#endif /* INCLUDE_SRTP */
...
/*SIP operation*/
	...
	CMSIP_RESPONSE_SIP_REGORUNREG = 1008, /* Indicate PJSIP is currently registering or unregistering */
}CMSIP_RESPONSE;
...
typedef enum _CMSIP_SIP_REG_ACTION
{
	CMSIP_SIP_REGISTERING = 0,
	CMSIP_SIP_UNREGISTERING = 1
} CMSIP_SIP_REG_ACTION;

typedef struct _CMSIP_SIP_REGORUNREG
{
	int accId;
	CMSIP_SIP_REG_ACTION regOrUnreg;
} CMSIP_SIP_REGORUNREG;
...
{
	CMSIP_REG_STATE_SUCCESS = 0, /* Register success */
	/* Not sure how to distinguish register failure and unregister failure */
	CMSIP_REG_STATE_FAILURE = 1, /* Register/Unregister failure */
	CMSIP_UNREG_STATE_SUCCESS = 2 /* Unregister success */
} CMSIP_REG_STATE;
/* 
...
/*
 * brief	Legal encryption and authentication policy in SRTP.
 */
#ifdef INCLUDE_SRTP
typedef enum _CMSIP_SRTP_SUITE
{
	CMSIP_SRTP_SUITE_AES_CM_128_HMAC_SHA1_80	= 0,
	CMSIP_SRTP_SUITE_AES_CM_128_HMAC_SHA1_32,
	CMSIP_SRTP_SUITE_F8_128_HMAC_SHA1_80,
	CMSIP_SRTP_SUITE_MAX
} CMSIP_SRTP_SUITE;
#endif /* INCLUDE_SRTP */
...
#ifdef INCLUDE_SRTP
	unsigned short			txUseSrtp;								 /* Indicate if SRTP will work in sending direction. */
	unsigned short			rxUseSrtp;  							 /* Indicate if SRTP will work in receiving direction. */
	CMSIP_SRTP_SUITE		txSuite;								 /* Indicate SRTP policy in sending direction. */
	CMSIP_SRTP_SUITE		rxSuite;								 /* Indicate SRTP policy in receiving direction. */
	unsigned char			txKeySalt[MAX_SRTP_MASTER_KEY_SALT_LEN]; /* Record SRTP master key and salt in sending direction. */
	unsigned char			rxKeySalt[MAX_SRTP_MASTER_KEY_SALT_LEN]; /* Record SRTP master key and salt in receiving direction. */
#endif /* INCLUDE_SRTP */
}CMSIP_SIP_RTPCREATE;
