...
int cmsip_sockRecv(int sockfd, char* buf, int size)
{
	...
	*/
	...
	while (sum < size)
	{
		...
		/* if recv() returns 0, it means other side socket has beend closed
		 * successfully, or other side send an empty buffer. We could make sure
		 * CM did not send empty buffer, so if we receive with empty,
		 * we should treat as CM disconnect and exit pjsua. Otherwise there will
		 * be an infinate loop which consumes all CPU resource.
		 */
		else if(ret == 0)
		{
			return -1;
...
int cmsip_send_regOrUnreg(int accId, CMSIP_SIP_REG_ACTION regOrUnreg)
{
	CMSIP_MSG cmSipMsg;
	CMSIP_SIP_REGORUNREG *reg = NULL;

	cmSipMsg.type = CMSIP_RESPONSE_SIP_REGORUNREG;

	reg = (CMSIP_SIP_REGORUNREG *)cmSipMsg.body;
	reg->accId = accId;
	reg->regOrUnreg = regOrUnreg;

	if (cmsip_sockSend(g_cmsip_cliSockfd, (char *)&cmSipMsg, sizeof(cmSipMsg)) < 0)
	{
		#ifdef CMSIP_DEBUG
			printf("%s,%d: can not send CMSIP_RESPONSE_SIP_REGORUNREG\n", __FUNCTION__, __LINE__);
		#endif
		return -1;
	}
...
#	endif
	...
	if (ipaddr)
	{
		pRegStat->family = family;
		strcpy(pRegStat->ipaddr, ipaddr);
	}
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
 ...
 *	param[in]	af				The address family of local/remote address.
 * param[out]	none
 *
 * return	int
 * retval	0	success
...
#ifdef INCLUDE_SRTP
					,
					unsigned short txUseSrtp,
					unsigned short rxUseSrtp,
					CMSIP_SRTP_SUITE txSuite,
					CMSIP_SRTP_SUITE rxSuite,
					unsigned char *txKeySalt,
					unsigned char *rxKeySalt
#endif /* INCLUDE_SRTP */
					)
{
	CMSIP_MSG	cmSipMsg;
	...
	pRtpCreate->af = af;
#ifdef INCLUDE_SRTP
	pRtpCreate->txUseSrtp = txUseSrtp;
	pRtpCreate->rxUseSrtp = rxUseSrtp;
	pRtpCreate->txSuite = txSuite;
	pRtpCreate->rxSuite = rxSuite;
	memcpy(pRtpCreate->txKeySalt, txKeySalt, MAX_SRTP_MASTER_KEY_SALT_LEN);
	memcpy(pRtpCreate->rxKeySalt, rxKeySalt, MAX_SRTP_MASTER_KEY_SALT_LEN);
#endif /* INCLUDE_SRTP */
