...
int useIPv6()
{
	if (app_config.ipv6 && app_config.udp_cfg.bound_addr6.slen)
		return PJ_TRUE;
	else
		return PJ_FALSE;
}
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	cfg->udp_cfg.qos_params.ipv6_tclass = 0;
#endif
...
#else
	/* BosaZhong@15Aug2013, modify, 4000 -> 60000 */
    cfg->rtp_cfg.port = 60000;		
#endif
...
#	endif
	   ...
	   OPT_TLS_SRV_NAME,
#endif
	   OPT_ENABLE_T38, /*ycw-pjsip-enableT38*/

#if defined(INCLUDE_USB_VOICEMAIL)
	   OPT_ENABLE_VM_REMOTE_ACCESS, /* wlm: add for voice mail remote access */
#endif /* INCLUDE_USB_VOICEMAIL */
#if defined(PJ_HAS_IPV6) && PJ_HAS_IPV6
	   OPT_IPV6,
	 	OPT_BOUND_ADDR6,
		OPT_BOUND_ADDR6_PLEN,
		OPT_FWTYPE6,
#endif
		OPT_SIP_QOS,
		OPT_RTP_QOS,
	   ...
	   /*USB Voice Mail*/
		...
		OPT_SEL_NOTIFY,
		OPT_PIN_NUMBER,
		OPT_HAS_TELEVT, /*ycw-pjsip. add telephone-event pt or not*/
		OPT_NEXT_ENDPT,
		OPT_BOUND_IFNAME,
		OPT_FWTYPE,
		...
		OPT_SET_MARK,
		OPT_USER_AGENT_DOMAIN, /* BosaZhong@17Jun2014, add, UserAgentDomain. */
		OPT_UNREG_BEFORE_REG, /* BosaZhong@17Jun2014, add, unreg before reg. */
		OPT_LINE_NUMBER,
		OPT_LINE_ID,
    };
...
/* wlm: for voice mail remote access */
		...
		{ "ipv6",	 						0, 0, OPT_IPV6},
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
		{ "bound-addr6", 					1, 0, OPT_BOUND_ADDR6},
		{ "bound-addr6-plen", 				1, 0, OPT_BOUND_ADDR6_PLEN},
		{ "fwtype6",						1,	0,	OPT_FWTYPE6},
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
#endif
/*ycw-pjsip-qos*/
#if 0
...
/*Because of the complexity of */
		...
		/*<< BosaZhong@17Jun2014, add, UserAgentDomain. */
		case OPT_USER_AGENT_DOMAIN:
			cur_acc->userAgentDomain = pj_str(pj_optarg);
			break;
		/*>> endof BosaZhong@17Jun2014, add, UserAgentDomain. */

		/*<< BosaZhong@17Jun2014, add, unreg before reg. */
		case OPT_UNREG_BEFORE_REG:
			cur_acc->unregBeforeReg = pj_strtoul(pj_cstr(&tmp, pj_optarg));
			break;
		/*>> endof BosaZhong@17Jun2014, add, unreg before reg. */
...
#	endif
	...
	case OPT_LINE_ID:	/*line	id */
	{
		...
		cur_acc->lineId= pj_str(pj_optarg);
	}
		break;
	...
	case OPT_USERNAME:   /* Default authentication user */
		...
		CMSIP_PRINT("authenticate name(%d, %.*s)",
			(int)cur_acc->cred_info[cur_acc->cred_count].username.slen,
			(int)cur_acc->cred_info[cur_acc->cred_count].username.slen,
			cur_acc->cred_info[cur_acc->cred_count].username.ptr
		);
	    break;

	case OPT_LINE_NUMBER:	/* line number for a trunk */
		cur_acc->linenumber = pj_str(pj_optarg);
				CMSIP_PRINT("############authenticate name(%d, %.*s)######################",
			(int)cur_acc->linenumber.slen,
			(int)cur_acc->linenumber.slen,
			cur_acc->linenumber.ptr
		);
		break;
...
/*ycw-pjsip*/
	...
	case OPT_IPV6:
	    cfg->ipv6 = PJ_TRUE;
	#ifdef INCLUDE_VOIP_SUPPORT_IPV6
		cfg->udp_cfg.qos_params.ipv6_tclass = PJ_TRUE;
	#endif
	    break;
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	case OPT_BOUND_ADDR6:
		cfg->udp_cfg.bound_addr6 = pj_str(pj_optarg);
		pjsua_var.BoundIp6 = pj_str(pj_optarg);
		break;
#if 0
	case OPT_BOUND_ADDR6_PLEN:
		cfg->udp_cfg.addr6_plen = pj_strtoul(pj_cstr(&tmp, pj_optarg));
		break;
#endif
	case OPT_FWTYPE6:
		cfg->fwType6 = (pj_uint8_t)my_atoi(pj_optarg);
		break;
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
#endif
	/*ycw-pjsip-qos*/
	...
	case OPT_SET_MARK:
		{
			pj_str_t strTmp = pj_str(pj_optarg);
			cfg->mark = pj_strtoul2(&strTmp, NULL, 16);
		}
		break;
	default:
	    PJ_LOG(1,(THIS_FILE, 
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
		 && cfg->udp_cfg.bound_addr6.slen <= 0
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
		)
	 {
    ...
    if (call_info.state == PJSIP_INV_STATE_DISCONNECTED)
	 {
		...
		/* The server block us , so we should try another server for register. */
		if (call_info.last_status == PJSIP_SC_REQUEST_TIMEOUT)
		{
			pjsua_acc_try_next_ip(call_info.acc_id, 1);
		}
	 ...
	 else
	 {
	    ...
	    PJ_LOG(3,(THIS_FILE, "Call %d state changed to %s (%d %.*s)", 
		      ...
		      code, (int)msg->line.status.reason.slen, msg->line.status.reason.ptr));
	} else {
				     ...
				     int af)
{
	...
	size = sprintf(boundIpBuf, "%s%s%.*s%s", schemes[sipUriType], begin_token, (int)pBoundIp->slen,
						pj_strnull(pBoundIp->ptr), end_token);
	...
	ipUri = pjsip_parse_uri((pj_pool_t *)pool, boundIpBuf, strlen(boundIpBuf), 0);
	if (NULL == ipUri)
	{
	...
	dummyUri = (pjsip_sip_uri*)pjsip_uri_clone((pj_pool_t *)pool, dstSipUri);
	...
	CMSIP_PRINT("user(%.*s,%.*s) passwd(%.*s, %.*s) host(%.*s,%.*s) port(%d,%d)",
		(int)dummyUri->user.slen, dummyUri->user.ptr, (int)ipSipUri->user.slen, ipSipUri->user.ptr,
		(int)dummyUri->passwd.slen, dummyUri->passwd.ptr, (int)ipSipUri->passwd.slen, ipSipUri->passwd.ptr,
		(int)dummyUri->host.slen, dummyUri->host.ptr, (int)ipSipUri->host.slen, ipSipUri->host.ptr,
...
#	endif
		...
		idUri = pjsip_parse_uri((pj_pool_t *)pool, acc->cfg.id.ptr, acc->cfg.id.slen, 0);
		if (NULL == idUri)
				{
		...
		idSipUri = (pjsip_sip_uri*)pjsip_uri_get_uri(idUri);
			CMSIP_PRINT("user(%.*s,%.*s) passwd(%.*s, %.*s) host(%.*s,%.*s) port(%d,%d)",
		(int)dummyUri->user.slen, dummyUri->user.ptr, (int)idSipUri->user.slen, idSipUri->user.ptr,
		(int)dummyUri->passwd.slen, dummyUri->passwd.ptr, (int)idSipUri->passwd.slen, idSipUri->passwd.ptr,
		(int)dummyUri->host.slen, dummyUri->host.ptr, (int)idSipUri->host.slen, idSipUri->host.ptr,
		...
		else if (PJSIP_ECMPHOST==status)
		{
			...
			if( pjsua_matchInAccHostIpList(i, ip) || 0==pj_strcmp(&dummyUri->host, pBoundIp))
				{
				if ((0==idSipUri->port 
...
pj_status_t app_init(int argc, char *argv[])
{
	...
	pjsua_transport_id transport6_id = -1;
...
#	else
	{
		...
		pj_str_t defaultIp;
		defaultIp.ptr = buf;
		if (pjsua_var.BoundIp.slen)
		{
		memcpy(defaultIp.ptr, pjsua_var.BoundIp.ptr, pjsua_var.BoundIp.slen);
		defaultIp.slen = pjsua_var.BoundIp.slen;
		}
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
		else if (pjsua_var.BoundIp6.slen)
		{
			memcpy(defaultIp.ptr, pjsua_var.BoundIp6.ptr, pjsua_var.BoundIp6.slen);
			defaultIp.slen = pjsua_var.BoundIp6.slen;
		}
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
		defaultIp.ptr[defaultIp.slen] = 0;
	 	pjsua_init_random_seed(&defaultIp);
...
#   endif /* INCLUDE_USB_VOICEMAIL */
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	pjsua_var.fwType6 = app_config.fwType6;
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
	pjsua_var.tcMark = (app_config.mark & CMSIP_TC_MARK_MASK) >> CMSIP_TC_MARK_OFFSET;
	pjsua_var.fwMark = app_config.mark & CMSIP_ROUTE_MARK_MASK;
	...
	pj_nameserver_set_routemark(pjsua_var.fwMark);
	pj_nameserver_clear_nameservers();
	for (i = 0; i < app_config.cfg.nameserver_count; i++)
	{
		pj_nameserver_set_nameserver(app_config.cfg.nameserver[i].ptr);
	}
...
#	endif
	...
	{
		if (app_config.udp_cfg.bound_addr.slen)
		{
		...
		if (app_config.ipv6
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
			&& app_config.udp_cfg.bound_addr6.slen
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
			)
		{
...
/*			else
			...
			status = pjsua_transport_create(type, &udp_cfg, &transport6_id);
			if (status != PJ_SUCCESS)
			{
				goto on_error;
			}
			...
			if (app_config.udp_cfg.port == 0)
			{
				...
				pjsua_transport_get_info(transport6_id, &ti);
				a = (pj_sockaddr_in*)&ti.local_addr;
...
#	endif
	...
	if (transport_id == -1 && transport6_id == -1)
	{
	...
	for (i=0; i<app_config.acc_cnt; ++i)
	{
		...
		if (transport6_id != PJSUA_INVALID_ID)
			app_config.acc_cfg[i].transport6_id = transport6_id;

		if (PJSUA_INVALID_ID == transport_id)
			app_config.acc_cfg[i].transport_id = transport6_id;

#if defined(PJMEDIA_HAS_SRTP) && (PJMEDIA_HAS_SRTP != 0)
		app_config.acc_cfg[i].use_srtp = app_config.cfg.use_srtp;
#endif /* PJMEDIA_HAS_SRTP */
...
#	endif
	...
	if (status != PJ_SUCCESS)
	{
		printf("create media transport error!!!\n");
		goto on_error;
	}
...
pj_status_t create_ipv6_media_transports(void)
{
	...
	pj_str_t *pBoundAddr;
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	pBoundAddr = &app_config.udp_cfg.bound_addr6;
#else
	pBoundAddr = &app_config.udp_cfg.bound_addr;
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
...
#	if defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && PJ_MEDIA_TRANSIT_BY_PJSIP!=0
		...
		if (status != PJ_SUCCESS)
		    break;
				...
				status = pj_sockaddr_init(pj_AF_INET6(), &bound_addr, pBoundAddr, (unsigned short)(port+k));
		if (status != PJ_SUCCESS)
		    break;
		
		status = pj_sock_setsockopt(sock[k], pj_SOL_SOCKET(), pj_SO_REUSEADDR(), 
						&on, sizeof(on));
...
pjsua_transport_config *app_get_rtp_cfg()
{
	return &app_config.rtp_cfg;
}
