 ...
 */
int get_ip_addr_ver(const pj_str_t *host)
{
    pj_in_addr dummy;
    pj_in6_addr dummy6;
	...
	*/
    if (ip_addr_ver || resolver->res == NULL)
	{
		...
		if (ip_addr_ver != 0)
		{
		    ...
		    if (ip_addr_ver == 4)
			{
		        if (af == pj_AF_INET6())
				{
	                ...
	                status = pj_getaddrinfo(pj_AF_INET6(), &target->addr.host, &count, ai);
			    	if (status == PJ_SUCCESS && count > 0 && ai[0].ai_addr.addr.sa_family == pj_AF_INET6())
					{
						pj_sockaddr_init(pj_AF_INET6(), &svr_addr.entry[0].addr, NULL, 0);
						svr_addr.entry[0].addr.ipv6.sin6_addr = ai[0].ai_addr.ipv6.sin6_addr;
			    	}
					else
					{
			        	pj_sockaddr_init(pj_AF_INET(), &svr_addr.entry[0].addr, NULL, 0);
			        	pj_inet_pton(pj_AF_INET(), &target->addr.host, &svr_addr.entry[0].addr.ipv4.sin_addr);
					}
				}
				else
				{
		...
		else
		{
			...
			/* Resolve */
#ifdef INCLUDE_ROBIN_DNS
			/* dingcheng-pjsip.We use only one addr info before. But now, in order to cache, we need more!	*/
...
#ifdef INCLUDE_ROBIN_DNS
			/* dingcheng-pjsip.Save all the ip addrs we get */
			for (i = 0; i < count; i++)
			{		
				pj_sockaddr_cp(&svr_addr.entry[i].addr, &ai[i].ai_addr);
				if (af == pj_AF_UNSPEC())
					af = ai[i].ai_addr.addr.sa_family;
			}
		...
		PJ_LOG(5,(THIS_FILE, 
			...
			svr_addr.entry[i].checked = PJ_FALSE;
			svr_addr.entry[i].addr_len = pj_sockaddr_get_len(&svr_addr.entry[i].addr);
		}
#else /* !INCLUDE_ROBIN_DNS */
...
//	PJ_TODO(SUPPORT_DNS_NAPTR);
...
#endif /* INCLUDE_ROBIN_DNS */
	{
#endif /* ifndef INCLUDE_SOL */
#endif /* ifndef INCLUDE_WISP_DE */
    ...
    if (query->query_type == PJ_DNS_TYPE_SRV)
	{
		...
		if (af == pj_AF_UNSPEC())
		    opt = PJ_DNS_SRV_FALLBACK_A | PJ_DNS_SRV_FALLBACK_AAAA | PJ_DNS_SRV_RESOLVE_AAAA;
...
#ifdef INCLUDE_ROBIN_DNS
		/** Address checked.-dingcheng Added in 2014.8.15 */
		srv->entry[srv->count].checked = PJ_FALSE;
#endif /* INCLUDE_ROBIN_DNS */
