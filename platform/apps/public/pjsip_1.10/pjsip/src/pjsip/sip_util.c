				 ...
				 pjsip_host_info *dest_info)
{
	...
	*/
	...
	if (PJSIP_URI_SCHEME_IS_SIPS(target_uri))
	{
		...
		if (PJSIP_TRANSPORT_UDP == dest_info->type)
		{
			if (6 == get_ip_addr_ver(&dest_info->addr.host))
				dest_info->type = PJSIP_TRANSPORT_UDP6;
		}
		else if (PJSIP_TRANSPORT_TCP == dest_info->type)
		{
			if (6 == get_ip_addr_ver(&dest_info->addr.host))
				dest_info->type = PJSIP_TRANSPORT_TCP6;
		}
#endif
	}
				  ...
				  const struct pjsip_server_addresses *addr)
{
	...
	*/
		...
		tdata->dest_info.addr.entry[0].type == PJSIP_TRANSPORT_UDP)
	{
		...
		FILE *file2 = fopen("/var/tmp/forbiddenAcc.txt","r");
		char forbiddenAcc2[1024]={0};
		if(file2 == NULL)
		{
		...
		{
			fgets(forbiddenAcc2, 1024,file2);
			fclose(file2);
			forbiddenAcc2[strlen(forbiddenAcc2)-1]='\0';
			if(tdata->buf.start != NULL)
			{
				if(strstr(tdata->buf.start, forbiddenAcc2) !=NULL)
				{
					return;
				}
				else
