...
#if  defined(INCLUDE_SOL)
    /* dingcheng-pjsip:add the forbidden IP list to avoid forbidden request */
    char		forbidden_list[FORBIDDEN_LIST_MAXSIZE][MAX_IPADDR_LEN];
    int 		forbiddenlist_num;
#endif	
 ...
 */
pj_status_t pjsip_endpt_forbiddenIPList_addItem(pjsip_endpoint *endpt,char * forbiddenIP)
{
	if (endpt->forbiddenlist_num >= FORBIDDEN_LIST_MAXSIZE)
	{
		cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, "the Forbidden IP list is full, add IP (%s) failed", 
						forbiddenIP);
		return (-1);
	}
	strcpy(endpt->forbidden_list[endpt->forbiddenlist_num],forbiddenIP);

	cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, "add IP (%s) into the Forbidden IP list ", 
						forbiddenIP);
	
	endpt->forbiddenlist_num++;
 ...
 */
pj_status_t pjsip_endpt_forbiddenIPList_checkItem(pjsip_endpoint *endpt,char * checkedIP)
{
	int i;
	for(i = 0 ; i < endpt->forbiddenlist_num; i++)
	{
		if(!strcmp(endpt->forbidden_list[i],checkedIP))
		{
...
#if  defined(INCLUDE_SOL)
	/* dingcheng-pjsip:check whether the packet is in forbidden IP list */
	if(pjsip_endpt_forbiddenIPList_checkItem(endpt, rdata->pkt_info.src_name))
	{
