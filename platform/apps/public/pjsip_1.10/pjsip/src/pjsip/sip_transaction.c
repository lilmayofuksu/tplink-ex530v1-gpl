...
static pj_status_t tsx_destroy( pjsip_transaction *tsx, int is_pending_destory);
 ...
 * liguanxiong add,
 *
 * the memory alloc for pjsip_transaction will completely free here.
 *
 */
static void tsx_destroy_pending_stub( pjsip_transaction *tsx )
{
	/* Clear TLS, so that mutex will not be unlocked */
    struct tsx_lock_data* lck_p = (struct tsx_lock_data*) pj_thread_local_get(pjsip_tsx_lock_tls_id);
    while (lck_p)
	 {
		if (lck_p->tsx == tsx)
		{
    ...
    PJ_LOG(4,(tsx->obj_name, "Transaction completely destroyed!"));
...
/* Destroy transaction. */
/* liguanxiong add, to avoid memory is stomped, make sure the tsx->mutex is unlocked before pjsip_transaction is completely destory. */
static pj_status_t tsx_destroy( pjsip_transaction *tsx, int is_pending_destory)
{
    struct tsx_lock_data *lck;
	...
	/*
	**  liguanxiong add.
	**  if is_pending_destory is set, we just return here and the memory alloc for pjsip_transaction
	**  will be free after tsx->mutex is unlocked by calling tsx_destroy_pending_stub().
	**/
	if (is_pending_destory) {
		tsx->is_pending_destory = 1;
		PJ_LOG(4,(tsx->obj_name, "Will completely destroy after tsx->mutex is unlocked in the future."));
		return PJ_SUCCESS;
	}
...
static void tsx_timer_callback( pj_timer_heap_t *theap, pj_timer_entry *entry)
{
	...
	if (tsx->is_pending_destory == 1)
	{
		tsx_destroy_pending_stub(tsx);
	}
				        ...
				        pjsip_tx_data *tdata )
{
	...
	/* liguanxiong add, pjsip_transtraction cleanup */
	if (tsx->is_pending_destory == 1)
	{
		tsx_destroy_pending_stub(tsx);
	}
	...
	*/
	if (tsx->transport)
	{
		CMSIP_PRINT("-----tsx has transport-remote(%.*s:%d)----------------",
			(int)tsx->transport->remote_name.host.slen, tsx->transport->remote_name.host.ptr,
			tsx->transport->remote_name.port);

#ifdef INCLUDE_SPEC_OPTUS
		FILE *file = fopen("/var/tmp/forbiddenAcc.txt","r");
		char forbiddenAcc[1024]={0};
		if(file ==NULL)
		{
		...
		{
			fgets(forbiddenAcc, 1024,file);
			fclose(file);
			forbiddenAcc[strlen(forbiddenAcc)-1]='\0';

			if(tdata->buf.start != NULL)
			{
				if(strstr(tdata->buf.start, forbiddenAcc) !=NULL)
				{
					status = PJ_SUCCESS;
				}
				else
...
CMSIP_PRINT("==========process transaction========================");
	...
	if (tsx->method.id == PJSIP_INVITE_METHOD) {
	    ...
	    if (status != PJ_SUCCESS)
		return status;
#ifdef INCLUDE_USER_AGENT_IN_ACK
		pjsua_process_msg_data(ack_tdata, NULL);
#endif /*INCLUDE_USER_AGENT_IN_ACK*/
