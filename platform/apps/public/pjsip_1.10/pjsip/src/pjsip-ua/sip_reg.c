...
#ifdef INCLUDE_ROBIN_DNS
	/* dingcheng -pjsip. Before send the message,  
	  * we will check whether we have server IP cache .
	  * In the first Register request, the server_name will be empty.
	  * But later, we will use the cached IP.
	  */
	if(regc->srv_ipinfo.server_name.slen)
	{
		pj_strdup( tdata->pool, &tdata->dummy_dest_info.name, &regc->srv_ipinfo.server_name);
		memcpy(&tdata->dummy_dest_info.addr, &regc->srv_ipinfo.server_addr, sizeof(regc->srv_ipinfo.server_addr));
		tdata->dummy_dest_info.cur_addr = regc->srv_ipinfo.cur_addr;
	}
#endif /* INCLUDE_ROBIN_DNS */
					  ...
					  pjsip_tx_data **p_tdata)
{
	...
	pjsip_contact_hdr * pContactHdr = NULL;
	...
	/*<< BosaZhong@17Jun2014, add, unreg before reg. */
	if (regc->enableUseContactStar)
	{
		/* user "contact: *" */
		pContactHdr = (pjsip_contact_hdr *) pjsip_contact_hdr_create(tdata->pool);
		pContactHdr->star = 1;
		pjsip_msg_add_hdr(msg, (pjsip_hdr *)pContactHdr);
	}
	else
	/*>> BosaZhong@17Jun2014, add, unreg before reg. */
	{
...
#ifdef INCLUDE_ROBIN_DNS
	/*	dingcheng-pjsip.If regc has no IP cache and the domain name has been resolved, we save the IP infos from the tdata.
	  */
	if (0 == regc->srv_ipinfo.server_name.slen && tdata->dest_info.addr.count > 0)
	{
		pj_in_addr dummy;
		pj_in6_addr dummy6;
			...
			&& pj_inet_pton(pj_AF_INET6(), &tdata->dest_info.name, &dummy6) != PJ_SUCCESS)			
		{
			...
			pj_strdup(regc->pool, &regc->srv_ipinfo.server_name, &tdata->dest_info.name);
			memcpy(&regc->srv_ipinfo.server_addr, &tdata->dest_info.addr, sizeof(regc->srv_ipinfo.server_addr) );
			regc->srv_ipinfo.cur_addr = tdata->dest_info.cur_addr;
			cmsip_send_systemlog(CMSIP_SYSTEMLOG_DEBUG, "save IP info to regc for domain(%.*s) index(%d) total count (%d)", 
					regc->srv_ipinfo.server_name.slen,
					regc->srv_ipinfo.server_name.ptr,
					regc->srv_ipinfo.cur_addr,
					regc->srv_ipinfo.server_addr.count);
