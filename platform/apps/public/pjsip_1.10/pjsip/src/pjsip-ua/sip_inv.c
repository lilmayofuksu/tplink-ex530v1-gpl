...
#if defined(INCLUDE_WISP_DE) || defined(INCLUDE_TRTC)
static pj_status_t inv_check_sdp_true(pjsip_rx_data *rdata);
#endif /*defined(INCLUDE_WISP_DE) || defined(INCLUDE_TRTC)*/
...
#if defined(INCLUDE_WISP_DE) || defined(INCLUDE_TRTC)
		if ( statusCode == PJSIP_SC_PROGRESS)
		{
			if ( ! inv_check_sdp_true(rdata) )
			{
				printf("change status code from %d to %d\n", PJSIP_SC_PROGRESS, PJSIP_SC_RINGING);
				statusCode = PJSIP_SC_RINGING;
			}
		}
#endif /*defined(INCLUDE_WISP_DE) || defined(INCLUDE_TRTC)*/
...
static pj_status_t inv_send_ack(pjsip_inv_session *inv, pjsip_event *e)
{
	...
	pjsua_process_msg_data(inv->last_ack, NULL);
...
/*ycw-pjsip-enableT38*/
     ...
     * that feature too.
	 *
	 * Note: BosaZhong@24Oct2013, modify.
	 *   if we use "if ( rem_option & PJSIP_INV_SUPPORT_100REL || rem_option & PJSIP_INV_REQUIRE_100REL)"
	 *   there is a fax problem in tailand.
     */
...
CMSIP_PRINT("==========process call========================");
    ...
    PJ_ASSERT_ON_FAIL(tsx && dlg, return);
#ifdef INCLUDE_TRTC
	/* liuqianhao add:When device first receive 183 and second 180, voip_client will continur to
	 * 				  play early media stream but we should play local ringback tone, otherwise
	 *				  we cannot hear ring back tone, so now we have to send 180/183 event to cm.
	 */
	 if (PJSIP_EVENT_TSX_STATE == e->type &&
		PJSIP_EVENT_RX_MSG == e->body.tsx_state.type &&
		inv->cancelling != PJ_TRUE &&
...
CMSIP_PRINT("==========process call========================");
   ...
   PJ_ASSERT_ON_FAIL(tsx && dlg, return);
	...
	if (tsx->state == PJSIP_TSX_STATE_TRYING) {
		...
		CMSIP_PRINT("==============Check if we have INVITE pending===========================");
	    if (inv->invite_tsx && inv->invite_tsx!=tsx)
		{
		 ...
		 CMSIP_PRINT("==============can not process any re-INVITE===========================");
		...
		CMSIP_PRINT("==============Process session timers headers===========================");
		...
		CMSIP_PRINT("==============Process SDP ===========================");
#ifdef SUPPORT_METABOLIC_MEDIA_PORT
		pjmedia_endpt_unset_mediaPort(pjsua_var.med_endpt);
#endif
	...
	else if (tsx->status_code >= 300 && tsx->status_code < 700)
	{
	    ...
	    if (tsx == inv->invite_tsx)
	    {
 		 ...
 		 /*ycw-pjsip:send CMSIP_RESPONSE_SIP_HOLDSTATUS*/
		 /* Trunk svn pjsip#4573: modifed 03Sep2013.
		 ** 修复：当一个事务处理完远方的最终响应之后还要经历TERMINATE和DESTROY两个状态，
		 **   这都是由定时器触发的；如果拍叉之后未收到远端响应之前前面的事务刚好出发，就
		 **   会发送HOLD Failure的消息给CM，导致CM处理错误。
		 */
