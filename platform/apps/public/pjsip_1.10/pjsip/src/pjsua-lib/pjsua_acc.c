...
/*For HKBN customize*/
#ifdef INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL
#ifndef TIMER_ACTIVE
...
#endif
#endif /*INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL*/
...
extern pj_status_t create_ipv6_media_transports(void);

extern pjsua_transport_config *app_get_rtp_cfg(void);

#ifdef TRANSPORT_ADAPTER_SAMPLE
extern pj_status_t transport_adapter_sample(void);
#endif
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */

static void schedule_reregistration(pjsua_acc *acc);

static pjsua_transport_id get_next_tpid(pjsua_transport_id curr_tpid);
...
static pj_status_t initialize_acc(unsigned acc_id)
{
	...
	if (0 == getaddrinfo(proxy_domain, "sip", &hints, &ret))
	{
		host_ip_node* pNode = NULL;
		host_ip_node** ppNext = &acc->host_ip_list;

		for(ptr = ret; ptr; ptr = ptr->ai_next)
		{
			psin = (struct sockaddr_in *)ptr->ai_addr;
			psin6 = (struct sockaddr_in6 *)ptr->ai_addr;
			pNode = PJ_POOL_ZALLOC_T(acc->pool, host_ip_node);
			if (PJ_AF_INET == ptr->ai_addr->sa_family)
				inet_ntop(PJ_AF_INET, &psin->sin_addr, pNode->host_ip, sizeof(pNode->host_ip));
			else if (PJ_AF_INET6 == ptr->ai_addr->sa_family)
				inet_ntop(PJ_AF_INET6, &psin6->sin6_addr, pNode->host_ip, sizeof(pNode->host_ip));
			*ppNext = pNode;
			ppNext = &pNode->next;
		}
...
#	endif
	...
	for (i=0; i<acc_cfg->proxy_cnt; ++i)
	{
		...
		hints.ai_protocol = IPPROTO_UDP;
		ret = NULL;
		if (0 == getaddrinfo(outbound_proxy, "sip", &hints, &ret))
		{
			host_ip_node* pNode = NULL;
			host_ip_node** ppNext = &acc->secure_ip_list;
			for (ptr = ret; ptr; ptr = ret->ai_next)
			{
				psin = (struct sockaddr_in *)ptr->ai_addr;
				psin6 = (struct sockaddr_in6 *)ptr->ai_addr;
				pNode = PJ_POOL_ZALLOC_T(acc->pool, host_ip_node);
...
#	endif
	...
	/*<< BosaZhong@17Jun2014, add, unreg before reg. */
	pjsua_var.acc[acc_id].forceUnregBeforeRegStartFlag = 0;
	/*>> endof BosaZhong@17Jun2014, add, unreg before reg. */
...
#	endif
	...
	PJ_LOG(4,(THIS_FILE, "[%s:%d]Account %.*s added with id %d", __FUNCTION__,__LINE__,(int)cfg->id.slen, cfg->id.ptr, id));

	/* If accounts has registration enabled, start registration */
	/*ycw-pjsip*/
	if (isValid && pjsua_var.acc[id].cfg.reg_uri.slen)
	{	
		/*<< BosaZhong@16Jun2014, add, unreg firstly. */
		if (pjsua_var.acc[id].cfg.unregBeforeReg)
		{
			CMSIP_PRINT("start reg\n");
			pjsua_var.acc[id].forceUnregBeforeRegStartFlag = 1;
			status = pjsua_acc_set_registration(id, PJ_FALSE, PJ_FALSE);
		}
		/*>> endof BosaZhong@16Jun2014, add, unreg firstly. */
		else
		{
			CMSIP_PRINT("start reg\n");
			//pjsua_acc_registration_manual(id, PJ_FALSE);
			status = pjsua_acc_set_registration(id, PJ_TRUE, PJ_FALSE);	
		}
	}
...
#endif
	/*
	 * Liuqianhao: Cancel forbidden timer when account is deleted
	 */
#ifdef INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL
	if(pjsua_var.acc[acc_id].forbiddenTimer.id)
	{
		pjsua_var.acc[acc_id].forbiddenTimer.id = TIMER_INACTIVE;
		pjsua_cancel_timer(&pjsua_var.acc[acc_id].forbiddenTimer);
	}
#endif /*INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL*/
...
#if 1
void* sockaddr_get_addr(const pj_sockaddr *a)
{
...
static const char *addr_string(const pj_sockaddr *addr)
{
    static char str[PJ_INET6_ADDRSTRLEN];
    pj_inet_ntop(addr->addr.sa_family, 
...
static void keep_alive_timer_cb(pj_timer_heap_t *th, pj_timer_entry *te)
{
	...
	pjsip_transport_type_e ptype = PJSIP_TRANSPORT_UDP;
    pj_status_t status;
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	type_name.ptr = tp_sel.u.transport->type_name;
	type_name.slen = strlen(tp_sel.u.transport->type_name);
    ptype = pjsip_transport_get_type_from_name(&type_name);

	pj_strset2(&addr, addr_string(&acc->ka_target));
	if (6 == get_ip_addr_ver(&addr))
	{
		ptype = PJSIP_TRANSPORT_UDP6;
	}
#endif
    PJ_LOG(5,(THIS_FILE,
...
//(ipv6 no need)	    && (param->rdata->tp_info.transport->key.type != PJSIP_TRANSPORT_UDP6)
		)
	)
	{
...
/*(ipv6 no need)
	pj_str_t addr;
	char addrtxt[PJ_INET6_ADDRSTRLEN]="2001:8d8:104:100:212:227:124:129";
	pj_strset2(&addr, addrtxt);
	pj_sockaddr_set_str_addr(PJ_AF_INET6, &acc->ka_target, &addr);
*/	
...
#ifdef INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL
/*
 *liuqianhao: callback for account 403 forbidden timer, execute after timeout
 * each account has individual timer.
*/
static void forbidden_timer_callback( pj_timer_heap_t *theap, pj_timer_entry *entry)
{
	pjsua_acc *acc = (pjsua_acc*)entry->user_data;

	acc->has_acc_forbidden = PJ_FALSE;
	CMSIP_PRINT("Here in forbidden_timer_callback, wait time gone, now set registration!");
	pjsua_acc_set_registration(acc->index, PJ_TRUE, PJ_FALSE);
}

/*
 *liuqianhao: function to set a new timer when receive 304 forbidden
 * in SIP registration.
*/
pj_status_t set_acc_forbidden_timer(pjsua_acc *acc)
{
	pj_time_val delay;
	acc->forbiddenTimer.user_data = acc;
	acc->has_acc_forbidden = PJ_TRUE;
	acc->forbiddenTimer.id = TIMER_ACTIVE;
	acc->forbiddenTimer.cb = &forbidden_timer_callback;
	delay.sec = INCLUDE_SIP_REG_FORBIDDEN_INTERVAL_FIXED + rand() % INCLUDE_SIP_REG_FORBIDDEN_INTERVAL_RANDOM;
	delay.msec = 0;
	CMSIP_PRINT("Forbidden! Now account (%d) wait for (%d) seconds until next reregister!",acc->index, delay.sec);
	return pjsip_endpt_schedule_timer(pjsua_var.endpt, &acc->forbiddenTimer, &delay);
}
#endif /*INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL*/
...
#	endif
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	pjsua_transport_data *tpdata = NULL;
	char *ipaddr = NULL;
	int family = AF_INET;
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
	...
	/*<< BosaZhong@17Jun2014, add, unreg before reg. */
	if (acc->cfg.unregBeforeReg && acc->forceUnregBeforeRegStartFlag && acc->valid)
	{
		acc->forceUnregBeforeRegStartFlag = 0;
		acc->regc->enableUseContactStar = 0;
		pjsip_regc_destroy(acc->regc);
		acc->regc = NULL;

		/* start the real register */
		pjsua_acc_set_registration(acc->index, PJ_TRUE, PJ_FALSE);
		PJSUA_UNLOCK();
		return ;
	}
...
#	endif
#ifdef INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL
		/*liuqianhao: start forbidden timer for account*/
		if(param->code == PJSIP_SC_FORBIDDEN)
		{
			if(set_acc_forbidden_timer(acc) != PJ_SUCCESS)
			{
				PJSUA_UNLOCK();
				return;
			}
...
#ifdef INCLUDE_ROBIN_DNS
		/* Change the checked label ,it means this entry is unusable */
		if (pjsua_acc_try_next_ip(acc->index, 0 ))
		{
			PJSUA_UNLOCK();
			return ;
		}
#endif /* INCLUDE_ROBIN_DNS */
		pjsip_regc_destroy(acc->regc);
		acc->regc = NULL;
		acc->contact.slen = 0;
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
...
#	endif
		...
		/* Stop keep-alive timer if any. */
		update_keep_alive(acc, PJ_FALSE, NULL);
#ifdef INCLUDE_SPEC_OPTUS
		if(param->code == 403)
		{
			FILE *file = fopen("/var/tmp/forbiddenAcc.txt","w+");
			fputs(pjsua_var.acc[acc->index].cfg.id.ptr, file);
			fclose(file);		 
		}
#endif
#ifdef INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL
	...
	else if (PJSIP_IS_STATUS_IN_CLASS(param->code, 200))
	{
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
		if (PJSUA_INVALID_ID != acc->cfg.transport_id)
		{
			pjsip_transport *tmptp;
			pjsip_transport_type_e tp_type;
			pj_str_t type_name;
			
			tpdata = &pjsua_var.tpdata[acc->cfg.transport_id];
			tmptp = tpdata->data.tp;
			type_name.ptr = tmptp->type_name;
			type_name.slen = strlen(tmptp->type_name);

#if 0
			tp_type = pjsip_transport_get_type_from_name(&type_name);
#else
			if ( tpdata->type & PJSIP_TRANSPORT_IPV6 )
			{
			...
			ipaddr = tpdata->local_name.host.ptr;
			if (PJSIP_TRANSPORT_UDP == tp_type)
			{
				family = pj_AF_INET();
			}
			else if (PJSIP_TRANSPORT_UDP6 == tp_type)
			{
				family = pj_AF_INET6();
			}
		}
		...
		if (param->expiration < 1)
		{
			...
			/*ycw-pjsip:send CMSIP_SIP_REGSTATUS*/
			cmsip_send_acc_regStatus(acc->cmAcctIndex, CMSIP_UNREG_STATE_SUCCESS
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
			, 0
#	endif
#ifdef INCLUDE_RFC3325
...
#	endif
	...
	*/
	...
	{
		pjsua_transport_id tpid = get_next_tpid(acc->cfg.transport_id);
		
		cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, 
			"register fail, response code(%d)", param->code);

		if (PJSUA_INVALID_ID != tpid)
			pjsua_acc_set_transport(acc->index, tpid);
...
static pj_status_t pjsua_regc_init(int acc_id)
{
	...
	*/
	if (pjsua_var.acc[acc_id].cfg.transport_id != PJSUA_INVALID_ID)
	{
		pjsip_tpselector tp_sel;
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
		if (PJSUA_INVALID_ID != pjsua_var.acc[acc_id].cfg.transport_id)
		{
			pjsua_init_tpselector(pjsua_var.acc[acc_id].cfg.transport_id, &tp_sel);
		}
		else
		{
			pjsua_init_tpselector(pjsua_var.acc[acc_id].cfg.transport6_id, &tp_sel);
		}
#else
		pjsua_init_tpselector(pjsua_var.acc[acc_id].cfg.transport_id, &tp_sel);
#endif
...
#	endif
	...
	/*
	 * Liuqianhao: Cancel forbidden timer to release resource, otherwise when re-registration
	 * success, this timer could not release
	 */
#ifdef INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL
	if(acc->forbiddenTimer.id)
	{
		acc->forbiddenTimer.id = TIMER_INACTIVE;
		pjsua_cancel_timer(&acc->forbiddenTimer);
	}
#endif /*INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL*/
	...
	if (renew)
	{
		...
		cmsip_send_regOrUnreg(acc->cmAcctIndex, CMSIP_SIP_REGISTERING);
	}
	else
	{
		if (manual)
...
#	endif
		...
		/*<< BosaZhong@17Jun2014, add, unreg before reg. */
		if (acc->valid && acc->cfg.unregBeforeReg && acc->forceUnregBeforeRegStartFlag && acc->regc)
		{
			acc->regc->enableUseContactStar = 1;
		}
		/*>> endof BosaZhong@17Jun2014, add, unreg before reg. */
		
		status = pjsip_regc_unregister(acc->regc, &tdata);

		cmsip_send_regOrUnreg(acc->cmAcctIndex, CMSIP_SIP_UNREGISTERING);
	}
...
#	endif
	...
	char linenumber[MAX_URI_LEN]={0};

	PJSUA_LOCK();

	af = rdata->tp_info.transport->local_addr.addr.sa_family;	
	...
	{
		CMSIP_PRINT("This is a INVITE request---");
		isForward = pjsua_isForwardCall(rdata->msg_info.msg, &isIpCall, (int)rdata->tp_info.transport->local_addr.addr.sa_family);
#	if defined(SUPPORT_IPCALL_NO_ACCOUNT) && SUPPORT_IPCALL_NO_ACCOUNT!=0
		if (ipCall)
		{
			*ipCall = isIpCall;
...
#	endif
	...
	/*get request uri's username*/
	pjsua_parseStrSipUri2(req_sip_uri, &strReqUri);
	CMSIP_PRINT("Request:type(%d), username(%s), host(%s), port(%s)", strReqUri.type,
		strReqUri.username, strReqUri.host, strReqUri.port);
	...
	/*Get uri of To header*/
	pjsua_parseStrSipUri2(to_uri, &strToUri);	
	CMSIP_PRINT("To:type(%d), username(%s), host(%s), port(%s)", strToUri.type,
		strToUri.username, strToUri.host, strToUri.port);
	...
	/*get host part of from header*/
	pjsua_parseStrSipUri2(from_uri, &strFromUri);
	CMSIP_PRINT("From:type(%d), username(%s), host(%s), port(%s)", strFromUri.type,
		strFromUri.username, strFromUri.host, strFromUri.port);
	...
	if (via)
	{
		if (strchr(via->sent_by.host.ptr, ':'))
		{
			begin_token = "[";
			end_token = "]";
		}
		if (via->sent_by.port > 0)
		{
			sprintf(bufVia, "%s%.*s%s:%d", begin_token, (int)via->sent_by.host.slen, via->sent_by.host.ptr, end_token, via->sent_by.port);
		}
		else
		{
			sprintf(bufVia, "%s%.*s%s:5060", begin_token, (int)via->sent_by.host.slen, via->sent_by.host.ptr, end_token);
		}
	}
	CMSIP_PRINT("via:%s", bufVia);
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	else if (PJ_AF_INET6 == af)
	{
		memcpy(boundIp, pjsua_var.BoundIp6.ptr, pjsua_var.BoundIp6.slen);
		boundIp[pjsua_var.BoundIp6.slen] = '\0';
	}
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
...
#	endif
		...
		strAorUri.username, strAorUri.host, strAorUri.port);
						...
						strncpy(linenumber,acc->cfg.linenumber.ptr, acc->cfg.linenumber.slen+1);

						if (((!strcmp(strAorUri.username, strReqUri.username)) ||
							(!strcmp(linenumber, strReqUri.username)))
#if  defined(INCLUDE_SOL)
				...
				/* dingcheng-pjsip:add this IP into forbidden list */
 				pjsip_endpt_forbiddenIPList_addItem(pjsua_get_var()->endpt,rdata->pkt_info.src_name);
#endif	
...
#	endif
		...
		strAorUri.username, strAorUri.host, strAorUri.port);
						...
						if (idlen <= reqlen)
						{
							if (!strcmp(strAorUri.username, &strReqUri.username[reqlen-idlen]))
							{
								if (acc_id < 0)
								{
									acc_id = tmp_acc_id;
						...
						{
							if (strstr(strAorUri.username, strReqUri.username) != NULL)
							{
								if (acc_id < 0)
								{
									acc_id = tmp_acc_id;
...
#else
						strncpy(accName, strAorUri.username, sizeof(accName));
						if (idlen > reqlen)
						{
							pjsua_stripDelForTelUri(accName);
							idlen = strlen(accName);
						}

						if (idlen <= reqlen && !strcmp(accName, &strReqUri.username[reqlen-idlen]))
						{
							if (acc_id < 0)
							{
								acc_id = tmp_acc_id;
...
#	endif
		...
		strAorUri.username, strAorUri.host, strAorUri.port);
						...
						if (((!strcmp(strAorUri.username, strToUri.username)) || 
							(!strcmp(linenumber, strToUri.username)))
#if  defined(INCLUDE_SOL)
							&& pjsua_matchInAccSecureIpList(tmp_acc_id, rdata->pkt_info.src_name)
...
#	endif
	...
	strAorUri.username, strAorUri.host, strAorUri.port);
					...
					if (idlen <= tolen)
					{
						if (!strcmp(strAorUri.username, &strToUri.username[tolen-idlen]))
						{
							if (acc_id < 0)
							{
								acc_id = tmp_acc_id;
					...
					{
						if (strstr(strAorUri.username, strToUri.username) != NULL)
						{
							if (acc_id < 0)
							{
								acc_id = tmp_acc_id;
					...
					if (idlen <= tolen && !strcmp(accName, &strToUri.username[tolen-idlen]))
					{
						if (acc_id < 0)
						{
							acc_id = tmp_acc_id;
...
#ifdef INCLUDE_ROBIN_DNS
pj_bool_t pjsua_acc_try_next_ip(pjsua_acc_id acc_id, int renew)
{
	...
	if (acc->regc)
	{
		PJ_LOG(2, (THIS_FILE, " acc->regc->srv_ipinfo (%.*s) cur addr %d", 
				acc->regc->srv_ipinfo.server_name.slen,acc->regc->srv_ipinfo.server_name.ptr,
				acc->regc->srv_ipinfo.cur_addr));

		/* dingcheng-pjsip.If the server domain has more than one IP cache,we will have last try! */
		if (acc->regc->srv_ipinfo.server_addr.count > 0)
		{
			/* Change the checked label ,it means this entry is unusable */
			acc->regc->srv_ipinfo.server_addr.entry[acc->regc->srv_ipinfo.cur_addr].checked = PJ_TRUE;

			/* we should check whether every IP of the domain has been test! */
			 for (index = 0; index < acc->regc->srv_ipinfo.server_addr.count; ++index)
		 	{
				/* This IP hasn't been tested , so we try it!! */
		 		if (!acc->regc->srv_ipinfo.server_addr.entry[index].checked)
		 		{
					cmsip_send_systemlog(CMSIP_SYSTEMLOG_DEBUG, "register failed, try another IP:(%s) index:(%d) total count:(%d)",
						pj_sockaddr_print(&acc->regc->srv_ipinfo.server_addr.entry[index].addr, addr_str,sizeof(addr_str), 3),
						index,acc->regc->srv_ipinfo.server_addr.count);
					acc->regc->srv_ipinfo.cur_addr = index;
		 			break;
		 		}
			 }
			 if (index == acc->regc->srv_ipinfo.server_addr.count)
			 {
				cmsip_send_systemlog(CMSIP_SYSTEMLOG_WARN, "every IP of the domain has been tried, but we didn't find the correct one!");
				memset(&acc->regc->srv_ipinfo, 0, sizeof(acc->regc->srv_ipinfo));
				/*add by ydk for robindns*/
				schedule_reregistration(acc);
				return PJ_TRUE;
				/*end*/
			 }
#if 0
			 if (renew || !(index == acc->regc->srv_ipinfo.server_addr.count))
#endif
			 /*add by ydk for robindns*/
			 else if (renew || !(index == acc->regc->srv_ipinfo.server_addr.count))
			 /*end*/
			 {
...
#if !defined(INCLUDE_TFC_ES) 
		/*By YuChuwei, For Telefonica*/
	 	acc && !pj_list_empty(&acc->route_set))
#	else
		0)
#	endif
	 	)
	...
	else if (sip_uri->transport_param.slen == 0)
	{
		...
		if (acc_id >= 0)
		{
			pjsua_transport_id tpid = acc->cfg.transport_id;
			
			if (PJSUA_INVALID_ID != tpid)
			{
				pjsua_transport_data *tpdata = &pjsua_var.tpdata[tpid];
				tp_type = tpdata->type;
			}
		}
#else
	...
	* to use IPv6 as well.
	*/
#ifndef INCLUDE_VOIP_SUPPORT_IPV6
...
#	if defined(SUPPORT_IPCALL_NO_ACCOUNT) && SUPPORT_IPCALL_NO_ACCOUNT!=0
	if (NULL == acc)
	{
		...
		if (use_ipv6)
		{
			if (pjsua_var.BoundIp6.slen > 0)
			{
				contact->slen = pj_ansi_snprintf(contact->ptr, PJSIP_MAX_URL_SIZE,
						     "<%s:[%.*s]>",
						     (secure ? PJSUA_SECURE_SCHEME : "sip"),
						    (int) pjsua_var.BoundIp6.slen, pjsua_var.BoundIp6.ptr);	
			}
		}
		else
		{
...
#	endif
	...
	*contact = acc->cfg.force_contact;
    ...
    if (PJSIP_URI_SCHEME_IS_SIPS(sip_uri))
 	...
 	{
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
 		if ( pjsua_var.tpdata[acc->cfg.transport_id].type & PJSIP_TRANSPORT_IPV6 )
		{
			tp_type = PJSIP_TRANSPORT_UDP6;
		}
		else
     ...
     */
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
    if ((pj_strchr(&sip_uri->host, ':')) && (tp_type < PJSIP_TRANSPORT_IPV6))
#else
	if (pj_strchr(&sip_uri->host, ':'))
#endif
		tp_type = (pjsip_transport_type_e)(((int)tp_type) + PJSIP_TRANSPORT_IPV6);
...
static pjsua_transport_id get_next_tpid(pjsua_transport_id curr_tpid)
{
	...
	if (curr_tpid == PJSUA_INVALID_ID)
	{
		for (id = 0; id < PJ_ARRAY_SIZE(pjsua_var.tpdata); id++)
		{
			if (pjsua_var.tpdata[id].data.ptr)
			{
				found = 1;
				break;
			}
	...
	{	
		...
		while (id != curr_tpid)
		{
			...
			id++;
			id %= PJ_ARRAY_SIZE(pjsua_var.tpdata);
		}
	}
