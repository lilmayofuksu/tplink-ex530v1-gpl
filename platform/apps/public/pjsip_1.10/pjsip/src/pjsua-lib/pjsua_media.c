...
#	endif
		 ...
		 * 1. When user uses SRTP, med_orig refers to UDP transport which stores RTP packet address,
		 *    med_tp refers to SRTP transport which doesn't store any address info.
		 * 2. Otherwise, med_orig and med_tp both refer to UDP transport.
		 */
		call->med_orig = call->med_tp;
		if (PJMEDIA_SRTP_MANDATORY == srtp_opt.use)
		{
			call->med_tp = srtp;
		}
    }
...
/*ycw-pjsip. Compare two stream info*/
static pj_bool_t pjsua_media_stream_info_equal(pjmedia_stream_info* stream1, pjmedia_stream_info* stream2)
{
...
CMSIP_PRINT("------------create socket and register to ioqueue------------------");
	...
	pj_sockaddr rtcp_addr;
	pj_bool_t isIPv6 = PJ_FALSE;
#endif
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	if (pjsua_var.BoundIp6.slen)
	{
		isIPv6 = PJ_TRUE;
		PJ_LOG(1,(THIS_FILE, "pjsua_media_transport_create_for_single_call IPV6"));	
	}
	else
	{
		isIPv6 = PJ_FALSE;
		PJ_LOG(1,(THIS_FILE, "pjsua_media_transport_create_for_single_call IPV4"));	
	}
#endif
	...
	if (isIPv6)
	{
		...
		pj_bzero(&rtcp_addr, sizeof(rtcp_addr));
		status = pj_sockaddr_init(pj_AF_INET6(), &rtp_addr, &cfg->bound_addr6, rtp_port);
		if (status != PJ_SUCCESS)
		{
			pjsua_perror(THIS_FILE, "Unable to resolve transport rtp address", status);
			return status;
		}
		status = pj_sockaddr_init(pj_AF_INET6(), &rtcp_addr, &cfg->bound_addr6, rtcp_port);
		if (status != PJ_SUCCESS)
		{
			pjsua_perror(THIS_FILE, "Unable to resolve transport rtcp address", status);
			return status;
		}
		/* Create RTP socket. */
		status = pj_sock_socket(pj_AF_INET6(), pj_SOCK_DGRAM(), 0, &sock[0]);
	}
	else
#endif
	{
	...
	/*<< BosaZhong@20Aug2013, add, set mark for usbvm rtp */
	/* 0x61 is the same with TC_VOIP_MARK which defined in oal_tc.h */
	int rtpmark = pjsua_var.fwMark; 
	status = pj_sock_setsockopt(sock[0], pj_SOL_SOCKET(), pj_SO_MARK(),
					&rtpmark, sizeof(rtpmark));
	/*>> endof BosaZhong@20Aug2013, add, set mark for usbvm rtp */

	/* Bind RTP socket */
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	if (isIPv6)
	{
		status = pj_sock_bind(sock[0], &rtp_addr, pj_sockaddr_get_len(&rtp_addr));
	}
	else
#endif
	{
	...
	if (isIPv6)
	{
		status = pj_sock_socket(pj_AF_INET6(), pj_SOCK_DGRAM(), 0, &sock[1]);
	}
	else
#endif
	{
	...
	/*<< BosaZhong@20Aug2013, add, set mark for usbvm rtcp */
	/* 0x61 is the same with TC_VOIP_MARK which defined in oal_tc.h */
	int rtcpmark = pjsua_var.fwMark; 
	status = pj_sock_setsockopt(sock[1], pj_SOL_SOCKET(), pj_SO_MARK(),
					&rtcpmark, sizeof(rtcpmark));
	/*>> endof BosaZhong@20Aug2013, add, set mark for usbvm rtcp */

	/* Bind RTCP socket */
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	if (isIPv6)
	{
		status = pj_sock_bind(sock[1], &rtcp_addr, pj_sockaddr_get_len(&rtcp_addr));
	}
	else
#endif
	{
