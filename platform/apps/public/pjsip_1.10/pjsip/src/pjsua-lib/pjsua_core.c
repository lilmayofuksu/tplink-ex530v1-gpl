...
#	endif
{
    ...
    /* Add primary IP address */
	 ...
	 else if (pj_inet_pton(pj_AF_INET6(), defaultIp, &addr.ipv6.sin6_addr)== PJ_SUCCESS)
	 	seed = pj_hash_calc(seed, &addr.ipv6.sin6_addr, 16);
	 else
	 {
...
#	endif
		...
		if (pFwRule->dstBuf[0])
		{
			...
			pj_firewall_set_rule_accept(PJ_FIREWALLCFG_DEL, pFwRule->protocol, &pFwRule->destination, 
					NULL, pFwRule->dport, &pFwRule->source, NULL, pFwRule->sport, pj_firewall_get_fwtype(pj_AF_INET())
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
		pFwRule = &pjsua_var.fwRule6;
		if (pFwRule->dstBuf[0])
		{

		pj_firewall_set_rule_accept(PJ_FIREWALLCFG_DEL, pFwRule->protocol, &pFwRule->destination, 
					NULL, pFwRule->dport, &pFwRule->source, NULL, pFwRule->sport, pj_firewall_get_fwtype(pj_AF_INET6())
					, pj_AF_INET6()
					,pjsua_var.tcMark
				);
		}
...
{
    static char str[128];
	int af = ((const pj_sockaddr*)addr)->addr.sa_family;
				       ...
				       pj_sockaddr *p_pub_addr)
{
    ...
    if (pj_AF_INET() == af && cfg->bound_addr.slen)
	 {
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	else if (pj_AF_INET6() == af && cfg->bound_addr6.slen)
	{
		CMSIP_PRINT("++++++++this is ipv6 sip++++++++\n");
		status = pj_sockaddr_init(af, &bind_addr, &cfg->bound_addr6, 
				  (pj_uint16_t)port);
		if (status != PJ_SUCCESS)
		{
	   		pjsua_perror(THIS_FILE, 
	...
	{
		if (0 != pjsua_var.fwMark)
		{
			if(PJ_SUCCESS != (status = pj_sock_setsockopt(sock, pj_SOL_SOCKET(), pj_SO_MARK(), 
				&pjsua_var.fwMark, sizeof(pjsua_var.fwMark))))
			{
				pjsua_perror(THIS_FILE, "setsockopt set mark error", status);
			}
		}
	}
					    ...
					    pjsua_transport_id *p_id)
{
	...
	/*ycw-firewall*/
 	#if defined(PJSIP_HAS_FIREWALL_FILTER)&& PJSIP_HAS_FIREWALL_FILTER!=0
	PJ_FIREWALL_RULE* pFwRule = &pjsua_var.fwRule;	 
	#endif
	PJSUA_LOCK();
	...
	if (type==PJSIP_TRANSPORT_UDP || type==PJSIP_TRANSPORT_UDP6)
	{
		...
		*/
		CMSIP_PRINT("=========udp listener ip[%.*s], port[%d]\n", 
							(int)cfg->bound_addr.slen, cfg->bound_addr.ptr, cfg->port);
		if (PJSIP_TRANSPORT_UDP == type && cfg->bound_addr.slen)
		{
		...
		pFwRule->sport = -1;
			pFwRule->family = pj_AF_INET();
		}
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
		if (PJSIP_TRANSPORT_UDP6 == type && cfg->bound_addr6.slen)

		{
			CMSIP_PRINT("+++++++++++set ipv6 iptable++++++\n");
			status = pj_firewall_set_rule_accept(PJ_FIREWALLCFG_ADD, PJ_TRANSPORT_UDP, &cfg->bound_addr6,
						NULL, cfg->port, NULL, NULL, -1, pjsua_var.fwType6
						, pj_AF_INET6()
						,pjsua_var.tcMark
				);
			if (status != PJ_SUCCESS)
			...
			memset(pFwRule, 0, sizeof(PJ_FIREWALL_RULE));
			pFwRule->protocol = PJ_TRANSPORT_UDP;
			sprintf(pFwRule->dstBuf, "%.*s", (int)cfg->bound_addr6.slen, cfg->bound_addr6.ptr);
			pFwRule->destination = pj_str(pFwRule->dstBuf);
			sprintf(pFwRule->dstNetMaskBuf, "%.*s", (int)cfg->bound_addr_netmask6.slen, cfg->bound_addr_netmask6.ptr);
			pFwRule->dstNetmask = pj_str(pFwRule->dstNetMaskBuf);
			pFwRule->dport = cfg->port;
			pFwRule->sport = -1;
			pFwRule->family = pj_AF_INET6();
		}
#endif /* INCLUDE_VOIP_SUPPORT_IPV6 */
		#endif
	...
	*/
	CMSIP_PRINT("=========tcp listener ip[%.*s], port[%d]\n", 
						(int)cfg->bound_addr.slen, cfg->bound_addr.ptr, cfg->port);
	if (PJSIP_TRANSPORT_TCP == type && cfg->bound_addr.slen)
	{
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	if (PJSIP_TRANSPORT_TCP6 == type && cfg->bound_addr6.slen)
	{
		status = pj_firewall_set_rule_accept(PJ_FIREWALLCFG_ADD, PJ_TRANSPORT_TCP, &cfg->bound_addr6,
					NULL, cfg->port, NULL, NULL, -1, pjsua_var.fwType6
					, pj_AF_INET()
					, pjsua_var.tcMark
					);
	...
	*/
	...
	{
	status = pj_firewall_set_rule_accept(PJ_FIREWALLCFG_ADD, PJ_TRANSPORT_TCP, &cfg->bound_addr,
					NULL, cfg->port, NULL, NULL, -1, pjsua_var.fwType, pjsua_var.tcMark);
	if (status != PJ_SUCCESS)
	{
		pjsua_perror(THIS_FILE, "Error create Netfilter rule", status);
		goto on_return;
    ...
    /* auto re-registration does not work if application does not set on_transport_state(), see trac.pjsip.org #1550 */
					      ...
					      pjsua_transport_info *info)
{
    ...
    if ((pjsua_var.tpdata[id].type == PJSIP_TRANSPORT_UDP) || (pjsua_var.tpdata[id].type == PJSIP_TRANSPORT_UDP6))
	{
