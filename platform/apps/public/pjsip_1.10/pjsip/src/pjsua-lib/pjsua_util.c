...
{ 
	...
	tmp.ptr = ps;
	tmp.slen = strlen(ps);
	return pj_inet_pton(af, &tmp, (void *)sin_buf);         
} 
	...
	else
	{
		if (isIp(strSipUri))
		{
			...
			if (*p == '[')
			{
				++p;
				p1 = strchr(p, ']');
				memcpy(pParsedUri->host, strSipUri, p1 - p);
...
int pjsua_isForwardCall(pjsip_msg* msg, pj_bool_t* isIpCall, int af)
{
	...
	*isIpCall = PJ_FALSE;
	...
	if (pBoundIp->slen <= 0)
	{		
		PJ_LOG(4, (THIS_FILE, "empty bound ip address"));
		return -1;
	}
		...
		if (PJSUA_STR_SIP_URI_NOMAL == strToUri.type)
		{
			if (strcmp(strReqUri.username, strToUri.username) ||
					(strcmp(strReqUri.host, strToUri.host) && 
						strncmp(strReqUri.host, pBoundIp->ptr, pBoundIp->slen) &&
						strncmp(strToUri.host, pBoundIp->ptr, pBoundIp->slen)) ||
					...
					)
				{
					...
					if (0==strcmp(strAOR.username, strToUri.username) &&
							(0==strcmp(strAOR.host, strToUri.host) ||
								...
								0==strncmp(strToUri.host, pBoundIp->ptr, pBoundIp->slen)) &&
...
void pjsua_parseStrSipUri2(const pjsip_sip_uri *req_sip_uri, PJSUA_STR_SIP_URI* pParsedUri)
{
	...
	{
		pParsedUri->type = PJSUA_STR_SIP_URI_NOMAL;
		strncpy(pParsedUri->username, req_sip_uri->user.ptr, req_sip_uri->user.slen);
	}
	else
	{
		memcpy(buf, req_sip_uri->host.ptr, req_sip_uri->host.slen);
		buf[req_sip_uri->host.slen] = '\0';
		if (isIp(buf))
			pParsedUri->type = PJSUA_STR_SIP_URI_IP;
		else
			pParsedUri->type = PJSUA_STR_SIP_URI_UNKNOWN;
	...
	strncpy(pParsedUri->host, req_sip_uri->host.ptr, req_sip_uri->host.slen);
	sprintf(pParsedUri->port, "%d", req_sip_uri->port > 0 ? req_sip_uri->port : 5060);
}