										...
										pj_timer_entry *entry)
{
	...
	for (i = 0; i < pjsua_var.acc_cnt; ++i)
	{
		...
		CMSIP_PRINT("account (%d) has forbidden (%d)", i, acc->has_acc_forbidden);
...
#	endif
#ifdef INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL
			/*liuqianhao:if forbidden, stop account auto re-registration*/
			&& !acc->has_acc_forbidden
#endif /*INCLUDE_SIP_REG_FORBIDDEN_RANDOM_INTERVAL*/
			)
		{
...
#	endif
	...
	{
		if (1 == pjsua_isForwardCall(msg, &isIpCall, af))
		{
...
#if defined(INCLUDE_RFC3325) && !defined(INCLUDE_TRTC)
	if (acc)
	{
...
#endif /* defined(INCLUDE_RFC3325) && !defined(INCLUDE_TRTC) */
...
#	ifdef SUPPORT_NORMAL_CALL_WHEN_INVALID_CODEC_IN_OFFER
					,supportLiteCodecs
#	endif
	 , (int)rdata->tp_info.transport->local_addr.addr.sa_family);
	   ...
	   pjsua_media_channel_deinit(call->index);
		...
		for (i = 0; i < PJ_ARRAY_SIZE(call->fwRule); ++i)
		{
			...
			if (pFwRule->destination.slen > 0 || pFwRule->dport > 0)
			{
				...
				&pFwRule->source, &pFwRule->srcNetmask, pFwRule->sport, pj_firewall_get_fwtype(pFwRule->family)
...
CMSIP_PRINT("--local media name[%.*s], remote media name[%.*s]",
				(int)local_sdp->media[0]->desc.media.slen, local_sdp->media[0]->desc.media.ptr,
				(int)remote_sdp->media[0]->desc.media.slen, remote_sdp->media[0]->desc.media.ptr);
...
static pjsip_uri*	getUriByReferto(const pj_pool_t* pool, const pjsip_generic_string_hdr* referto)
{
	...
	pj_strdup_with_null((pj_pool_t *)pool, &tmp_referto, &referto->hvalue);
	referto_uri = pjsip_parse_uri((pj_pool_t *)pool, tmp_referto.ptr, tmp_referto.slen, 0);
											...
											pj_bool_t* blindXfer, pj_bool_t lockDlg)
{
	...
	msg = PJ_POOL_ZALLOC_T((pj_pool_t *)pool, pjsip_msg);
	...
	if (PJSIP_URI_SCHEME_IS_SIP(refer_to) || PJSIP_URI_SCHEME_IS_SIPS(refer_to))
	{
		...
		while(param != &uri->header_param)
		{
			...
			hdr = (pjsip_hdr*)pjsip_parse_hdr((pj_pool_t *)pool, &param->name, param->value.ptr,
			        ...
			        pjsip_rx_data *rdata )
{
	...
	int af = rdata->tp_info.transport->local_addr.addr.sa_family;
...
#	endif
    ...
    if (pjsua_var.ua_cfg.cb.on_call_transfer_request)
    {
		(*pjsua_var.ua_cfg.cb.on_call_transfer_request)( tmp_pool,
							existing_call->index,
							refer_to_uri, 
							&code, &transferToSelf, &dstAcctId, af);
    }
						  ...
						  const pjsip_event *e)
{
...
#ifdef INCLUDE_VOIP_SUPPORT_IPV6
	if (PJ_AF_INET6 == call->med_rtp_addr.addr.sa_family)
	{
	...
	if (!pj_strcmp2(pBoundIp, targetUri))
	{
		PJSUA_UNLOCK();
		return PJSIP_REDIRECT_REJECT;
	}
