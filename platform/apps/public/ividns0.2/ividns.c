...
#ifdef M_DEBUG
#define IVIDNS_DEBUG(s, a...) fprintf(stderr, "ividns# Dbg: " s "\n", ##a), fflush(stderr)
#define IVIDNS_MSG(s, a...) fprintf(stderr, "ividns# Msg: " s "\n", ##a), fflush(stderr)
#define IVIDNS_ERR(s, a...) \
		fprintf(stderr, "ividns# file: %s;line: %d; error = ", __FILE__, __LINE__), \
		perror(""); fprintf(stderr, "mldProxy# Err: " s "\n", ##a), fflush(stderr)

#else
#define IVIDNS_DEBUG(s, a...)
#define IVIDNS_MSG(s, a...)
#define IVIDNS_ERR(s, a...) \
		fprintf(stderr, "ividns# file: %s;line: %d; error = ", __FILE__, __LINE__), \
		perror(""); fprintf(stderr, "mldProxy# Err: " s "\n", ##a), fflush(stderr)
...
namecpy(void *src, void *dsc, uint32_t dlen, struct pointer_list *labelplist_p)
{
	...
	while (1)
	{
		...
		{
			IVIDNS_MSG("label starts with neither 00 or 11");
			return -1;
		}
	}
	return n;
...
rrcpy(void **psrc, void **pdsc, void *edsc, struct pointer_list *labelplist_p, struct offset_list *a_list_p, const void *som, struct in6_addr *prefixp, int prefixlen)
{
	...
	*pdsc += 10;
	...
	{
		case 1:  //A record
		/*	remove by lwj, for map, don't convert type A to AAAA
		*typep = htons(28);
			ap = *psrc;
...
detect_msg(void *src, uint16_t *qtype)
{
	...
	ancnt = ntohs(*(p + 3));
	p2 += 12;
	IVIDNS_DEBUG("pdcnt:%d,  ancnt:%d",(int)qdcnt,(int)ancnt);
	if (qdcnt != 1)
		return -1;
	p2 = jmp_name(p2);
	if (p2 == NULL)
...
//	struct in6_addr *p;
	...
	while(1)
	{
		...
		if (fork() == 0)
		{
			...
			{
				IVIDNS_MSG("send socket generation err");
				exit(1);
			}	
			struct timeval tv;
			tv.tv_sec = 5;
			...
			if (n2 <= 0){
				IVIDNS_MSG("recv from dns server Err");
				exit(1);
			}
			if(id != ntohs(*((uint16_t *)mesg_rec)))
			{
				IVIDNS_MSG("recv dns id not equal to send id");
				exit(1);
			}
			r = detect_msg(mesg_rec, &qtype);
			if (r < 0){
				IVIDNS_MSG("pls check the funtion detect_msg()");
				exit(1);
			}
			if ( r == 1 || qtype != 28)
			{
...
main(int argc, char **argv)
{
		...
		switch (r) {
			case 'p':
				...
				if (cp != NULL)
				{
					...
					{
							IVIDNS_ERR("Prefix lengh should be set btw 0 and 96\n");
							exit(1);
					}
				}
				else
	...
	{
		IVIDNS_ERR("please check your prefix:%s\n", optarg);
		exit(1);
	}
	if (argc - optind != 1)
	{
		IVIDNS_ERR("optind= %d",optind);
		print_usage();
		exit(1);
	}
	/**********INITIALIZE**********/
	...
	{
		IVIDNS_ERR("please check the dns address");
		exit(1);
	}
	...
	{
		IVIDNS_ERR("recv socket generation err");
		exit(1);
	}
