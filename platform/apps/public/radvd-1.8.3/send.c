...
#ifndef INCLUDE_IPV6_CE_ROUTER_AUTH
			/*  
			 * If defined CE_Router macro,
			 * then only one route6 default item written by kernel could exist.
			 * Its metric is 1024, so we should omit this judge condition.
			 */
...
send_ra(struct Interface *iface, struct in6_addr *dest)
{
	...
	if (iface->cease_adv) {
        ...
        /* To fix bug 359178, we don't need to check ipv6 default route of WAN.  modifiy by Lai Daokuan, 2020.4.14 */
#if 0 
		radvert->nd_ra_router_lifetime = (!check_ip6_forwarding() && check_ip6_dftroute())
											? htons(iface->AdvDefaultLifetime) : 0;
#else
		radvert->nd_ra_router_lifetime = (!check_ip6_forwarding()) ? htons(iface->AdvDefaultLifetime) : 0;
#endif
        /* Lai Daokuan modify end. */
	}
...
#else
				//2018/08/04 Bug 237623 - valid lifetime is not zreo when ip change.
				if (iface->cease_adv)
				{
