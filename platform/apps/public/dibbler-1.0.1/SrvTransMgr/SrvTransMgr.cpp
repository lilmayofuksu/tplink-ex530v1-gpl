...
bool TSrvTransMgr::unicastCheck(SPtr<TSrvMsg> msg) {
	...
	/* add by lwj 23Dec15, For RFC3315, recivice Unicast packets, 
	 * drop packets when message type is SOLICIT,CONFIRM, REBIND;
	 * Return UseMulticast Status Code, when Message is REQUEST,RENEW,RELEASE,DECLINE;
	 */
	SPtr<TMsg> question, answer; // question and answer
    question = (Ptr*) msg;
	...
	case CONFIRM_MSG:
	case REBIND_MSG:{
		Log(Info) << "Messages "<< msg->getType() << " use unicast address,drop it" << LogEnd;
		return false;
		break;
	}
	...
	case RELEASE_MSG:
	case DECLINE_MSG:{
		 Log(Info) << "Messages "<< msg->getType() << " use unicast address, send UseMulticast" << LogEnd;
    TOptList options;
    	 SPtr<TOpt> status(new TOptStatusCode(STATUSCODE_USEMULTICAST, string("Please send your message to multicast, not to ")
                                         + msg->getLocalAddr()->getPlain(), NULL));
    options.push_back(status);

    // Message will be sent in the constructor
    	 answer = new TSrvMsgReply((Ptr*)msg, options);

		 /* send Packet*/
		 sendPacket((Ptr*)answer);
		 return false;
		 break;
	}	 
	default:
