    ...
    :TMsg(iface, addr, buf, bufSize), forceMsgType_(0), physicalIface_(iface)
{
    ...
    while (pos<bufSize)	{
        ...
        if (!allowOptInMsg(MsgType,code)) {
			...
			/* For Server ID is not allow in some message,we need add it to option, 
			 * in order to do some opearation in reply Messag, add by lwj, 24Dec15 */
			 if (((MsgType == SOLICIT_MSG || MsgType == CONFIRM_MSG || MsgType == REBIND_MSG) && code == OPTION_SERVERID)
				|| (MsgType == INFORMATION_REQUEST_MSG && code == OPTION_IA_NA)
			 ){
				/*nothing to do*/
			 }else {
            continue;
