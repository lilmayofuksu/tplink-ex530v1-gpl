...
class TClntIfaceIface: public TIfaceIface {
 public:
	...
	/*add SOL_MAX_RT option and INF_MAX_RT option, by lwj*/
	bool setMaxSolicitTimeout(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, unsigned int value);
	bool setMaxInfRequestTimeout(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, unsigned int value);

	unsigned int getMaxSolicitTimeout();
	unsigned int getMaxInfRequestTimeout();
 ...
 private:
	...
	/* RFC7083, add by lwj */
	unsigned int MaxSolicitTimeout;
	unsigned int MaxInfRequestTimeout;
