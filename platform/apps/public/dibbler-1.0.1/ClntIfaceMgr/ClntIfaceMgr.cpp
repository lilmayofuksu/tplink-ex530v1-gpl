...
using namespace std;
extern unsigned long randomIaid;
TClntIfaceMgr * TClntIfaceMgr::Instance = 0;

unsigned long macMapToLong(const char *mac, int len)
{	
	if(len < 6)
	{
		return 1;
	...
	unsigned long num = (unsigned char)mac[2];
	num = num*256 + (unsigned char)mac[3];
	num = num*256 + (unsigned char)mac[4];
	num = num*256 + (unsigned char)mac[5];
    ...
    : TIfaceMgr(xmlFile, false)
{
	...
	if(ptr!=NULL)
	{
		randomIaid = macMapToLong(ptr->mac, ptr->maclen);
	}
...
#ifdef CMM_MSG
		/* ֻ���滷�ؽӿں�ָ����wan�ӿ� */
		if ((strcmp(ptr->name, intfName) == 0) || (ptr->flags & IFF_LOOPBACK) ){

			Log(Debug) << "find match iface: " << ptr->name << LogEnd;
#endif /* CMM_MSG */
...
bool TClntIfaceMgr::modifyPrefix(int iface, SPtr<TIPv6Addr> prefix, int prefixLen,
                                 ...
                                 TNotifyScriptParams* params /*= NULL*/)
{
#ifdef CMM_MSG	/* Add by ZDB: sent prefix to COS, do nothing here, 21Dec11 */
	Log(Debug) << "PD: " << prefix->getPlain() << "/" << (int)prefixLen
                << ", do nothing here as we will set prefix in cos!" << LogEnd;
	return true;
#endif /* CMM_MSG */
