...
TClntIfaceIface::TClntIfaceIface(char * name, int id, unsigned int flags, char* mac,
  ...
  :TIfaceIface(name, id, flags, mac, maclen, llAddr, llAddrCnt, globalAddr, globalAddrCnt, hwType),
   TunnelEndpoint(), LifetimeTimeout(DHCPV6_INFINITY) {
    ...
    this->Timezone = "";
	this->MaxInfRequestTimeout = SOL_MAX_RT;
	this->MaxSolicitTimeout = INF_MAX_RT;
...
bool TClntIfaceIface::setMaxSolicitTimeout(SPtr<TDUID> duid, SPtr<TIPv6Addr> srv, unsigned int value) {
	if (value < RT_RANGE_MIN || value > RT_RANGE_MAX){
		Log(Info) << "Set Interface SOL_MAX_RT value invaild" << LogEnd;
		return false;
	}
	
	this->MaxSolicitTimeout = value;

    Log(Info) << "Set Solicit Message Max Retransminssion time " << this->MaxSolicitTimeout << " seconds ." << LogEnd;
    return true;
}

bool TClntIfaceIface::setMaxInfRequestTimeout(SPtr < TDUID > duid, SPtr < TIPv6Addr > srv, unsigned int value){
	if (value < RT_RANGE_MIN || value > RT_RANGE_MAX){
		Log(Info) << "Set Interface INF_MAX_RT value invaild" << LogEnd;
		return false;
	}
	
	this->MaxInfRequestTimeout = value;

    Log(Info) << "Set Infomation Request Message Max Retransminssion time " << value << " seconds ." << LogEnd;
    return true;
}

unsigned int TClntIfaceIface::getMaxSolicitTimeout(){
	return MaxSolicitTimeout;
}

unsigned int TClntIfaceIface::getMaxInfRequestTimeout(){
	return MaxInfRequestTimeout;
}