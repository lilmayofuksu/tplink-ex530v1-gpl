        ...
        switch (mode) {
...
#ifdef CMM_MSG	/* Add by ZDB: sent prefix to COS */
                if (isUsed == false) 
                {
                    memcpy(&dhcp6cInfoMsgBody.prefix.addr, prefix->getPrefix()->getAddr(), sizeof(struct in6_addr));
                    dhcp6cInfoMsgBody.prefix.plen =  prefix->getPrefixLength();
                    dhcp6cInfoMsgBody.prefix.pltime = prefix->getPref();
                    dhcp6cInfoMsgBody.prefix.vltime = prefix->getValid();
                    dhcp6cInfoMsgBody.prefix.mode = IPV6_INFO_ADD;
            ...
            case TClntIfaceMgr::PREFIX_MODIFY_UPDATE:
                ...
                if (isUsed == false)
                {
                    ...
                    if (prefix->getValid())
                    {
                        dhcp6cInfoMsgBody.prefix.mode = IPV6_INFO_UPDATE;
                        isUsed = true;
                    }
                    else
                    {
                        dhcp6cInfoMsgBody.prefix.mode = IPV6_INFO_DEL;
                    }
