...
bool TClntOptIA_NA::doDuties() {
    ...
    while (ptrOptAddr = this->getAddr() ) {
		...
		/* add by lwj,05Jan16. If IA_NA option preferred-lifetime > valid-lifetime, it is invalid, ignore it.*/
		if (ptrOptAddr->getValid() < ptrOptAddr->getPref()){
			Log(Info) << "IA_NA IAAddr preferred-lifetime > valid-lifetime, ignore it." << LogEnd;
			continue;
		}
        ...
        if (!ptrAddrAddr) {
...
#ifdef CMM_MSG	/* Add by ZDB: sent address to cos.  */
				if (isUsed == false) /* 若存在多个地址，我们只取第一个信息。*/
				{
					memcpy(&dhcp6cInfoMsgBody.ip.addr, ptrOptAddr->getAddr()->getAddr(), 16);
					dhcp6cInfoMsgBody.ip.plen = prefixLen;
					dhcp6cInfoMsgBody.ip.pltime = ptrOptAddr->getPref();
					dhcp6cInfoMsgBody.ip.vltime = ptrOptAddr->getValid();
					dhcp6cInfoMsgBody.ip.mode = IPV6_INFO_ADD;
            ...
            if (ptrIface){
				...
				if (isUsed == false) 
				{
					...
					dhcp6cInfoMsgBody.ip.pltime = ptrOptAddr->getPref();
					dhcp6cInfoMsgBody.ip.vltime = ptrOptAddr->getValid();
					dhcp6cInfoMsgBody.ip.mode = IPV6_INFO_UPDATE;
					dhcp6cInfoMsgBody.status |= DHCP6C_ASSIGNED_ADDR;
					isUsed = true;
				}
#endif /* CMM_MSG */
