    ...
    if (cfgIfaceCnt) {
        ...
        while(cfgIface = parser->ClntCfgIfaceLst.get()) {
            ...
            if (is_addr_tentative(ifaceIface->getName(), ifaceIface->getID(), tmp)
             ...
             /*
             * Although the information cannot be obtained through DHCPv6 when M and O are both 0, 
             * the current platform does not support other ways to obtain DNS and PD
             */
            if (obeyRaBits()) 
            {
                if (ifaceIface->getMBit() || ifaceIface->getOBit()) 
                {
                    mBit = ifaceIface->getMBit();
                    oBit = ifaceIface->getMBit() || ifaceIface->getOBit();
                    cfgIface->setMbit(mBit);
                    cfgIface->setObit(oBit);
                }
            }
...
int TClntCfgMgr::countAddrForIA(unsigned long IAID)
{
...
SPtr<TClntCfgIA> TClntCfgMgr::getIA(unsigned long IAID)
{
    SPtr<TClntCfgIface> iface;
    firstIface();
    while (iface = getIface() )
...
SPtr<TClntCfgPD> TClntCfgMgr::getPD(unsigned long IAID)
{
    SPtr<TClntCfgIface> iface;
    firstIface();
    while (iface = getIface() )
...
bool TClntCfgMgr::setIAState(int iface,unsigned  long iaid, enum EState state)
{
    firstIface();
...
SPtr<TClntCfgIface> TClntCfgMgr::checkInactiveIfaces()
{
    ...
    while (x = InactiveLst.get()) {
        ...
        if (iface->flagUp() && iface->flagRunning() && iface->getLLAddress()) {
            ...
            {
                managed = !obeyRaBits() || iface->getMBit();
                otherConf = !obeyRaBits() || iface->getMBit() || iface->getOBit();
            }
