...
long TClntCfgIA::newID(){
    static unsigned long nextIAID=randomIaid;
    return nextIAID++;
}
...
unsigned long  TClntCfgIA::getIAID() {
    return IAID;
}

void TClntCfgIA::setIAID(unsigned long iaid) {
    IAID=iaid;
}
...
TClntCfgIA::TClntCfgIA(SPtr<TClntCfgIA> right, unsigned long iaid)
