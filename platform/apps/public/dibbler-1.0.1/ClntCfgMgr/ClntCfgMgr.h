...
class TClntCfgMgr : public TCfgMgr
{
 ...
 public:
    ...
    // --- Iface related ---
    SPtr<TClntCfgIA> getIA(unsigned long IAID);
    SPtr<TClntCfgPD> getPD(unsigned long IAID);
    ...
    //IA related
    bool setIAState(int iface, unsigned long  iaid, enum EState state);
    int countAddrForIA(unsigned long IAID);
