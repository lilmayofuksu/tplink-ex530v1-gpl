...
bool TClntAddrMgr::delIA(unsigned long IAID) {
...
bool TClntAddrMgr::delPD(unsigned long IAID) {
				...
				int length, bool quiet)
{
	...
	/* if update prefix is not exist in the addrMgr, add it; 
	 * if the valid time of prefix is zero, delete it,add by lwj, 30Dec15 */
	 if (!prefix) {
        Log(Error) << "Attempt to update null prefix failed." << LogEnd;
        return false;
    }
    ...
    if (!pd) {
        Log(Info) << "PD: can,t find PD of Prefix " << prefix->getPlain() << " . Add it to update." << LogEnd;
        return TAddrMgr::addPrefix(ptrClient, srvDuid, srvAddr, ifname, iface, IAID, T1, T2, prefix,
                                    pref, valid, length, quiet);
    }
	...
	if (!ptrPrefix) {
        Log(Info) << "PD: Prefix " << prefix->getPlain() << " is not known. Add it to update." << LogEnd;	
        return TAddrMgr::addPrefix(ptrClient, srvDuid, srvAddr, ifname, iface, IAID, T1, T2, prefix,
			     					pref, valid, length, quiet);
    }

	if (valid == 0){
		Log(Info) << "PD: Prefix " << prefix->getPlain() << " vaildtime is zero. Delete it to update." << LogEnd;
		return  TAddrMgr::delPrefix(ptrClient->getDUID(), IAID, prefix, quiet);
	}
