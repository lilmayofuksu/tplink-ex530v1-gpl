...
#ifdef CMM_MSG	/* Add by ZDB: do nothing here as we set address in cmm, 14Nov15.*/
...
int is_addr_tentative(char * ifacename, int iface, char * addr)
{
	...
	/* modify by lwj,For decline Message, if we don't find the address in rtb,
	   we think it DAD failed then the address already be removed */
    //int tentative = LOWLEVEL_TENTATIVE_DONT_KNOW;
	  int tentative = LOWLEVEL_TENTATIVE_YES; 
