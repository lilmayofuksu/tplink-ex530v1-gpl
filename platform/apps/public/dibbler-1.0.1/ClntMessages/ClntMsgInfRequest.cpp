...
TClntMsgInfRequest::TClntMsgInfRequest(SPtr<TClntCfgIface> iface)
	...
	/* get MRT from INF_SOL_MAX option ,add by lwj*/
	SPtr<TClntIfaceIface> clntIface = (Ptr*)ClntIfaceMgr().getIfaceByID(this->Iface);
    if (clntIface) {
        MRT = clntIface->getMaxInfRequestTimeout();
    }
