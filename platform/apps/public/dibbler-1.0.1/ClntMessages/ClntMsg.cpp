...
/* MAP-T */
#ifdef INCLUDE_IPV6_MAP 	/* Add map-t support, 13Nov15 */
		   ...
		   :TMsg(iface, addr, buf, bufSize)
{
	...
	case OPTION_S46_CONT_MAPT:
		{
			Log(Debug) << "Option " << code<< " in message " << MsgType << LogEnd;
            ptr = new TClntOptMapt(code, buf+pos, length, this);
            break;
        }
	...
	/* Add for SOL_MAX_RT & INF_MAX_RT option, lwj*/
	case OPTION_SOL_MAX_RT: {
			Log(Debug) << "Option" << code << " in message " << MsgType << LogEnd;
			ptr = new TClntOptMaxSolicitTimeout(buf+pos, length, this);
			break;
		}

	case OPTION_INF_MAX_RT: {
			Log(Debug) << "Option" << code << " in message " << MsgType << LogEnd;
			ptr = new TClntOptMaxInfRequestTimeout(buf+pos, length, this);
			break;
		}
...
void TClntMsg::setDefaults()
{
	...
	gettimeofday(&uLastTimeStamp, NULL);
...
{
     /* modify by lwj, calculate Msg timeout use usecond.
     long diff = (LastTimeStamp+RT) - time(NULL);
     return (diff<0) ? 0 : diff;
     */ 
	struct timeval currtime,difftime,rttime;
	
	rttime.tv_sec = uLastTimeStamp.tv_sec + RT;
	rttime.tv_usec = uLastTimeStamp.tv_usec;

	gettimeofday(&currtime, NULL);
	timersub(&rttime, &currtime, &difftime);

	if (difftime.tv_sec < 0){
		return 0;
	} 

	if (difftime.tv_usec >= 500000){
		difftime.tv_sec += 1;
	}

	Log(Debug) << "Msg timeout ulasttimestamp: " << uLastTimeStamp.tv_sec 
			   << " rttime: " << rttime.tv_sec << " difftime: " << difftime.tv_sec 
			   << " " << difftime.tv_usec << LogEnd;

	return difftime.tv_sec;
...
void TClntMsg::send()
{
	...
	/* modify by lwj, for CE-Router, RFC 3315, calculate RT time need less than the criterion. */
#ifdef INCLUDE_IPV6_CE_ROUTER_AUTH 
	if (!RC)
	RT=(int)(IRT);
    else
	RT =(int)(2.0*RT);
    ...
    if (MRT != 0 && RT>MRT)
	RT = (int)(MRT);
#else /* INCLUDE_IPV6_CE_ROUTER_AUTH */
...
#endif /* INCLUDE_IPV6_CE_ROUTER_AUTH */
	...
	Log(Debug) << "Send " << this->getName() << " RC:" << RC << " RT:" << RT << " MRT:" << MRT 
		<< " MRD:" << MRD << " MRC: " << MRC << LogEnd;
    this->storeSelf(pkt);
...
void TClntMsg::appendRequestedOptions() {
	...
	if ( (*gen)->State == STATE_NOTCONFIGURED ||
		...
		Log(Debug) << "Opion type:" << (*gen)->OptionType << "**************" << LogEnd;
...
void TClntMsg::answer(SPtr<TClntMsg> reply)
{
...
#ifdef INCLUDE_IPV6_MAP 
	memset(&mapMsgBody, 0, sizeof(DHCP6C_MAP_DOMAIN)); 
	strncpy(mapMsgBody.intfName, iface->getName(), sizeof(mapMsgBody.intfName));
#endif /* INCLUDE_IPV6_MAP */
#endif /* CMM_MSG */
...
#ifdef CMM_MSG	/* Add by ZDB: sent DNS SERVERS to COS */
...
#ifdef CMM_MSG	/* Add by lwj: sent SNTP SERVERS to COS */
				List(TIPv6Addr) sntpSrvsAddrs = ntpservers->getAddrLst();
 				SPtr<TIPv6Addr> sntpSrvAddr;
				int sntpCnt = 0;

				sntpSrvsAddrs.first();
    			while (sntpSrvAddr = sntpSrvsAddrs.get()) 
				{
					if (sntpCnt >= 2)
					{
						Log(Debug) << "Too many SNTP address, Only send two of them to cos." << LogEnd;
						break;
					}
				
					memcpy(&dhcp6cInfoMsgBody.sntp[sntpCnt++], sntpSrvAddr->getAddr(), sizeof(struct in6_addr));
       			}
  
				if (sntpCnt > 0) 
				{
					dhcp6cInfoMsgBody.status |= DHCP6C_ASSIGNED_SNTP;
				}			
#endif /* CMM_MSG */
                break;
            }
...
#ifdef CMM_MSG	/* Add by lwj: sent Domain search list to COS */
				List(std::string) domainLst = domains->getDomainLst();
 				SPtr<string> domain;
				int domainLenCnt = 0;

				domainLst.first();
    			while (domain = domainLst.get()) 
				{
					if (domainLenCnt == 0)
					{
						if ((strlen(domain->c_str()) + 1) > sizeof(dhcp6cInfoMsgBody.domain))
						{
							Log(Debug) << "Too many domain search list, send nothing to cos." << LogEnd;
							break;
						}
						sprintf(dhcp6cInfoMsgBody.domain, "%s", domain->c_str());
						domainLenCnt += strlen(domain->c_str());
					}
					else
					{
						if ((domainLenCnt + strlen(domain->c_str()) + 2) > sizeof(dhcp6cInfoMsgBody.domain)) 
						{
							Log(Debug) << "Too many domain search list, Only send " << domainLenCnt 
										<< " chars of them to cos." << LogEnd;
							break;
						}
						
						sprintf(dhcp6cInfoMsgBody.domain + domainLenCnt, ",%s", domain->c_str());
						domainLenCnt += strlen(domain->c_str()) + 1;
					}
       			}
  
				if (domainLenCnt > 0) 
				{
					dhcp6cInfoMsgBody.status |= DHCP6C_ASSIGNED_DOMAIN;
				}			
#endif /* CMM_MSG */
                break;
            }
	...
	case OPTION_AFTR_NAME:
		{
			SPtr<TOptDomainLst> dsliteOpt = (Ptr*)option;
	...
	case OPTION_SOL_MAX_RT:
		{	
            SPtr<TClntOptMaxSolicitTimeout> maxSolicitTimeoutOpt = (Ptr*)option;
			if (maxSolicitTimeoutOpt) {
			    maxSolicitTimeoutOpt->doDuties();
			}
			break;
		}
	...
	case OPTION_INF_MAX_RT:
		{
			SPtr<TClntOptMaxInfRequestTimeout> TClntOptMaxInfRequestTimeout = (Ptr*)option;
			if (TClntOptMaxInfRequestTimeout) {
			    TClntOptMaxInfRequestTimeout->doDuties();
			}
			break;
		}
	...
	case OPTION_S46_CONT_MAPT:
		{
			...
			Log(Debug) << "Received OPTION_S46_CONT_MAPT" << LogEnd;
			SPtr<TClntOptMapt> maptOpt = (Ptr*)option;
			//if (clntOpt->getStatusCode()!=STATUSCODE_SUCCESS) {
			//Log(Warning) << "Received OPTION_S46_CONT_MAPT(" << clntOpt->getOptType() << ") with non-success status:"
			//	     << clntOpt->getStatusCode() << ", ignored." << LogEnd;
			//break;
			//}

			if (maptOpt)
			{
				// configure received map-t option 
				#ifdef INCLUDE_IPV6_MAP_MAPE
				memcpy(&mapMsgBody.transportMode, "Translation", sizeof(mapMsgBody.transportMode));
				#endif /*INCLUDE_IPV6_MAP_MAPE*/
				
				maptOpt->doDuties();
			}

			// delete request for OPTION_S46_RULE, if it was mentioned in Option Request
			if ( optORO && (optORO->isOption(OPTION_S46_CONT_MAPT)) ) 
			{
				optORO->delOption(OPTION_S46_CONT_MAPT);
			}
	...
	case OPTION_S46_CONT_MAPE:
		{
			...
			if (maptOpt) 
			{
				// configure received map-e option 
				memcpy(&mapMsgBody.transportMode, "Encapsulation", sizeof(mapMsgBody.transportMode));
				maptOpt->doDuties();
			}

			// delete request for OPTION_S46_RULE, if it was mentioned in Option Request
			if ( optORO && (optORO->isOption(OPTION_S46_CONT_MAPE)) ) 
			{
				optORO->delOption(OPTION_S46_CONT_MAPE);
			}
...
#ifdef CMM_MSG	/*Send message to COS, 13Nov15 */
	if (dhcp6cInfoMsgBody.status != 0) {
		...
		Log(Debug) << "dibbler client send dhcp6c message to cos!" << LogEnd;
		msg_connCliAndSend(CMSG_ID_COS, &dhcp6cMsgFd, &dhcp6cMsg);
	}
#ifdef INCLUDE_IPV6_MAP 
	/*Send map message to COS */
	if (mapMsgBody.status != 0) {
		memset(&dhcp6cMsg, 0, sizeof(dhcp6cMsg));
		dhcp6cMsg.type = CMSG_IPV6_MAP_STATUS;
		memcpy(dhcp6cMsg.content, &mapMsgBody, sizeof(mapMsgBody));
		Log(Debug) << "dibbler client send map message to cos!" << LogEnd;
		msg_connCliAndSend(CMSG_ID_COS, &dhcp6cMsgFd, &dhcp6cMsg);
	}
#endif /* INCLUDE_IPV6_MAP */
#endif /* CMM_MSG */
