    ...
    :TClntMsg(iface, addr, SOLICIT_MSG)
{
	...
	/* Get MRT for interface,add by lwj */
	SPtr<TClntIfaceIface> clntIface = (Ptr*)ClntIfaceMgr().getIfaceByID(iface);
    if (clntIface) {
        MRT = clntIface->getMaxSolicitTimeout();
		Log(Debug) << "SET SOLICIT Message with MRT " << MRT << LogEnd;
    }
...
{
	/* add for rfc7083, when advertise contained option SOL_MAX_RT, we should Set it */
	SPtr<TClntOptMaxSolicitTimeout> maxSolRt = (Ptr*)msg->getOption(OPTION_SOL_MAX_RT);
	if (maxSolRt){
		unsigned int maxSolValue = maxSolRt->getValue();
		if (maxSolValue >= RT_RANGE_MIN && maxSolValue <= RT_RANGE_MAX){
			Log(Debug) << "Message contained option SOL_MAX_RT(" << maxSolRt->getValue() << "), Set SOL_MAX_RT" << LogEnd;
			maxSolRt->doDuties();
			MRT = maxSolRt->getValue();
		} else {
			Log(Debug) << "SOL_MAX_RT vlaue invalid " << maxSolRt->getValue() << LogEnd;
		}
	}
