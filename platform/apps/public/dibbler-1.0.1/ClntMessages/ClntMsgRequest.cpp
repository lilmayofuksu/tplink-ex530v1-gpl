...
TClntMsgRequest::TClntMsgRequest(List(TAddrIA) IAs,
	...
	/* modify by lwj, for Segmentation fault */
    ...
    {
    	/* Modify by lwj, add support for IA_PD option*/
    	if (IATYPE_IA == ClntAddrIA->getIAType()){
		...
		} else if (IATYPE_PD == ClntAddrIA->getIAType()){
        	SPtr<TClntOptIA_PD> IA_PD = new TClntOptIA_PD(ClntAddrIA, this);
        	Options.push_back((Ptr*)IA_PD);
		}
    }
...
void TClntMsgRequest::answer(SPtr<TClntMsg> msg)
{
    ...
    */
	...
	/* add by lwj, When the client recives a UnspecFail status from the server in response to a Request, 
	 * the client should re-issue the Request Message; When the client recives a UseMulticast status,
	 * the client should resend REQUEST message with Multicast address; When the client recive a NotOnLink 
	 * status, the client can either re-issue the Request without specifying any addresses or restart 
	 * the DHCP server discovery process, our dibbler will re-issue the Request. RFC 3315 18.1.8
	 */
	SPtr<TOptStatusCode> optStateCodeReply = (Ptr*) msg->getOption(OPTION_STATUS_CODE);
	if (optStateCodeReply){
		if (STATUSCODE_UNSPECFAIL == optStateCodeReply->getCode()){
			Log(Info) << "Recives Replay Message with StatusCode UnspecFail! Return"<< LogEnd;
			return ;
		}else if (STATUSCODE_USEMULTICAST == optStateCodeReply->getCode()){
			Log(Info) << "Recivces Replay Message with StatusCode UseMulticast!!" << LogEnd;
			PeerAddr_.reset();
			return ;
		}else if (STATUSCODE_NOTONLINK == optStateCodeReply->getCode()){
			Log(Info) << "Recivces Replay Message with StatusCode NotOnLink!! " << LogEnd;
			return ;
		}
	}
