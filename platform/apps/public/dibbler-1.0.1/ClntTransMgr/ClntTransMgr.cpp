...
void TClntTransMgr::removeExpired() {
    ...
    while (ptrIA = ClntAddrMgr().getIA()) {
        ...
        while (ptrAddr = ptrIA->getAddr()) {
			...
			if (strcmp(ptrIface->getName(), intfName) == 0)
			{
				...
				Log(Debug) << "Address is expired: " << ptrAddr->get()->getPlain() << "/" << 
							(int)ptrIface->getPrefixLength() << ", notify cos to delete it" << LogEnd;
				dhcp6cMsg.type = CMSG_IPV6_DHCP6C_STATUS;
				strncpy(dhcp6cInfoMsgBody.intfName, intfName, sizeof(dhcp6cInfoMsgBody.intfName));
    ...
    while (ptrPD = ClntAddrMgr().getPD()) {
	    ...
	    ClntIfaceMgr().delPrefix(ptrPD->getIfindex(), ptrPrefix->get(), ptrPrefix->getLength(),
                &params);
#ifdef CMM_MSG	/* Add by ZDB, sent expired prefix info to cos */
			if (strcmp(ptrIface->getName(), intfName) == 0)
			{
				memset(&dhcp6cMsgFd, 0 , sizeof(CMSG_FD));
				memset(&dhcp6cMsg, 0 , sizeof(CMSG_BUFF));
				memset(&dhcp6cInfoMsgBody, 0, sizeof(DHCP6C_INFO_MSG_BODY));
				memcpy(&dhcp6cInfoMsgBody.prefix.addr, ptrPrefix->get()->getAddr(), sizeof(struct in6_addr));
				dhcp6cInfoMsgBody.prefix.plen = ptrPrefix->getLength();
				dhcp6cInfoMsgBody.ip.mode = IPV6_INFO_DEL;
				dhcp6cInfoMsgBody.status |= DHCP6C_ASSIGNED_PREFIX;
				Log(Debug) << "prefix is expired: " << ptrPrefix->get()->getPlain() << "/" << 
							(int)ptrPrefix->getLength() << ", notify cos to delete it" << LogEnd;
				dhcp6cMsg.type = CMSG_IPV6_DHCP6C_STATUS;
				strncpy(dhcp6cInfoMsgBody.intfName, intfName, sizeof(dhcp6cInfoMsgBody.intfName));
				memcpy(dhcp6cMsg.content, &dhcp6cInfoMsgBody, sizeof(DHCP6C_INFO_MSG_BODY));
				msg_connCliAndSend(CMSG_ID_COS, &dhcp6cMsgFd, &dhcp6cMsg);	
...
void TClntTransMgr::sendRequest(List(TAddrIA)IAs, SPtr < TDUID > srvDUID, int iface)
{
    SPtr<TClntMsg> ptr = new TClntMsgRequest(IAs, srvDUID, iface);
    Transactions.append( (Ptr*)ptr );
}
...
void TClntTransMgr::checkSolicit() {
    ...
    while( (iface=ClntCfgMgr().getIface()) )
    {
		...
		/* RFC7084 : WPD-4. Add by rj 26Nov15 */
    	SPtr<TIfaceIface> ifaceIface = ClntIfaceMgr().getIfaceByID(iface->getID());
        SPtr<TClntCfgPD> pd4;		
		bool MorO = false;
		bool WPD4 = false;	
        ...
        iface->firstPD();		
        while ( pd4 = iface->getPD() ) {
            if (pd4->getState()==STATE_NOTCONFIGURED && MorO) {
				WPD4 = true;
            }
        }
		Log(Debug) << "WPD4 = " << WPD4 << LogEnd;
		
        if (iface->stateless() && !WPD4)
            continue;
		/* End */
        ...
        while(cfgIA=iface->getIA())
        {
            ...
            if(cfgIA->getState()==STATE_NOTCONFIGURED && !iface->stateless())
                iaLst.append(cfgIA);
        }
        ...
        if (iface->countTA() ) {
            iface->firstTA();
            ta = iface->getTA();
            if (ta->getState()!=STATE_NOTCONFIGURED && !iface->stateless())
                ta.reset();
        }
                ...
                while ( ptrAddr= ptrIA->getAddr() ) {
                    if (ptrAddr->getTentative() == ADDRSTATUS_YES) {
						...
						if (strcmp(ptrIface->getName(), intfName) == 0)
						{
							...
							memcpy(&dhcp6cInfoMsgBody.ip.addr, ptrAddr->get()->getAddr(), 16);
							dhcp6cInfoMsgBody.ip.plen = ptrAddr->getPrefix();
							dhcp6cInfoMsgBody.ip.mode = IPV6_INFO_DEL;
							dhcp6cInfoMsgBody.status |= DHCP6C_ASSIGNED_ADDR;
							Log(Debug) << "Address is tentative: " << ptrAddr->get()->getPlain() << "/" << 
										(int)ptrAddr->getPrefix() << ", notify cos to delete it" << LogEnd;
							dhcp6cMsg.type = CMSG_IPV6_DHCP6C_STATUS;
							strncpy(dhcp6cInfoMsgBody.intfName, intfName, sizeof(dhcp6cInfoMsgBody.intfName));
							memcpy(dhcp6cMsg.content, &dhcp6cInfoMsgBody, sizeof(DHCP6C_INFO_MSG_BODY));
							msg_connCliAndSend(CMSG_ID_COS, &dhcp6cMsgFd, &dhcp6cMsg);	
