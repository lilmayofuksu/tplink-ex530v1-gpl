...
TSrvOptIA_NA::TSrvOptIA_NA(SPtr<TSrvOptIA_NA> queryOpt, SPtr<TSrvMsg> queryMsg, TMsg* parent)
	...
	/* add by lwj, check the request addrss is in subnet of address pools */
	Log(Debug) << "QueryMsg Type: " << queryMsg->getType() << " parent Type: " << parent->getType() << LogEnd;
	if (queryMsg->getType() == REQUEST_MSG){
		SPtr<TOptIAAddress> optAddr = (Ptr*)queryOpt->getOption(OPTION_IAADDR);
		SPtr<TSrvCfgAddrClass> ptrClass;
		if (optAddr && optAddr->getAddr()){
			Log(Debug) << "IA_NA request contained address: " << (optAddr->getAddr())->getPlain() << LogEnd;
    		ptrClass = SrvCfgMgr().getClassByAddr(Iface, optAddr->getAddr());
			if (!ptrClass){
				Log(Info) << "IA_NA request address is valid for this link." << LogEnd;
				SubOptions.append(new TOptStatusCode(STATUSCODE_NOTONLINK,
                                  "Sorry, those addresses are not valid for this link.", Parent));
			}	
		}
	}

    // --- LEASE ASSIGN STEP 6: client's hint ---
    if (assignRequestedAddr(queryMsg, queryOpt, quiet)) {
        return;
    }
    
	// --- LEASE ASSIGN STEP 7: Cached address? ---
...
bool TSrvOptIA_NA::renew(SPtr<TSrvOptIA_NA> queryOpt, bool complainIfMissing)
{
	...
	/* For RFC3315 18.2.3, if request IA_NA address do not contained the invalid address, 
	   Server must return the address with lifetime 0,add by lwj, 24Dec15*/
	SPtr<TOptIAAddress> queryAddr = (Ptr*)queryOpt->getOption(OPTION_IAADDR);
	if (queryAddr && queryAddr->getAddr()){
		if (!ptrIA->getAddr(queryAddr->getAddr())){
			Log(Info) << "IA_NA query address " << queryAddr->getAddr()->getPlain() 
				      << " are invalid, set lifttime zero" << LogEnd;
			 SPtr<TOptIAAddress> optAddr;
        	 optAddr = new TSrvOptIAAddress(queryAddr->getAddr(), 0, 0,this->Parent);
        	 SubOptions.append( (Ptr*)optAddr );
