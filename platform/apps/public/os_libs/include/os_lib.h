...
#define LOCK_DUBUG_FILE_MAX_SIZE (16*1024)
#endif
...
#define OS_THREAD_POOL_COUNT_MIN (2)
#define OS_THREAD_POOL_COUNT_MAX (10)
#define OS_THREAD_POOL_WARN_MULTI (5)
#define OS_THREAD_POOL_WAIT_MULTI (20)

typedef void (*OS_THREAD_POOL_WORK_FUNC)(void *, void *);

typedef enum _OS_THREAD_PTHREAD_STATUS
{
	PTHREAD_STATUS_IDLE = 0,
	PTHREAD_STATUS_RUNNING,
} OS_THREAD_POOL_PTHREAD_STATUS;

typedef struct _OS_THREAD_POOL_WORK
{
	...
	void *arg1;
	void *arg2;
	OS_THREAD_POOL_WORK_FUNC func;
	struct _OS_THREAD_POOL_WORK *next;
} OS_THREAD_POOL_WORK;

typedef struct _OS_THREAD_POOL_PTHRAEAD
{
	int index;
	OS_THREAD_POOL_PTHREAD_STATUS status;
	int highPrio;
	OS_THREAD tid;
	struct _OS_THREAD_POOL_HANDLE *handle;
	struct _OS_THREAD_POOL_PTHRAEAD *next;
	struct _OS_THREAD_POOL_WORK *work;
} OS_THREAD_POOL_PTHREAD;

typedef struct _OS_THREAD_POOL_ATTR
{
	int stackSize;
	int highPrio;
	int lowPrio;
	int highPrioThreadCount;
} OS_THREAD_POOL_ATTR;

typedef struct _OS_THREAD_POOL_HANDLE
{
	int _pthreadCount;
	struct _OS_THREAD_POOL_ATTR attr;
	pthread_mutex_t mutex;
	pthread_cond_t highPrioCond;
	pthread_cond_t lowPrioCond;
	struct _OS_THREAD_POOL_WORK *highPrioQueue;
	struct _OS_THREAD_POOL_WORK *lowPrioQueue;
	struct _OS_THREAD_POOL_PTHRAEAD *pthreadList;
} OS_THREAD_POOL_HANDLE;
 ...
 */
 #ifdef INCLUDE_DEBUG_LOCK
 #define DBG_INFO_LEN 	512
 int os_writeDebugFile(char fileName[], int fileMaxSize, const char* fmt, ...);
 #define os_semVTake(sem) os_semVTakeDebug(sem, __FUNCTION__)
 ...
 */
 #ifdef INCLUDE_DEBUG_LOCK
  #define os_semVGive(sem) os_semVGiveDebug(sem, __FUNCTION__)
...
/* 
 * fn		int os_queueReceive(OS_MSGQ qid, int type, int timeout, int bufSize, char *pBuf)
