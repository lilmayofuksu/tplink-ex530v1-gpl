...
int cmmlog_attachFirewallLogShm(void **pShmAddr)
{
	...
	shmFlg = IPC_CREAT | 0666;
	shmId = os_shmGet(SYSLOG_FIREWALL_SHARED_MEM_KEY, (SYSLOGD_FIREWALL_SHM_LOG_LEN + sizeof(SYSLOGD_SHM_CONTROL_BLOCK)), shmFlg);
	if (shmId < 0)
	{
		printf("syslogd : get syslogd firewall shared buffer error.");
		return -1;
	}

	pAddr = os_shmAt(shmId, NULL, shmFlg);
	if (NULL == pAddr)
	{
		printf("syslogd : attach firewall shared buffer error\n");
		return -1;
	}

	ret = os_semVGet(SYSLOG_FIREWALL_SEM_KEY, &syslogSem);
	if (ret < 0)
	{
		printf("syslogd : get firewall semaphore fail\n");
		os_shmDt(pAddr);
		return -1;
	}

	ret = os_semVTake(syslogSem);
	if (ret < 0)
	{
		printf("syslogd : os_semVTake fail\n");
		os_shmDt(pAddr);
...
int cmmlogSendOfflineLog()
{
	...
	pLogMsg->type = LOG_SEND_OFFLINE_MSG;

	DEBUG_PRINT("syslogd : Send offline log to remote ...\n");
	msg_send(&msgFd, &msgBuf);
