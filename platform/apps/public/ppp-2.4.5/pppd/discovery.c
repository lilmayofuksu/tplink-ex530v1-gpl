...
#endif
extern unsigned int discoveryTimeout;
...
waitForPADO(PPPoEConnection *conn, int timeout)
{
	...
	if (packet.code == CODE_PADO) {
	    ...
	    if (conn->error)
	    {
			log_to_cmm(LOG_NOTICE, "received illegal PADO packet.");
			return;
	    }
	    if (!pc.seenACName) {
...
waitForPADS(PPPoEConnection *conn, int timeout)
{
	...
	if (packet.code == CODE_PADS) {
	    ...
	    if (conn->error)
	    {
			log_to_cmm(LOG_NOTICE, "received illegal PADS packet.");
			return;
	    }
...
#ifdef INCLUDE_PADT_PRECEDE
	g_sessionID = ntohs(packet.session);
...
discovery(PPPoEConnection *conn)
{
    ...
    int padrAttempts = 0;
#if 1
	int timeout = discoveryTimeout;
#else
    int timeout = conn->discoveryTimeout;
#endif /* INCLUDE_SPEC_EX530v */
	int ret;
	discover_stage = 1;
	...
	if (enablePadtPrecede && oldSession != 0xffff)
	{
	    /* send PADT */
		sendPADT(conn, NULL);
		memset(buf, 0, MAX_LOG_LEN);
