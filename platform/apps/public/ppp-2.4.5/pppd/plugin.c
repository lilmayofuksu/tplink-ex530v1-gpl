...
#ifdef INCLUDE_ADSL_REMOTE
static bool is_pppoa = 0;
static const uint8_t l_fakeRemoteMac[ETH_ALEN] = {0xfe, 0xff, 0xff, 0xff, 0xff, 0xfd};
#endif
...
#endif /* Ended by Yin Jun. */
#ifdef INCLUDE_ADSL_REMOTE
	{ "is_pppoa", o_bool, &is_pppoa, "PPPoE for PPPoA", 1 },
#endif
...
#ifdef INCLUDE_ADSL_REMOTE
	if (is_pppoa) {
		conn->session = htons(1);
		memcpy(conn->peerEth, l_fakeRemoteMac, ETH_ALEN);
	} else
#endif
    if (existingSession) {
