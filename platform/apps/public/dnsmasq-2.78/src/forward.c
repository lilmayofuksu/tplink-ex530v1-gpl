...
void reply_query(int fd, int family, time_t now)
{
  ...
  /* Modified by huangzhibin, Sep 5, 2019, follow official patch from dnsmasq */
#if 1
  if ((RCODE(header) == SERVFAIL || RCODE(header) == REFUSED) &&
      forward->forwardall == 0 &&
      !(forward->flags & FREC_HAS_EXTRADATA))
#else
  /* According to the behavior of dnsmasq-2.67,
   * if DNS server refused this query or query failed on DNS server,
   * replying result to LAN device rather than query another one
   * when daemon have option with OPT_ORDER.
   * If not so, CPU of DUT will 100% used due to the loop
   * while DUT forward query to DNS server and DNS server refuse this query. */
  /* Modified by huangzhibin, Jun 18, 2019 */
  if ((RCODE(header) == SERVFAIL || RCODE(header) == REFUSED) &&
      !option_bool(OPT_ORDER) &&
      forward->forwardall == 0 &&
      !(forward->flags & FREC_HAS_EXTRADATA))
  /* End modified by huangzhibin, Jun 18, 2019 */
#endif
  /* End modified by huangzhibin, Jun 18, 2019 */
    /* for broken servers, attempt to send to another one. */
    {
       ...
       * this executes. */
      /* Added by huangzhibin, Sep 5, 2019, follow official patch from dnsmasq */
      if (option_bool(OPT_ORDER))
      {
      ...
      /* End added by huangzhibin, Sep 5, 2019 */
      
      /* recreate query from reply */
      pheader = find_pseudoheader(header, (size_t)n, &plen, NULL, &is_sign, NULL);
      if (!is_sign && server) /* Modified by huangzhibin, Sep 5, 2019, follow official patch from dnsmasq */
	{
	  header->ancount = htons(0);
	  header->nscount = htons(0);
	  header->arcount = htons(0);
