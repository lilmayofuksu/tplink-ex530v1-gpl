...
void
ScanDirectory(const char *dir, const char *parent, media_types dir_types, int depth)
{
	int i = 0;
	int startID=0;
	...
	DPRINTF(E_DEBUG, L_SCANNER, _("scan dir = %s\n"), dir);
	if (depth > MAX_DIR_DEPTH)
	{
		printf("%s, %d: direcotry depth reaches MAX_DIR DEPTH(%d), ignore it!\n", __func__, __LINE__, depth);
		return;
	}
	...
	while (NULL != (pEntryItem = readdir(pDir)))
	{
		...
		{
		case ALL_MEDIA:
			flag = filter_avp(pEntryItem);
			break;
			
		case TYPE_AUDIO:
			flag = filter_a(pEntryItem);
			break;
			
		case TYPE_AUDIO|TYPE_VIDEO:
			flag = filter_av(pEntryItem);
			break;
			
		case TYPE_AUDIO|TYPE_IMAGES:
			flag = filter_ap(pEntryItem);
			break;
			
		case TYPE_VIDEO:
			flag = filter_v(pEntryItem);
			break;
			
		case TYPE_VIDEO|TYPE_IMAGES:
			flag = filter_vp(pEntryItem);
			break;
			
		case TYPE_IMAGES:
			flag = filter_p(pEntryItem);
			break;
		...
		type = TYPE_UNKNOWN;
		sprintf(full_path, "%s/%s", dir, pEntryItem->d_name);
		name = escape_tag(pEntryItem->d_name, 1);
		...
		{	
			...
			ScanDirectory(full_path, parent_id, dir_types, ++depth);
		}
...
void *start_scanner(void *arg)
{
	...
	for( media_path = media_dirs; media_path != NULL; media_path = media_path->next )
	{
			...
			|| (sql_get_int64_field(db, "SELECT count(*) from SETTINGS where KEY = 'media_dir' and VALUE = %Q", media_path->path) <= 0) )
		{
			...
			ScanDirectory(media_path->path, parent, media_path->types, 0);
