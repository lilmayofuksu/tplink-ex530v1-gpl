...
void update_scanDirectory(const char *dir, const char *parent, media_types dir_types)
{
	...
	while (NULL != (pEntryItem = readdir(pDir)))
	{
		...
		if (scan_mediaTypesIsCorrect(dir_types, pEntryItem) == 0)
		{
			/* Check if any caption added in the directory.  */
			if (isCaption(pEntryItem->d_name))
			{
		...
		if (is_playlist(pEntryItem->d_name))
		{
		...
		if( (type == TYPE_DIR) && (access(full_path, R_OK|X_OK) == 0) )
		{
			...
			ScanDirectory(full_path, parent_id, dir_types, 0);
		}
