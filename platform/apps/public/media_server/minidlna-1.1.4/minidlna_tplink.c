...
static BOOL mdConvertMediaDirsToAbsolute()
{
	...
	while (NULL != (pEntryItem = readdir(pDir)))
	{
		...
		if (!mdInitInfo.shareAll)
		{
			...
			snprintf(devPath, sizeof(devPath), "%s%s", DEV_DIR_PREFIX, pEntryItem->d_name);
											...
											sizeof(uuid), sizeof(type), sizeof(label)))
			{
				...
				while(media_path)
				{
					...
					snprintf (path, sizeof(path), "%s/%s%s", USB_DISK_DIRECTORY, pEntryItem->d_name, media_path->path);
		...
		else 
		{
			...
			snprintf (path, sizeof(path), "%s/%s", USB_DISK_DIRECTORY, pEntryItem->d_name);
