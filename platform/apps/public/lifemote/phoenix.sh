...
BACKOFF_INTERVAL_MAX=3600  # Maximum value for backoff interval
CHECK_INTERVAL=1800        # Interval for checking whether the script is alive

#use ./phoenix.sh URL to set $LIFEMOTE_AGENT_URL
...
fetch_and_run_script() {
    ...
    while true; do
        ...
        backoff=$(( PERIOD + 0x$rand_bytes % PERIOD ))
        
        echo "Failed. Backing off $backoff seconds..."
